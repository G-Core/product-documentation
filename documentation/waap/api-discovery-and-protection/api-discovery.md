...
If API Discovery isn't available in your plan and you can't automatically map APIs with API Baseline, use the **API base path** feature. It lets you manually add API paths that belong to your domain and should be protected by WAAP.
...
* **Settings**: A page with available configuration and customization options where you can set up scan settings, configure API base URL, and upload a Swagger file. 
...
* **Confirmed**: The endpoint is a valid API and should be protected by WAAP. If you manually add the endpoint in the API base path, it’ll automatically get this status. You can change the status at any time. 
...
For example, you can mark certain APIs with the **Indicate API Privileged User** or **Indicate API Admin User** tags to make those paths accessible only to privileged or admin users. Our Advanced API protection guide provides instructions on how to do that. 
...
5\. Enter a new or updated path. When editing a path, you can use curly braces to signify query string parameters. For example, `/api/v1/stacks/{stack_id}/sites/{site_id}`. 
...
For guidelines on how to configure API base path in Gcore WAAP, check out the Manually add endpoints to API base path guide. 
