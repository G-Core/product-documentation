...
<table>
  ...
  <tr><td>404</td><td>Not Found</td><td>There's no requested video, or the live stream is temporarily not delivering chunks. Check the request link or activate your video.<br/>For Live streams in CMAF format you can check extra header "X-Err-Code":<ul><li>1000 – Master-stream is missed. Stream is not pushed or is not transcoded, so start a stream or restart transcoding.</li><li>2000 – Invalid StreamID. Identifier is not parsed from the requested URL, check URL.</li><li>3000 – Stream is not ready for delivery. See the logs or contact support. Most often it happens when the master-stream has wrong parameters, check the master-stream parameters like video and audio codecs, FPS, bitrate, etc.</li></ul></td></tr>
