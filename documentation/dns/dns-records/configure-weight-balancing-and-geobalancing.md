...
Dash, should there be a link here? ^^ seems incomplete
2\. Enable the **Dynamic response** toggle.
Dash, I removed the warning because it exists on the advanced mode page.
...
5\. Set appropriate metadata to which the value in the query will be compared. Specify the value. You can add several metadata fields for one value by clicking the plus button.
Dash, specify what value exactly? It's a bit vague because we are talking about metadata here. A screenshot would also resolve the question.
<expandable-element title="Available metadata types">
...
If the metadata field “weight” is left empty, the default value 50 is used. This means each record has equal probability.
Dash, is it true even if there are more than 2 values considered?
