...
Note: You cannot change the network stack after the cluster has been created.
...
Dash, it's unclear - is there Basic DDoS Protection as standard and this is on top of that? Or is there no DDoS Protection unless it's enabled?
...
Dash, the image above is not clearly related to the text. I think we need to add 1 sentence to line 121 - it seems like it's not simply entering the SSH key into the field, instead they have to click to add the key to the list first, is that right? Seems like maybe there are really 3 options: pick an existing key, add a key to the dropdown, or create a new key.
...
Dash, check line 136. The 2 options were not parallel and didn't cover all options before.
