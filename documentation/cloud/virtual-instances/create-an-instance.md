...
pageDescription: Learn how to create a Linux or Windows VM in Gcore Edge Cloud. Start using your virtual machine in minutes with ease.
---
...
2\. Choose the region where you want to deploy your VM.
...
Oksana, please link line 33 "send us a request" to the appropriate way to contact.
...
- **ARM**: ARM architecture is designed for energy efficiency and low power consumption, also supporting strong performance, making it ideal for high-performance computing tasks. However, ARM instances are compatible with fewer OS distributions.
...
- **Intel® Xeon® Scalable, 3rd Gen or 2nd Gen** if you selected x-86-64 architecture in the previous step.
...
*   **Shared**. VMs that share the core of a physical machine with other VMs. Designed for workloads that do not require high performance.  
    Availability: Luxembourg
**Note:** The bandwidth limit for the **Shared** flavor is <100 Mbps; for other configurations, it is <1 Gbps.
...
*   **SGX**. VMs that support Intel SGX (Security Guard Extension) help to protect data from disclosure or modification by isolating private parts of code and data (enclaves). The best option for storing critical and/or sensitive data in the cloud.
...
8\. Configure an SSH key for a remote SSH connection. You can add an existing SSH key or generate a new one. For instructions on how to generate and configure the key, check out our guide on <a href="https://gcore.com/docs/cloud/virtual-instances/connect/connect-via-ssh" target="_blank">connecting to an VM via SSH</a>.
...
In addition to SSH keys, you can also set up a password for your VM, as described in [step 9](#configure-user-data). Setting a password is necessary if you want to connect to a Linux VM <a href="https://gcore.com/docs/cloud/virtual-instances/connect/connect-to-your-instance-via-control-panel#connect-to-a-linux-instance" target="_blank">from the Customer Portal</a>. 
...
On Cloud VMs with Windows OS, you can't use the `password` parameter both in the “Access” and “User data” fields. Since the "Access" field is required, configuring user data on Windows VMs is not possible. 
...
- **Anti-affinity** groups work the opposite way: All virtual machines in this group will be separated across different physical hardware. This increases the fault tolerance of a cluster: Even if something goes wrong with one server, machines on the other server(s) will remain available.
