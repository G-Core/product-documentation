...
ai-navigation: Enable Super Transit DDoS protection by configuring anycast tunneling, verifying router MSS clamping support, IRR entries, ASNs, and RPKI validation for on-p
ai-navigation: Configure Super Transit with anycast tunneling, MSS clamping, IRR entries, and ASN validation for DDoS protection.
---
...
To properly configure DDoS protection and traffic routing, we need details about your network setup and traffic patterns. Complete and submit the required information as described in our guide on [activating DDoS protection](/ddos-protection/onboarding-service/service-activation).
...
To establish a secure connection between Gcore's network and your infrastructure, configure tunnels on both the Gcore side and your router. Follow the instructions in the [Set Up a GRE Tunnel with Gcore](/ddos-protection/onboarding-service/gre-tunneling/set-a-gre-tunnel-with-gcore) guide.

## Step 7. Advertise prefixes

After submitting your [network information](/ddos-protection/onboarding-service/network-submission), we will process and announce your prefixes through the Gcore network using Border Gateway Protocol (BGP). Once completed, all incoming traffic will be routed through Super Transit for DDoS protection before reaching your infrastructure.