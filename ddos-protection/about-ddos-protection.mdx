...
| Maximum time to recognize an attack | Under 3 seconds | 3 minutes |
...
Advanced protection routes all traffic through a Threat Mitigation System (TMS) at all times, even when there's no attack. TMS immediately filters malicious traffic on detection, within 3 seconds, and forwards only legitimate traffic to the server, so the server stays online throughout an attack.
...
Always-on protection is available for L3-L7 protection layers. For custom configurations, contact the Gcore sales team to request a tailored plan.
