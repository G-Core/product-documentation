...
2. Review the list of available profiles:
   - **Default profiles** (static): System-managed profiles that cannot be modified or deleted. These profiles are identified by `managed_by: system` in API responses.
   - **Custom profiles** (dynamic): User-managed profiles that can be created, modified, and deleted. These profiles are identified by `managed_by: user` in API responses.
