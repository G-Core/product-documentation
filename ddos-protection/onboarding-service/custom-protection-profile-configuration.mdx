...
DDoS protection profiles are classified into two types based on management control:

### Static profiles (managed by Gcore)

Static profiles are preconfigured and maintained by Gcore. These profiles:

- Cannot be modified or deleted by clients
- Are optimized for common use cases and traffic patterns
- Receive automatic updates and security improvements
- Are identified by `"managed_by": "gcore"` in API responses

### Dynamic profiles (managed by client)

Dynamic profiles are created and managed by clients. These profiles:

- Can be fully customized, updated, and deleted by clients
- Require manual configuration and maintenance
- Allow complete control over rules and policies
- Are identified by `"managed_by": "client"` in API responses

### Identifying profile types

When working with the API, check the `managed_by` field in the response to determine the profile type:

```json
{
  "id": "profile-123",
  "name": "Custom Web Server Profile",
  "managed_by": "client",
  // other profile properties
}
```

**Important:** Attempts to modify or delete static profiles will result in API errors. Only dynamic profiles can be updated through the API or Customer Portal.
