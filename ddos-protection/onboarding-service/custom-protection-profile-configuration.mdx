...
Rules define traffic-matching criteria and apply a selected policy to matching traffic. Selecting the Basic template pre-populates four default rules that match TCP, UDP, ICMP, and other traffic to the corresponding default policy; add more rules to layer specific protections on top of these defaults.
...
Select a policy that matches the specific traffic pattern when adding a rule.
...
For the Basic template, the **Policy** tab includes the following fields:

- **GEOIP list**: countries to include in geo-based rate limiting.
- **Rate limiter low**: destination PPS limit for all traffic, from 1 to 50 kilo packets per second. Default: 50.
- **Rate limiter medium**: destination PPS limit for all traffic, from 50 to 150 kilo packets per second. Default: 150.
- **Rate limiter high**: destination PPS limit for all traffic, from 150 to 300 kilo packets per second. Default: 300.
- **Rate limiter geo**: destination PPS limit for GEO-matched traffic, from 1 to 300 kilo packets per second. Default: 300.
