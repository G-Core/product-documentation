...
Protection profiles are classified into two types based on their management model:

**Static profiles** are managed by Gcore and are read-only. You cannot update or delete static profiles. These profiles are typically provided as baseline templates or optimized configurations for specific use cases.

**Dynamic profiles** are managed by you and offer full control. You can create, update, and delete dynamic profiles to match your specific requirements.

### Identifying profile types

You can identify whether a profile is static or dynamic through:

- **Customer Portal**: Static profiles display with restricted actions (no edit or delete options)
- **API responses**: The profile object includes a `type` field indicating "static" or "dynamic"

### Operations by profile type

| Operation | Static profiles | Dynamic profiles |
|-----------|----------------|------------------|
| View | ✓ | ✓ |
| Create | — | ✓ |
| Update | — | ✓ |
...
**Note**: This static vs dynamic classification is separate from the default vs custom profile distinction. Both static and dynamic profiles can serve as default or custom configurations depending on your network setup.

## Step 1. Open protection profiles

1. In the [Gcore Customer Portal](https://accounts.gcore.com/reports/dashboard), navigate to **DDoS Protection** \> **Protection profiles**.
