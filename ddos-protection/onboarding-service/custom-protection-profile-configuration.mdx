...
Protection profiles define rules and policies based on network traffic and security requirements. There are two types of protection profiles:

- **Static profiles** (system-managed): Default profiles provided by Gcore that cannot be modified or deleted
- **Dynamic profiles** (user-managed): Custom profiles that you create, modify, and manage

When working with the DDoS Protection API, profiles include a `managed_by` field that indicates whether a profile is `system`-managed (static) or `user`-managed (dynamic).
...
2. Review the list of available profiles:
   - **Static profiles** (system-managed): Default profiles provided by Gcore
   - **Dynamic profiles** (user-managed): Custom profiles you've created
