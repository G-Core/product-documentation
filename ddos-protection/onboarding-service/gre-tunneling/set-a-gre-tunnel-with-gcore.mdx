...
ai-navigation: Configure a GRE tunnel and BGP peering session with Gcore on Cisco routers or Ubuntu Linux endpoints for DDoS Protection.
---
...
The following instructions configure the GRE tunnel and BGP session with Gcore.
...
The standard MTU size allowed on the internet is 1500 bytes. When using GRE, typically an additional 24 bytes are added to the packet, consisting of 20 bytes for an outer IP header and 4 bytes for GRE itself, so the maximum allowed packet size over the GRE tunnel is reduced to 1476 bytes. This example does not use encryption, so no additional headers apply; encryption would add 4 more bytes, which is out of scope for this article.

For TCP, the maximum segment size (MSS) value is 40 bytes less than the MTU size, as the TCP header is 40 bytes, so the MSS value must be explicitly specified in the configuration. In this example, the MSS value is 1436 bytes (1476 [the GRE MTU size] - 40 = 1436).
</Accordion>
...
For example, imagine Side A (AS 10) and Side B (AS 20) on the two ends of the tunnel. Side A provides DDoS Protection, so it accepts specific routes from clients, denies the default route for routing security, and does not advertise anything. Side B advertises only its own routes.

Configure Side A first. 
...
The configuration above establishes BGP sessions with Gcore over GRE tunnels, protecting applications and servers from DDoS attacks and other malicious traffic.

## Set a GRE tunnel on Ubuntu

Establish a tunnel on Linux-based servers functioning as endpoints in the network. 
