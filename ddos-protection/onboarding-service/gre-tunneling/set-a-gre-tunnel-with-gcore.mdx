...
Create a [BGP session](/ddos-protection/onboarding-service/bgp-sessions) in the Customer Portal and attach it to the protected network. The endpoint addresses in that session are the local routers Gcore peers with. The neighbor address in the example below is the BGP neighbor IP that Gcore provides.
