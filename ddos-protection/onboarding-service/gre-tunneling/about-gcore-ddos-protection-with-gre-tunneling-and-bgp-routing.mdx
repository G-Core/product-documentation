---
title: "About Gcore's DDoS protection with GRE tunneling and BGP routing"
sidebarTitle: "Overview"
ai-navigation: Configure GRE tunneling and BGP routing to protect network infrastructure from DDoS attacks globally, managing GRE encapsulation, MTU/MSS limits, and BGP autonomous system routing between endpoints.
---
...
[Generic Routing Encapsulation](https://gcore.com/blog/gre-tunneling-for-server-protection), or GRE, is a tunneling protocol that creates a virtual point-to-point connection between two endpoints over an IP network. The endpoints are usually routers or firewalls that are assigned IP addresses when connected with a GRE tunnel. Much like a real-life tunnel, a GRE tunnel acts as a pathway for data to travel from point A to point B.

<Frame>
  ![What is the GRE protocol](/images/docs/ddos-protection/gre-tunneling/overview/about-gcore-ddos-protection-with-gre-tunneling-and-bgp-routing-image1.jpg)
</Frame>
...
For more information about BGP, please refer to our [blog article](https://gcore.com/learning/what-is-bgp).
...
By default, DDoS protection only handles incoming traffic (from the Internet to the customer’s network). Outgoing traffic from the customer’s servers back to clients is routed through the customer’s data center edge router, using its default routing table and uplinks to the Internet or ISP. This return traffic does not pass through Gcore or its tunnels.

<Frame>
  ![What is the GRE protocol](/images/docs/ddos-protection/gre-tunneling/overview/about-gcore-ddos-protection-with-gre-tunneling-and-bgp-routing-image2.png)
</Frame>

## How does Gcore protect from DDoS attacks using GRE?

You send [us a request](https://gcore.com/contact-sales) and share the details of the internal networks you want to protect from DDoS attacks. Gcore configures its incoming filters to allow traffic originating from your internal networks. After BGP routing is configured, Gcore announces your networks on the internet. This means that everyone on the internet can see that your networks are now being routed through Gcore. All traffic to your networks is redirected to Gcore's scrubbing center.
...
To set up a GRE tunnel with the Gcore scrubbing center, please refer to [our step-by-step guide](/ddos-protection/onboarding-service/gre-tunneling/set-a-gre-tunnel-with-gcore).