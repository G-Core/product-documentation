...
Select the environment model by project size, access needs, and the required level of isolation. The table below maps each model to its best use case.
...
Use one account when the same team manages development and production assets. This model is appropriate when development and production assets do not require separate access boundaries, and keeps billing and operations in one place.
...
Use three or more accounts when the release process needs separate development, staging, production, or regional environments. Each account can have its own users, API tokens, assets, delivery settings, and billing setup. Service limits are shared by default and differ between accounts only when combined with custom plans.
