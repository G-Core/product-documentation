...
ai-navigation: Reference HTTP status codes 200, 403, 404, 422, 500, 502, 503, 504 returned by Gcore Video Streaming API when requesting manifests (.m3u8, .mpd) and chunks (.ts, .mp4), including geo-restriction, token authentication, CMAF format X-Err-Code headers, and transcoding error diagnostics.
---
