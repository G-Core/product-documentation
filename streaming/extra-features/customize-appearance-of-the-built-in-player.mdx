...
req = urllib.request.Request(
    "https://api.gcore.com/streaming/players",
    data=payload,
    method="POST",
...
print(f"Speed ctrl:  {player['speed_control']}")
```
  </Tab>
  <Tab title="Go SDK">
```go
...
func main() {
    ...
    body := `{"player":{"name":"My custom player","autoplay":false,"mute":false,"speed_control":true,"show_sharing":false,"bg_color":"#000000","fg_color":"#FFFFFF","hover_color":"#ef9047"}}`
    ...
    _ = json.Unmarshal(raw, &player)

    fmt.Printf("Player ID:   %v\n", player["id"])
    fmt.Printf("Name:        %v\n", player["name"])
    fmt.Printf("Speed ctrl:  %v\n", player["speed_control"])
}
```
  </Tab>
  <Tab title="curl">
