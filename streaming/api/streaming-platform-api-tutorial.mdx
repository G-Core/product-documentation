...
ai-navigation: Create streams and broadcasts using Video Streaming API endpoints post_streams_id and post_broadcasts, activate streams with patch_streams_id, update broadcast status to live with patch_broadcasts_id, and handle webhooks with live status notifications for video streaming integration.
---
...
You'll need to have:
  ...
  * A Gcore account with an active [Streaming](/account-settings/account/create-account) subscription
  * An [authentication](/api-reference/streaming) set up using the user credentials
  * A [webhook](/streaming/extra-features/get-webhooks-from-the-streaming-platform) set up
...
Use the [post_streams_id](/api-reference/streaming) method to create a stream object in the Video Streaming.
...
Use the [post_broadcasts](/api-reference/streaming) method to create a broadcast object.
...
Having received a successful response from your app, the user can now publish the stream. Use the [patch_streams_id](/api-reference/streaming) method to activate the stream.
...
Use the [patch_broadcasts_id](/api-reference/streaming) method to update the broadcast status to _Live_.
...
Use the [put_streams_id_start_recording](/api-reference/streaming) method to begin recording.
...
Use the [put_streams_id_stop_recording](/api-reference/streaming) method to stop recording
...
Use the [patch_broadcasts_id](/api-reference/streaming) method to update the broadcast status to Finished.
...
Use the [patch_streams_id](/api-reference/streaming) method to deactivate the stream.
...
Use the [delete_broadcasts_id](/api-reference/streaming) method to delete the broadcast.
...
Use the [get_api_videos_id](/api-reference/streaming) method to retrieve detailed information about the video.
...
The user can reuse the same stream for each broadcast. In this case, you don't need to create a new one. Just update the stream using the [patch_streams_id](/api-reference/streaming) method with the appropriate transcoding region.
...
```json
{
  ...
  * You can also check for the stream status using the [get_streams_id](/api-reference/streaming) method. If the response contains **"active": false** and the stream wasn't manually interrupted, then you can call the [patch_broadcasts_id](/api-reference/streaming) method to transition the broadcast to "Paused" status.
...
You can delete streams that have been inactive for some time and are associated with a deleted user account. To do this, use the [delete_api_videos_id](/api-reference/streaming) method.

If the returned HTTP status code is 204, the request is successful.

## Other API methods

Check out our Video Streaming API [documentation](/api-reference/streaming) for more methods that might come in handy for other use cases.