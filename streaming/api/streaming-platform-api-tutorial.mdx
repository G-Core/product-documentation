...
<Steps>
  ...
  <Step title="Enable the stream">
    Once the stream and broadcast are created, the user can publish the stream. The [Change&nbsp;live&nbsp;stream](/api-reference/streaming/streams/change-live-stream) endpoint (`PATCH /streaming/streams/{id}`) activates the stream.
...
Streams that have been inactive for some time and are associated with a deleted user account can be deleted. The [Delete&nbsp;video](/api-reference/streaming/videos/delete-video) endpoint (`DELETE /streaming/videos/{id}`) removes the associated recording.
