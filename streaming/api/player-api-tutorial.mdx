...
ai-navigation: Configure Gcore JS Video Player (@gcorevideo/player) to support HLS, LL-HLS, MPEG-DASH, LL-DASH, and MP4 playback with low-latency adaptive streaming, plugins, SDK methods, events, and error handling via hls.js, dash.js, and native video element.
---
...
This player does not collect or send usage statistics from end user devices. Viewing statistics are available from CDN data. See the apidocs section [Get unique viewers via CDN](/api-reference/streaming/statistics/get-live-unique-viewers-via-cdn).
