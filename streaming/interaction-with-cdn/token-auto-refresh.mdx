...
ai-navigation: Configure auto-refresh video secure tokens with short TTL (60 seconds) and IP-bound mode to prevent link sharing in HLS DASH streams; refresh tokens automatically in the player before expiry to invalidate copied URLs while maintaining uninterrupted playback.
---

<Info>
This article extends the [Video&nbsp;secure&nbsp;token](/streaming/interaction-with-cdn/video-secure-token) guide, which covers enabling the feature, URL format, and token generation. This article picks up from there and shows how to keep tokens short-lived and auto-refreshed so copied URLs become worthless within seconds.
</Info>

Short-lived tokens are the most effective defence against link sharing during live events or pay-per-view streams. Set the token to expire in 60 seconds, refresh it automatically in the player before it expires, and anyone who copies the URL from the browser or network inspector will have a useless link within a minute — while authenticated viewers experience uninterrupted playback.
...
![Secure token protection](/images/docs/streaming/interaction-with-cdn/token-auto-refresh/cdn-token-protected.png)
...
For maximum protection, use short-lived IP-bound tokens — a shared URL is both nearly expired and locked to the original viewer's IP. The tradeoff: the shorter the token lifetime, the more sensitive the system becomes to poor network conditions on the viewer's end.
...
- **A backend endpoint** that generates fresh signed URLs on demand (the server holds the secret key and verifies that the user is still authorized).
- **Player-side refresh logic** that calls the backend endpoint shortly before the current token expires and updates all outgoing URLs transparently — without stopping or reloading the stream.
...
Token generation must run server-side — the CDN secret key must never be sent to the browser. The backend must verify that the requesting user has a valid session and the right to access the specific video _before_ generating a token.


### What is Gcore FastEdge?

[Gcore FastEdge](https://gcore.com/fastedge) is a serverless edge-compute service that runs WebAssembly applications at CDN edge locations worldwide. Apps compile to WASM, start in microseconds with no cold starts, and respond in milliseconds. Billing is per request with no idle cost.
...
<Info>
**Need user authentication?** The demo FastEdge app generates tokens for anyone who calls it — there is no login check. To restrict access to authenticated users only (subscribers, paid accounts, ticket holders), implement a backend endpoint that verifies the user session or JWT before generating a token. FastEdge can still be used for that, but the authentication logic must be added to the app. Any backend stack works — the token generation formula is the same regardless of language.
</Info>
...
This FastEdge app is bound to the Gcore demo CDN resource and signs tokens for the `demo-protected.gvideo.io` domain only. Use it for testing and integration purposes — it cannot be used with a custom CDN resource or other content.
...
The demo endpoint is a Rust WebAssembly application deployed on FastEdge. Use it as a starting point for a custom token API.
...
<Warning>
**Production requirement:** Add an authentication check before generating a token. Verify that the requesting user has a valid session and is authorised to access the specific video. Without this check, any caller can request a token for any asset.

For user authentication, implement a backend that validates the session and returns the signed URL from the server — never expose the CDN secret key to the client.
</Warning>

## Player integration

The examples below use the demo API. Replace `TOKEN_API_URL` with an authenticated backend endpoint before deploying to production.
