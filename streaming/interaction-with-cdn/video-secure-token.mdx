...
ai-navigation: Implement video secure tokens to restrict access by user authentication, expiration time, and per-user tokens; use Custom CDN resource for geo-blocking, refe
ai-navigation: Enable video secure tokens to restrict access by user authentication, expiration time, and referrer domain for paid, private, or time-limited content.
---
...
Secure token can be enabled **only on a custom CDN resource**. You cannot enable it on the default Video Streaming domain.

To enable secure token:

1. Activate the **CDN** product on a **paid tariff**
2. Create a new **custom CDN resource** for your streaming origin
3. Enable **secure token** in that custom CDN resource
4. Follow the [Custom CDN resource](/streaming/interaction-with-cdn/custom-cdn-resource) guide and contact [Gcore Support](mailto:support@gcore.com)

Support will help configure `Streaming Rules` to route your streaming content correctly.

![Custom CDN resources](/images/docs/streaming-platform/integrated-cdn/cdn-resources.png)
...
Examples of templates of protected links for LIVE:
  ...
  * `https://domain.com/cmaf/{account_id}_{stream_id}/{token}/{expiration}/`**004chunk-stream4-10000000-05213.m4s?part=4**
...
```sh
https://domain.com/videos/account_id_video_id/filename.mp4?md5={token}&expires={expiration}
 ```
...
## Preventing link sharing with auto-refreshed tokens

Setting a long expiration solves the "session resumed after pause" problem, but it creates a different risk: a viewer can share the signed URL during a live event, and anyone with that link can watch for as long as the token remains valid.

The solution is to use short-lived tokens (60 seconds or less) and refresh them automatically in the player. Viewers on your platform get uninterrupted playback; anyone who copies the URL gets a 403/410 error within a minute.

See **[Auto-refresh secure tokens](/streaming/interaction-with-cdn/token-auto-refresh)** for step-by-step integration examples using the Gcore Video Player, hls.js, and dash.js — including a ready-to-deploy Gcore FastEdge token API written in Rust.
...
|---------------|----------|----------------------------------------------------------------------------|
| **account_id** | yes      | Your account ID                                                             |
...
<Tabs>
  <Tab title="Python">
    ...
    def gethash(account_id, video_id, secret, expires):
      hash_body = "%s_%s_%s_%s_" % (account_id, video_id, secret, expires)   # set of unique parameters of video
