...
Reset the token when it is compromised or when revoking encoder access without deleting the stream. To reset, call the [reset push token endpoint](/api-reference/streaming/streams/reset-live-stream-push-token):
