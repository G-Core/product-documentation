...
Gcore supports two recording methods for live streams:

| Method | Description | Availability |
|--------|-------------|--------------|
| **Transcoded recording** | Records the processed stream with all overlays, texts, and logos. Standard quality renditions are generated. | Available to all users |
| **Original recording** | Records the raw RTMP/SRT source without transcoding. Preserves original quality, framerate, and codecs. | Premium feature (contact support to enable) |

<Tabs>
<Tab title="Record transcoded stream">

## What is transcoded recording

Transcoded recording saves your live stream after processing. The recording includes any overlays, texts, or logos applied during the broadcast. Gcore generates multiple quality renditions for playback.

Use transcoded recording when you need:
- Standard VOD delivery with adaptive bitrate
- Recordings that include visual overlays
- Long-term storage (recordings persist indefinitely)
...
<Tab title="Record original stream">

## What is original recording
...
## Benefits of original recording
...
## How to enable original recording
...
For API details, see [Create stream](/api-reference/streaming/streams/create-stream) and [Start recording](/api-reference/streaming/streams/start-recording).

## Where to find original recordings

After recording completes, the original file URL is available in the `origin_url` field of the video entity. Retrieve it via the [Get video API](/api-reference/streaming/videos/get-video):

```json
{
  "id": 12345,
  "name": "Stream Record: My live stream, 2026-04-21 12:00:00",
...
Download or transfer original files within the 7-day window if you need them for archival or editing.

## Use cases

- **Video editing**: Download raw MP4 immediately after broadcast for post-production
- **Archival**: Preserve original quality footage before transcoding
- **Quality control**: Review source stream quality without transcoding artifacts
- **Compliance**: Maintain unmodified recordings for regulatory requirements
