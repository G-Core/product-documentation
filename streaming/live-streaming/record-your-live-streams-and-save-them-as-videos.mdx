...
1\. In the Gcore Customer Portal, navigate to [Streaming](https://portal.gcore.com/streaming/streaming) > **Live Streaming**. 
...
|-----------|----------|-------------|
| `name` | Yes | Current stream name — required by the SDK even when not changing it |
...
stream = client.streaming.streams.update(
    stream_id,
    stream={
        "name": "My live stream",   # name is required by the SDK for updates
        "auto_record": True,
