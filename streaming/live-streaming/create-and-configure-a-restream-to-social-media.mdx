...
ai-navigation: Create restreams to broadcast live streams to YouTube, Facebook, Twitch via RTMP/RTMPS using stream URL and stream key credentials.
---
...
You get the credentials from the target platform (Facebook, YouTube, Twitch, etc.): stream URL and stream key. Then create a restream in the Gcore Customer Portal. When creating the restream, add credentials and choose the appropriate live stream. The setup is complete. You can begin the live stream; it will be broadcast on our player and the target platform.
...
5\. Enter the credentials from step 1 of this guide as follows:
...
By default, we enable one available restream per account. If you want to use more than one, contact technical support and specify the number of restreams.
