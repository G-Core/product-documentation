...
The push token is the secret part of the ingest URL. It authenticates the stream source for RTMP/RTMPS, SRT, and WebRTC WHIP ingest. Only one push token exists for a stream at a time, and the token must be protected like any other credential. Anyone with a valid push URL can try to publish to that stream.
...
Use the following API method: https://gcore.com/docs/api-reference/streaming/streams/reset-push-token
...
<Frame>![Geo distributed ingest points for PUSH](/images/docs/streaming-platform/live-streaming/ingest-points.png)</Frame>
