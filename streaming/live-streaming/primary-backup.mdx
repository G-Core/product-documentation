...
ai-navigation: "Configure PUSH and PULL ingest points for Gcore Streaming Platform using RTMP, RTMPS, SRT, or WebRTC WHIP protocols with stream keys and push tokens for live video ingestion from encoders or origin servers."
---
...
Legacy RTMP cameras that don't support query strings in the URI accept an alternative path-based format, where the push token is a path segment instead of a query parameter:
...
Use the following API method: [/api-reference/streaming/streams/reset-live-stream-push-token](/api-reference/streaming/streams/reset-live-stream-push-token)
...
1. Primary stream active. Primary always takes priority over the backup stream.
...
Pay attention to rules of ingest server behavior for security reasons:
...
"PULL URL" field can contain 1 link, or many at once. In most cases 1 point is used. Multiple points are needed for large broadcasts with multiple backups. You can specify multiple addresses separated by a space (" "), so you can organize an extra-backup plan.
...
3. The system will try to pull the origin stream for 2 hours approximately, making requests approximately every 3 seconds.
