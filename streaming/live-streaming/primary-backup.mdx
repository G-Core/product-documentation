...
In PUSH mode, your encoder initiates the connection to the platform ingest server. You configure the encoder with a target URL and stream key, and the server listens for that incoming connection. [https://gcore.com/docs/api-reference/streaming/streams/reset-live-stream-push-token#reset-live-stream-push-token](https://gcore.com/docs/api-reference/streaming/streams/reset-live-stream-push-token#reset-live-stream-push-token) The push token is the secret part of the ingest URL. It authenticates the stream source for RTMP/RTMPS, SRT, and WebRTC WHIP ingest. Only one push token exists for a stream at a time, and the token must be protected like any other credential. Anyone with a valid push URL can try to publish to that stream.
...
```/api-reference/streaming/streams/reset-live-stream-push-token#reset-live-stream-push-token
...
Use the following API method: [Reset live stream push token](/api-reference/streaming/streams/reset-live-stream-push-token#reset-live-stream-push-token)
...
<Frame>
  ![Geo distributed ingest points for PUSH](/images/docs/streaming-platform/live-streaming/ingest-points.png)
</Frame>
