...
ai-navigation: Configure PUSH and PULL ingest for Gcore live streams, reset push tokens, create PULL streams with failover URIs, and check active ingest regions via the Customer Portal or REST API using Python SDK, Go SDK, and curl.
---
...
Use **PUSH** when your encoder, broadcast software, app, or automation agent sends the live stream to Gcore. Use **PULL** when Gcore must connect to your public origin URL and fetch the stream from your server.
...
For a first quick test, use PUSH with RTMP: it is the most common encoder workflow and requires only an ingest URL. Replace the URL with your stream's `push_url` in the command below. It generates a test video pattern and a test audio tone, encodes them as H.264/AAC, and pushes the stream to Gcore:
...
Expected result in the Control Panel:
...
For AI agents and automated backends, use this default flow:
...
2. Configure your encoder with the server ingest URL and your unique stream key or token.
...
PUSH ingest URLs are returned in the stream object and are also available in the control panel. Use the exact URL for the protocol selected in the stream settings:
...
For enterprise projects, the ingest URL format can be adapted to existing business or encoder requirements. For example, if your current workflow already uses this style:

```text
rtmp(s)://<ip:port>/app/<uuid>
```

Gcore can provide a compatible format such as:

```text
rtmp://vp-push-anx1.gvideo.co/app/76ac4a8b-5ff3-9028-2a4b-a31a23e5900a
```

Contact your account manager or support team if you need a custom ingest URL format.

### Reset the PUSH Token

Reset the push token if it was exposed, shared with the wrong encoder, or needs to be rotated by your security process. After a reset:
...
To reset the token via the Customer Portal, open the stream settings and click **Reset token**. Via the API, use `POST /streaming/streams/{stream_id}/reset_push_token` — see the REST API tab in the [PULL section](#pull) below for code examples that cover both operations.
...
In most cases 1 primary point is used for broadcasting. Multiple points are needed for large broadcasts with multiple backups.
 Streaming Platform offers a special failover mechanism for RTMP and SRT origin streams. You can push primary and backup origin streams simultaneously to organize a backup plan.
...
By default, all ingests are connected to one of the data centers in Europe, regardless of the location of your encoder.

Enterprise customers only have the option to select an alternative ingest point in other regions.
 Here are the additional locations you can connect:

- Europe Luxembourg, The Netherlands
- US Ashburn, Miami
- Singapore (location under development)

Reach out to your account manager for setup assistance.
...
<p>Set `pull: true` and provide one or more source URLs in the `uri` field, separated by spaces. The platform uses round-robin failover across all listed URIs.</p>
...
func main() {
    ...
    stream, err := client.Streaming.Streams.Update(context.TODO(), streamID, streaming.StreamUpdateParams{
...
req = urllib.request.Request(
    f"https://api.gcore.com/streaming/streams/{stream_id}/reset_push_token",
    method="POST",
...
print(f"New push URL (RTMP): {stream['push_url']}")
print(f"New push URL (SRT):  {stream['push_url_srt']}")
print(f"New push URL (WHIP): {stream['push_url_whip']}")
```
  </Tab>
  <Tab title="Go SDK">
```go
...
func main() {
    ...
    req, _ := http.NewRequest(http.MethodPost,
        fmt.Sprintf("https://api.gcore.com/streaming/streams/%d/reset_push_token", streamID),
        nil,
    )
    ...
    fmt.Printf("New push URL (RTMP): %s\n", stream["push_url"])
    fmt.Printf("New push URL (SRT):  %s\n", stream["push_url_srt"])
}
```
  </Tab>
  <Tab title="curl">
