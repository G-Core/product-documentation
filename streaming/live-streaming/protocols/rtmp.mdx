...
title: "RTMP"
sidebarTitle: "RTMP"
ai-navigation: Configure RTMP and RTMPS ingest for live streaming encoders using server URLs, stream keys, and H.264 video plus AAC audio codec on ports 1935 (RTMP/TCP) and 443 (RTMPS/TCP).
---
...
Gcore Live Streaming supports both RTMP and RTMPS. RTMPS is wrapped in TLS for encrypted transport, which helps traverse restrictive firewalls.
...
<Info>
  ...
  We plan to support H.265/HEVC and other extensions from the Enhanced RTMP specification. Stay tuned for updates.
</Info>

<Info>
  When the RTMP encoder connects, live transcoding reads the source FPS once from the incoming stream metadata at the beginning of the session. That FPS value is not re-read later and is not changed dynamically during transcoding. Gcore does not upscale a 10-12 fps source to 50 or 60 fps. If the source FPS was degraded when the session started, the customer must reconnect the stream from the encoder side after fixing the source settings.
</Info>
...
Ingest link looks like `rtmp://vp-push-ed1.gvideo.co/in/12345?aaabbbcccddd` and contains:
...
- exact stream key `12345?aaabbbcccddd`

For legacy RTMP cameras that don't support query strings, the push token can be provided as a path segment instead: `rtmp://vp-push-ed1.gvideo.co/in/12345/aaabbbcccddd`.
...
Ingester timeout limit is 3 sec (3000ms). If there is no data in the connection for this period, the connection will be discarded as inactive. Your encoder will need to reconnect again.
...
For more information, see [Primary, Backup, and Global Ingest Points](/streaming/live-streaming/primary-backup)
...
## Demo RTMP Streaming: How LowLatency Encoder Tuning Impacts Delivery

This short demo shows how optimized RTMP encoder settings reduce end-to-end latency and improve stream resilience. 
Two streams are compared: one with standard settings and one configured for low latency. You’ll see how the low-latency stream starts faster, reacts better under packet loss, and why these settings are crucial for stable, real-time live streaming.
...
In the video OBS RTMP2 lowlatency tuning settings are:
  ...
  <tbody>
    <tr>
      ...
      <td>
        <ul>
          ...
          <li>Check the video and audio output codecs in your encoder for RTMP output.</li>
    ...
    <tr>
      ...
      <td>
        ...
        Follow [step 3 of the guide](/streaming/live-streaming/create-a-live-stream#step-3-configure-your-stream-for-push-pull-or-webrtc-to-hls).
      </td>
    </tr>
    <tr>
