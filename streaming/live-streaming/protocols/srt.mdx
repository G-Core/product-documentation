...
title: "SRT"
sidebarTitle: "SRT"
ai-navigation: Configure SRT (Secure Reliable Transport) UDP-based protocol for live streaming ingestion with encoder settings including video codec, bitrate, latency window, keyframe interval, CPU preset, profile, tune, and x264/x265 options; obtain ingest URLs with server location, port 5001, stream key, and backup URL via portal or API; configure optional passphrase protection via API for SRT PUSH workflows.
---
...
For SRT PUSH streams, you can require a passphrase on the ingest connection. When `srt_passphrase` is set for a live stream, your encoder must send the same passphrase while connecting to the SRT ingest URL. The incoming MPEG-TS stream is then AES-encrypted by SRT between the sender and the Gcore ingester.

This protects the ingest path from your encoder to Gcore. It does not encrypt playback URLs and does not replace viewer-side access protection.
...
Then send the same passphrase from your encoder. In FFmpeg, pass it as a separate output option:
...
For more information, see [Ingest & Backup](/streaming/live-streaming/primary-backup)
...
<Frame>
![Video frame compression algorithm](/images/docs/streaming-platform/live-streaming/protocols/srt/compressed-frames.png)
</Frame>
...
<Frame>
![Lookahead buffer window of 40 frames](/images/docs/streaming-platform/live-streaming/protocols/srt/lookahead-40.png)
</Frame>
...
<Frame>
![Lookahead buffer window is minimized](/images/docs/streaming-platform/live-streaming/protocols/srt/lookahead-0.png)
</Frame>
