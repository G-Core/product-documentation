...
ai-navigation: Convert WebRTC WHIP ingest streams to HLS/DASH playback using Gcore Video Streaming platform with H.264 video and OPUS audio encoding, adaptive bitrate streaming, and regional WHIP servers for low-latency browser-based ingestion.
---
...
WHIP is an open standard, so streams can be published with the Gcore browser client or another compatible WHIP client.

  * Gcore JS WebRTC WHIP client. The [GitHub repository](https://github.com/G-Core/gcore-webrtc-sdk-js) contains source code, examples, and integration docs. The current version is listed on the [releases](https://github.com/G-Core/gcore-webrtc-sdk-js/releases) page. To load the hosted JavaScript module, remove the leading `v` from the release tag: [v0.89.14](https://github.com/G-Core/gcore-webrtc-sdk-js/releases/tag/v0.89.14) maps to `https://rtckit.gvideo.io/0.89.14/index.esm.js`.
...
The Gcore product documentation covers the full live stream setup in [Live Streaming](/streaming/live-streaming/create-a-live-stream) and low-latency playback links in [low-latency](/streaming/live-streaming/how-low-latency-streaming-works).
...
Get more details in [WebRTC to HLS/DASH](/streaming/live-streaming/webrtc-to-hls-dash) tutorial.
...
Since WHIP is an open standard, many libraries have been released for it in different languages. Use the Gcore WebRTC WHIP client or any other library specified in the [WHIP clients](#supported-whip-clients) section.

We provide a WebRTC WHIP library for working in browsers. It implements the basic calls and simplifies working with WebRTC:
...
The latest library version is listed on the GitHub releases page. For version 0.89.14, use `https://rtckit.gvideo.io/0.89.14/index.esm.js`.
...
The ingestion server returned an error, which can be identified by inspecting the `status` and `detail` fields of the error object. WHIP publish requests can also return a non-200 HTTP response, so inspect the HTTP status code and the `X-Reason-Code` response header before retrying.
...
<Frame>![Example of servers check in Chome WebRTC internals](/images/docs/streaming-platform/live-streaming/protocols/webrtc/check-sever-config.png)</Frame>
