...
<Frame>![Example of live broadcast](/images/docs/streaming-platform/live-streaming/protocols/webrtc/go-live-button.png)</Frame>
...
Since WHIP is an open standard, many libraries have been released for it in different languages. You can use our [WebRTC WHIP](https://rtckit.gvideo.io/0.72.0/index.esm.js) or any other library specified in the [WHIP clients](#supported-whip-clients) section. 
...
|------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 400              | The client’s request is wrong. It may be due to the incorrect request parameters sent by the WHIP client. If you see this error with an unintelligible description or with no description at all, contact the [Gcore support team](mailto:support@gcore.com). | **err.message**: Server request failed with status 400 <br/> **err.status**: 400 |
| 401              | The endpoint you are trying to connect to is unreachable. <br/> <br/> Probable causes based on the `X-Reason-Code` response header: <ul><li>**2001**: Stream does not exist.</li></ul> | **err.message**: Server request failed with status 401 <br/> **err.status**: 401 <br/> **X-Reason-Code**: 2001 |
| 403              | The endpoint you are trying to connect to is unreachable<br/> <br/> Probable causes based on the `X-Reason-Code` response header: <ul><li>**2002**: Stream token is invalid</li><li>**2003**: Someone else is already streaming. A stream was not closed correctly, but you push to that broken stream (time out is ±30 seconds. In this case, try again after that time).</li><li>**2004**: Stream disabled</li></ul> | **err.message**: Server request failed with status 403 <br/> **err.status**: 403 <br/> **X-Reason-Code**: 2002                                                                                                      |
