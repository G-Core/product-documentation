...
sidebarTitle: DVR
ai-navigation: Enable DVR on live streams to allow viewers to pause and rewind broadcasts, configure the DVR window duration, and clear the DVR cache via the Customer Portal or REST API using Python SDK, Go SDK, and curl.
---
...
**To enable DVR when creating a new stream**, pass the same fields in `POST /streaming/streams`:
