...
title: "FFmpeg"
sidebarTitle: "FFmpeg"
ai-navigation: Configure FFmpeg command-line parameters for RTMP live streaming to Gcore Video Streaming, including video bitrate (2000Kbps for 720p, 4000Kbps for 1080p), audio bitrate (128Kbps), H.264 encoder, keyframe interval (2s), CPU preset (veryfast), and resolution downscaling via bicubic filter.
---
...
2. To get the server URL and stream key, go to the [Streaming list](https://portal.gcore.com/streaming/streaming/list), open the **Live stream settings** you need, and copy the relevant value from the **URLs for the encoder** section.
...
- **Video Bitrate:** To stream at 720p resolution, set the bitrate to 2000Kbps (`-b:v 2000k`). If you’re broadcasting at 1080p, set the bitrate to 4000Kbps (`-b:v 4000k`)
