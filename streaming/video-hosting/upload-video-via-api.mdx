...
ai-navigation: Upload videos to Gcore Video Hosting via API using three methods - copy from external storage, upload from local device, or batch upload for migration; specify input parameters, codecs, protocols, video audio profiles, GOP structure, bitrate constraints, and set account-level priority for transcoding queue scheduling.
---

## How is Gcore video hosting organized?

Video Hosting refers to cloud storage of videos that are ready to be distributed and watched on end devices via the public internet. You can upload original video files in [different formats](/streaming/live-streams-and-videos-protocols-and-codecs/input-parameters-and-codecs), we process the videos in various ways, and we save the result to our internal storage. As a result, you get videos in [HLS/MP4 format](/streaming/video-hosting/hls-and-mp4) with [subtitles](/streaming/video-hosting/subtitles-and-closed-captions-for-vod), [timeline previews](/streaming/video-hosting/timeline-hover-previews-use-in-players-and-roku-devices), and other features available.

You can upload videos to the storage via the Gcore Customer Portal or API. Learn how to [add a video via the Gcore Customer Portal](/streaming/video-hosting/upload-a-video-and-embed-it-to-your-app).
...
<Info>
The original video provided from your side must strictly comply with the input parameters and codec requirements described in the [Input Parameters and Codecs](/streaming/live-streams-and-videos-protocols-and-codecs/input-parameters-and-codecs) documentation.  

Only videos that follow these specifications (supported protocols, video/audio codecs, profiles, GOP structure, bitrate constraints, and stable source parameters) can be accepted by the uploader servers and reliably transcoded for further distribution. Any deviation from the documented requirements may cause the connection to be rejected or lead to unstable transcoding and delivery.

These parameters must remain stable through the whole source file. Reprocessing cannot fix a file whose pixel format, color range, codec profile, or similar video properties change in the middle of the file.  
</Info>


## Video upload and processing statuses

Statuses of video processing are described in the [Video Status article](/streaming/video-hosting/vod-status).
...
Below, we explain how to [get HTTP public access links](/streaming/video-hosting/upload-video-via-api#batch-upload-to-migrate-a-vast-number-of-videos-from-other-services) in some popular external storages. 
...
-H 'Authorization: APIKey 1234_0d16599c' \
-d '{ 
  "video": { 
    "name": "Gcore Demo", 
...
**Note** : Please be aware of [RPS limits for using API](/api-reference/overview).
...
Once successfully uploaded and processed, the video will be available in the Customer Portal. You can check the status of the video by the [GET API request](/api-reference/streaming/videos/get-video) or via webhook [notifications](/streaming/extra-features/get-webhooks-from-the-streaming-platform). 
...
<Frame>![CodePen template](/images/docs/streaming/video-hosting/upload-video-via-api/codepen-example.png)</Frame>
...
Read more on the page [Generate and translate AI captions for a new VOD](/streaming/ai-video-service/generate-ai-subtitles-and-add-them-to-video/generate-captions-in-customer-portal).
