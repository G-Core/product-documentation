...
ai-navigation: Enable AES-128 encryption for HLS video delivery by configuring a keyserver to retrieve decryption keys via Gcore API, with EXT-X-KEY tags added to m3u8 playlists and optional country or domain access policies for additional security.
---
...
This request will allow your keyserver to retrieve the decryption key, which should then pass to the viewer in its original format. An example of a decryption key is a 16-byte binary value. In hexadecimal notation it looks like:

```
57d655a9f13c293342b53434a1b20f4e
```
