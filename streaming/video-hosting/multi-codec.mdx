...
ai-navigation: Check multi-codec quality presets and access codec-filtered HLS and MPEG-DASH manifests via the Streaming API for HEVC, AV1, and VP9 VOD delivery.
---
...
<p>Every video object returned by <code>GET /streaming/videos/{id}</code> includes an <code>hls_url</code> field. Append a codec suffix to serve a manifest containing only one codec family — useful when targeting specific device types or running A/B tests.</p>
