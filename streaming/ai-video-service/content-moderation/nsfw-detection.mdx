...
<p>Send `POST /streaming/ai/tasks` with `task_name: "content-moderation"` and `category: "nsfw"`. The response returns a `task_id` to poll for results.</p>
...
<p>Use `GET /streaming/ai/tasks/{task_id}` to check task status. Poll until `status` is `"SUCCESS"` or `"FAILURE"`. On success, check the `result` field for NSFW detection details.</p>
...
if task.status == "SUCCESS":
    ...
    print(f"Detections:    {task.result.detection_results}")
```
  </Tab>
  <Tab title="Go SDK">
```go
