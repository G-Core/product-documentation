...
ai-navigation: Detect child sexual abuse materials (CSAM) in MP4 videos using Gcore AI content moderation task, which combines soft nudity and hard nudity detection to identify obscene content involving children's faces, returning frame numbers, age categories, and confidence probability scores of at least 30%.
---

The child pornography content moderation task detects child sexual abuse materials (CSAM). 

To identify this content, we first run the [soft nudity](/streaming/ai-video-service/content-moderation/soft-nudity-detection) and [hard nudity detection](/streaming/ai-video-service/content-moderation/soft-nudity-detection) tasks. If both methods indicate the presence of obscene content with the involvement of children (child's face) in a frame, then such a video is marked as obscene. Frames are designated by the age category of identified children. 
...
<Frame>![Child pornography detection task details](/images/docs/streaming-platform/ai-video-service/ai-nudity-detection/child-pornography-detection.png)</Frame>