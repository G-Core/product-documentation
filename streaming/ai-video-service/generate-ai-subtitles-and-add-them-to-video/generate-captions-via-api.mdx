...
sidebarTitle: Using video upload parameters
ai-navigation: Generate AI captions and translate subtitles for VOD videos in Gcore Video Streaming using transcribe and translate parameters in video upload or subtitle post-processing APIs, or process external MP4 files via direct HTTPS links with the native AI API.
---
...
## Fast subtitle creation using AI
...
<Accordion title="Show advanced instructions">
...
<Info>
This is an advanced feature. To enable it, contact [Gcore support](mailto:support@gcore.com).
</Info>
...
Ensure you replace the example values `{your_api_key}` and `{your_video_url}/{quality}.mp4` with your actual [API token](/account-settings/api-tokens) and MP4 video URL.
