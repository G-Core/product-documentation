...
- **Python SDK — verified method names (sdk version 0.41.0, physically tested):**
  ...
  - `task.created_resources.baremetal_servers[0]` → WRONG. BM servers are also in `task.created_resources.instances[0]`
  ...
  - SDK `instances.create()` and `baremetal.servers.create()` use `name_template` (str, singular); REST API uses `name_templates` (array)
  - Flavor names are region-specific: Frankfurt-2 uses `g1-standard-2-4`, Luxembourg-3 uses `g2-standard-2-4`
  - Go SDK cannot be tested in non-interactive Windows environment (installer requires GUI); examples taken from auto-generated x-codeSamples in YAML spec
...
- **Never write a closing sentence that points to the next article.** "With these basics in place, the next step is to create an API token and make the first call." — the sidebar already shows what comes next. The reader does not need to be told. Stop when the content is done.
