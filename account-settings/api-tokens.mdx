...
Our API documentation describes how to get a [standard authorization token](/api-reference/account#section/Authentication) with a validity period of 1 hour, to update it, use the [Refresh token](/api-reference/account#tag/Account/paths/~1auth~1jwt~1refresh/post) request with a validity period of 24 hours. Such validity periods can be helpful for one-time requests.
...
Please refer to the specific product [API documentation](/api-reference/iam) to check if it supports permanent API tokens.
