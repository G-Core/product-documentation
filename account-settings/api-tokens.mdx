...
Our API documentation describes how to get a [standard authorization token](/api-reference/iam/account/login) with a validity period of 1 hour, to update it, use the [Refresh token](/api-reference/iam/account/refresh-token) request with a validity period of 24 hours. Such validity periods can be helpful for one-time requests.
