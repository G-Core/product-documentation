...
Gcore MCP Server enables AI assistants to manage Gcore services through natural language commands. The server implements the Model Context Protocol (MCP), exposing approximately 720 Gcore API operations as callable tools for provisioning infrastructure, querying resources, and chaining multi-step workflows.
...
A [Gcore API key](/account-settings/api-tokens) with appropriate permissions is required. Tool execution is synchronous—long operations block until completion.
