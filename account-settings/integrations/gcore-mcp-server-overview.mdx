...
<Frame>![Claude Code resolving region, flavor, and image automatically before asking for confirmation to create a VM](/images/docs/account-settings/integrations/gcore-mcp-server-overview/claude-code-create-vm.png)</Frame>

## Set up Gcore MCP Server

<Steps>
<Step title="Install and configure the server">
Connect the server to Claude Code or Cursor IDE with [Server&nbsp;setup](/account-settings/integrations/install-and-configure-gcore-mcp-server).
</Step>
<Step title="Test the available tools">
Explore exposed tools in a browser with [MCP&nbsp;Inspector](/account-settings/integrations/test-tools-with-mcp-inspector).
</Step>
<Step title="Filter tools for large tool sets">
Limit exposed tools for a specific workflow with [Tool&nbsp;filtering](/account-settings/integrations/filter-tools-with-gcore-tools) and the `GCORE_TOOLS` environment variable.
</Step>
</Steps>
