...
<p>For every incoming request, WAAP computes and logs its JA3 fingerprint automatically. No configuration is required.</p>

## View events by fingerprint

<p>Every event logged by WAAP includes the JA3 fingerprint of the originating request — visible both in the event list and in the request details panel. Filter by hash to isolate all traffic from a specific client library:</p>
...
<p>WAAP automatically assigns the `malicioustlsfingerprint` [predefined tag](/waap/waap-rules/custom-rules/tag-rules/predefined-tags) to requests whose fingerprint appears in Gcore's threat intelligence feed of known-malicious client libraries. A [tag-based rule](/waap/waap-rules/custom-rules/tag-rules) targeting this tag applies an action without requiring knowledge of a specific hash value:</p>

```text
tags.exists('malicioustlsfingerprint')
```

<p>When a fingerprint is not yet in the feed, an explicit `request.ja3` or `request.ja4` condition in an Advanced Rule is the fallback — as the incident below illustrates.</p>
...
<p>Each event in the response includes a `ja3` field. For JA4, use the `ja4` parameter with a value in the `prefix_hex12_hex12` format.</p>
