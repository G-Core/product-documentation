...
<Frame>![A custom rule for tagging a registered client](/images/docs/waap/waap-rules/custom-rules/tag-rules/reserved-tags/tag-registered-client.png)</Frame>
...
<Frame>![A custom rule for tagging a logged-in client](/images/docs/waap/waap-rules/custom-rules/tag-rules/reserved-tags/tag-logged-in-client.png)</Frame>
...
<Frame>![WAAP requests graph on the Analytics page](/images/docs/waap/waap-rules/custom-rules/tag-rules/reserved-tags/monitored-tag-analytics-page.png)</Frame>
...
<Frame>![A custom rule for tagging requests from certain countries](/images/docs/waap/waap-rules/custom-rules/tag-rules/reserved-tags/monitored-tag-certain-countries.png)</Frame>
...
<Frame>![A custom rule for tagging requests with ignored bot activity](/images/docs/waap/waap-rules/custom-rules/tag-rules/reserved-tags/monitored-tag-ignored-automation.png)</Frame>
...
<Frame>![A custom rule for tagging requests to login pages](/images/docs/waap/waap-rules/custom-rules/tag-rules/reserved-tags/tag-login-page-activity.png)</Frame>



5\. Click **Save** to create the rule. 

## Identify LLM resources

The **LLM Trigger** reserved tag marks traffic that belongs to Large Language Model (LLM) endpoints. The [LLM Attack](/waap/waap-policies/waf-and-owasp-top-threats#configure-owasp-threats-rules) policy inspects only requests with this tag — enabling the policy without tagging LLM resources leaves that traffic unprotected by LLM Attack rules.

Create a custom or advanced rule that matches reliable LLM resource identifiers — URL path segments, request headers, or response headers — then apply the **LLM Trigger** tag:

1\. In the **IF** section, add conditions that identify the LLM resource. Match a URL path that contains the LLM endpoint, or match a dedicated request or response header that the application sets for LLM traffic.

2\. In the **THEN** section, select the **Tag** action and choose **LLM Trigger**.


<Frame>![Custom rule applying the LLM Trigger reserved tag to an LLM URL path](/images/docs/waap/waap-rules/custom-rules/tag-rules/reserved-tags/tag-llm-trigger.png)</Frame>



3\. Click **Save** to create the rule.

4\. Enable the **LLM Attack** policy for the domain under **WAAP** \> **Default Rules** \> **OWASP Threats**.

After the rule is active, WAAP applies LLM Attack inspection to tagged requests and blocks matches for prompt injection (OWASP LLM01) and improper output handling (OWASP LLM05).
...
<Frame>![A custom rule for tagging requests from paid users](/images/docs/waap/waap-rules/custom-rules/tag-rules/reserved-tags/tag-paid-users-activity.png)</Frame>
...
<Frame>![A custom rule to detect denial of inventory attacks](/images/docs/waap/waap-rules/custom-rules/tag-rules/reserved-tags/tag-cart-checkout.png)</Frame>
...
<Frame>![A custom rule to detect item added to cart](/images/docs/waap/waap-rules/custom-rules/tag-rules/reserved-tags/tag-items-added-to-cart.png)</Frame>
