...
## Configure the rate-limit function
...
<p>Each request is counted individually per IP. If `1.2.3.4` sends more than 200 requests within 5 seconds, it is blocked. Another IP, `1.2.3.5`, is only blocked when it independently exceeds the same threshold.</p>
...
<p>Rate-limit JPEG requests from a specific private IP range. The `request.ip_in_range` condition acts as a filter — the rate limit aggregation still counts per IP within the range.</p>
...
<p>Rate-limit all `GET` or `HEAD` requests that return a `302` redirect with a specific content type. The `cluster` scope counts requests globally across all IPs rather than per individual IP — useful for protecting origin from aggregate traffic spikes.</p>
