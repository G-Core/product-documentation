...
Block requests to any URL that does not end in a known static asset extension — images, fonts, scripts, archives. Requests to dynamic paths are rate-limited at 20 per 120 seconds per IP.
...
Rate-limit JPEG requests from a specific private IP range. The `request.ip_in_range` condition acts as a filter — the rate limit aggregation still counts per IP within the range.
...
Rate-limit all `GET` or `HEAD` requests that return a `302` redirect with a specific content type. The `cluster` scope counts requests globally across all IPs rather than per individual IP — useful for protecting origin from aggregate traffic spikes.
