...
Advanced rate limiting rules can be created in the [Gcore Customer Portal](https://portal.gcore.com/) (as advanced rules) or via the [Gcore WAAP API](/api-reference/waap/advanced-rules/retrieve-advanced-rules). Rate limiting conditions are evaluated as part of a rule’s condition logic and can be combined with other request attributes to precisely control when an action is triggered.
