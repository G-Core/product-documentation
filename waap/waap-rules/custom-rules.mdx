...
<p>Custom rules match incoming requests against one or more conditions — IP address, country, URL, user agent, and others — and apply a configured action when the match fires. Rules are evaluated by priority: Allow overrides Block, Block overrides Captcha, and so on.</p>
