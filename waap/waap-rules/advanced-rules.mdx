...
If a `block` contains an `action_duration`, WAAP will add the `penalty` tag to matching requests as long as the `block` action is active. To ensure the blocking of specific sources, **you must also check for the penalty tag** by including the tag check as part of a rule's `source` condition (e.g., `or tags.exists('penalty')`) or by defining a separate rule that blocks requests with the `penalty` tag, like in the following example.
