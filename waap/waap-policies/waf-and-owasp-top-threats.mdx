...
ai-navigation: Configure OWASP Threats policies in Gcore WAAP via Customer Portal or REST API, including CSRF behavior when www and non-www hostnames differ.
---
...
| **Cross-site request forgery (CSRF)**      | CSRF forges requests that appear to come from an authenticated user. When enabled, WAAP classifies a request as cross-site if the Origin or Referer host does not exactly match the site domain, then validates a CSRF token. <br /> <br /> Enable the policy to block forged state-changing requests. See [CSRF checks for www and non-www hostnames](#csrf-checks-for-www-and-non-www-hostnames). |
...
| **LLM Attack**                             | Attackers exploit Large Language Model integrations to perform prompt injection (OWASP LLM01) and improper output handling (OWASP LLM05). <br /> <br /> Enabling this policy alone does not protect LLM traffic. First identify LLM resources with a custom or advanced rule that applies the [LLM Trigger](/waap/waap-rules/custom-rules/tag-rules/reserved-tags#identify-llm-resources) reserved tag. WAAP then inspects tagged requests for known LLM attack patterns. |

## CSRF checks for www and non-www hostnames

<p>When the CSRF policy is enabled, WAAP compares the Origin or Referer host to the site's configured domain with exact string equality — no normalization — so `www.example.com` and `example.com`, or the same hostname on a different port, count as different sites.</p>

<p>After classifying a request as cross-site, WAAP validates a CSRF token by checking a cookie value against tokens stored in the session. Cross-www navigation usually does not carry a matching token, so the check fails and the request is blocked.</p>

<p>CSRF checks run only on configured HTTP methods (POST by default) and skip static assets, API traffic, and the first GET of a session.</p>

### Options for www and non-www

<p>To keep CSRF and other WAAP protections active when both hostnames serve the same site, canonicalize to one hostname with a 301 redirect between www and non-www so Origin and Host always match.</p>

<p>If both hostnames must stay reachable without a redirect, disable the CSRF policy for the domain. Other WAAP policies remain on.</p>

<Steps>
  <Step title="Disable the CSRF policy">
    In the [Gcore Customer Portal](https://portal.gcore.com), open the domain's OWASP Threats rules:

    1. Navigate to **WAAP** > **Default Rules**.
    2. Select the domain.
    3. Open the **OWASP Threats** tab.
    4. Disable **Cross-site request forgery (CSRF)**.
  </Step>
</Steps>

<p>Custom rule actions apply to matched traffic as a whole. The **Allow** action excludes that traffic from all WAAP security checks — including WAF, bot protection, IP reputation, and CSRF — as described in [Custom Rules](/waap/waap-rules/custom-rules#actions-in-custom-rules).</p>
...
<p>Sets the policy to enabled or disabled by specifying the target state. Pass `mode: true` to enable and `mode: false` to disable.</p>

| Parameter | Required | Description |
|---|---|---|
| `policy_id` | Yes | ID of the policy to configure, obtained from the [View policy states](#view-policy-states) response. |
...
print(f"Policy is now {'enabled' if result.mode else 'disabled'}")
```
</Tab>
<Tab title="Go SDK">
```go
...
func main() {
	...
	result, err := client.Waap.Domains.Policies.Toggle(
		...
		waap.DomainPolicyToggleParams{
			DomainID:                 domainID,
			WaapDomainPolicySettings: waap.WaapDomainPolicySettingsParam{Mode: true},
		},
	)
	if err != nil {
		fmt.Println(err)
...
| LLM Attack | Blocks attacks targeting Large Language Model integrations. Requires the LLM Trigger reserved tag on LLM traffic | Disabled |
