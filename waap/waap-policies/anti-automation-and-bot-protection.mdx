...
<p>For a verified known bot set to Policy-based, Bot Attacks checks are automatically excluded to avoid false positives. Configure per-bot actions in [Known Bots](/waap/waap-policies/known-bots).</p>
...
<p>Sets the policy to enabled or disabled. Use the policy ID returned by the [View policy states](#view-policy-states) request.</p>
...
<Tabs>
  <Tab title="Python SDK">
    ...
    print(f"Anti-spam is now {'enabled' if result.mode else 'disabled'}")
    ```
  </Tab>
  <Tab title="Go SDK">
    ```go
