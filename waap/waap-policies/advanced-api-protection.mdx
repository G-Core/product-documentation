...
| Policy | Purpose | ID |
|---|---|---|
| Auth token protection | Blocks users who repeatedly attempt to authenticate with invalid tokens. | `S3008984` |
| Sensitive data exposure | Blocks API responses containing PII such as phone numbers, SSNs, email addresses, or credit card numbers. | `S3008985` |
| Invalid API traffic | Blocks API requests that do not conform to a valid JSON structure. | `S3008986` |
| API-level authorization | Blocks requests to admin or privileged endpoints from users not tagged as admin or privileged. | `S3008987` |
| Non-baselined API requests | Blocks requests to API endpoints that are not part of the defined API baseline. | `S3008988` |
