...
<p>Sets the policy to enabled or disabled by specifying the target state. Use the policy ID returned by the [View policy states](#view-policy-states) request.</p>
...
<Tabs>
  <Tab title="Python SDK">
    ...
    print(f"Logged-in WordPress admins is now {'enabled' if result.mode else 'disabled'}")
    ```
  </Tab>
  <Tab title="Go SDK">
    ```go
