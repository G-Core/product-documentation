...
<p>Protocol Validation policies block HTTP requests that violate protocol standards: one policy targets clients that tamper with internal service calls, the other enforces HTTP RFC compliance.</p>
...
| Prevent malformed request methods | Blocks or challenges clients whose requests do not conform to HTTP RFC standards. | `S3008980` | Disabled |
