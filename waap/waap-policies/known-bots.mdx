...
<p>Known Bots policies control how WAAP handles traffic from 118 recognized automated services — search engine crawlers, social media preview bots, monitoring tools, and payment processors. Each bot can be set to **Allow** (bypass WAF inspection) or **Policy-based** (subject to other active WAF rules).</p>
...
<p>The API returns the updated mode:</p>

```json
{"mode": true}
```

`mode: true` means Allow; `mode: false` means Policy-based.
