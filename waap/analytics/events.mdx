...
- **JA3** and **JA4** — [TLS fingerprints](/waap/threat-intelligence/tls-fingerprinting) computed from the client's TLS handshake. Both values are displayed for every event. Use them to identify clients by network stack rather than IP address, or to pivot from an event directly into a fingerprint-based filter.
