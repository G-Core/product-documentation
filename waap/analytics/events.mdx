...
title: "Events"
sidebarTitle: Events
ai-navigation: Filter HTTP requests and security events in WAAP Events by time range, event type, action (Blocked/Monitored/Allowed/Passed), country, IP, path, HTTP method
ai-navigation: Filter WAAP Events by time range, event type, action, country, IP, path, HTTP method, response code, request ID, reference ID, session ID; 30-day
---
