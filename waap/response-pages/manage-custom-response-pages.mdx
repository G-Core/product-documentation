...
<Tabs>
  <Tab title="Python SDK">
    ...
    for ps in page_sets:
        print(f"id={ps.id} name={ps.name!r} domains={ps.domains}")
    ```
  </Tab>
  <Tab title="Go SDK">
    ```go
    ...
    func main() {
        ...
        for _, ps := range page.Results {
            fmt.Printf("id=%d name=%s domains=%v\n", ps.ID, ps.Name, ps.Domains)
        }
    }
    ```
  </Tab>
...
<p>Updates the page configuration inside a page set. Only the fields included in the request are changed — omitted page types remain unchanged. Returns HTTP 204 with no response body.</p>
...
<p>Controls which domains display a page set. Pass a list of domain IDs to assign the set; pass an empty list to remove all assignments. A domain can have only one active page set — assigning a new set replaces the previous one.</p>
