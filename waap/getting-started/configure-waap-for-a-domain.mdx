...
<p>A WAAP domain is created automatically when WAAP is enabled on a CDN resource via the [CDN API](https://api.gcore.com/docs/cdn). Once created, the WAAP API provides operations to list domains and switch between Monitoring and Protection modes.</p>

<Info>
A permanent [API token](/account-settings/api-tokens) is required. The domain ID is returned by the list request below.
</Info>
