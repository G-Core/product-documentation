...
A [suspended resource](/cdn/cdn-resource-options/general/suspend-a-cdn-resource-automatically-or-manually) is automatically deleted after 90 days. During this period, WAAP settings remain intact, and you can reactivate both CDN and WAAP.
...
<p>The WAAP API provides operations to check a domain's protection status and delete an inactive domain. Switching between Monitoring and Protection modes is covered in the [domain setup guide](/waap/getting-started/configure-waap-for-a-domain).</p>

<Info>
A permanent [API token](/account-settings/api-tokens) is required. Use <code>GET /waap/v1/domains</code> to retrieve the domain ID.
</Info>
