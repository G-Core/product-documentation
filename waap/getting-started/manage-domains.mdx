...
ai-navigation: Manage WAAP-protected domains by checking status, disabling and re-enabling protection from the WAAP Domains page, and deleting inactive domains via Customer Portal or REST API.
---
...
<p>After [adding a domain to WAAP protection](/waap/getting-started/configure-waap-for-a-domain), it appears on the **Domains** page in the [Gcore Customer Portal](https://portal.gcore.com/). This page displays all domains and their statuses, provides access to CDN settings, and allows deletion of inactive domains.</p>
...
<p>A domain in WAAP can have the following statuses: </p>
  ...
  * **Disabled**: The domain isn't protected because WAAP is disabled. You can always [enable WAAP](/waap/getting-started/configure-waap-for-a-domain) for inactive domains from the WAAP Domains page. Note that inactive domains are still billable.
...
<p>You can't delete a domain that has enabled WAAP protection. To delete the domain, disable WAAP from the **WAAP domain mode** dropdown first. The **WAAP domain mode** should change to **Disabled**.</p>
