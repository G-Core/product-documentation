...
<p>A domain in WAAP can have the following statuses: </p>
  ...
  * **Disabled**: The domain isn't protected because WAAP is disabled. You can always [enable WAAP](/waap/getting-started/configure-waap-for-a-domain) for inactive domains in the CDN resource settings. Note that inactive domains are still billable.
...
<p>To activate WAAP protection again, [enable the WAAP toggle](/waap/getting-started/configure-waap-for-a-domain#step-2-enable-waap-in-cdn-resource-settings) in the CDN resource settings.</p>
