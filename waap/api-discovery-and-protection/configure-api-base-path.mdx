...
<p>When a domain uses APIs hosted on the same domain without [API&nbsp;Discovery](/waap/api-discovery-and-protection/api-discovery) enabled, endpoints can be added manually to the API base path. This defines a communication path for WAAP to expect API requests and protect those endpoints.</p>
...
- Paths are recursively allowed — `api/` allows `api/v1/*` and `api/v2/*`.
...
- Paths are recursively allowed — `api/` covers `api/v1/*` and `api/v2/*`.
- Wildcard and regex patterns are accepted — for example `api/v1/.*` or `v2/.*`.
...
<Tabs>
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        // Add a path
        err = client.Waap.Domains.Settings.Update(ctx, domainID, waap.DomainSettingUpdateParams{
