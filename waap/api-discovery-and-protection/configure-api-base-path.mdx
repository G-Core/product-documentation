...
<p>If a domain uses APIs hosted on the same domain and [API Discovery](/waap/api-discovery-and-protection/api-discovery) is not enabled, manually configure the API base path to tell WAAP which paths to treat as API traffic. WAAP then disables CAPTCHA and JavaScript validation for those paths while keeping DDoS, IP reputation, and rate limiting active.</p>
...
<Info>
  **Info**
  ...
  To make API calls, a [permanent API token](https://gcore.com/docs/account-settings/create-use-or-delete-a-permanent-api-token) is required. To get the domain ID, use [List domains](/waap/getting-started/manage-domains#list-domains).
</Info>
