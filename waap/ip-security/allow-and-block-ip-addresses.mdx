...
3\. Navigate to the needed section:
  ...
  * **Allowed IPs**: Permit specific IPs or IP ranges to access your domain without any restrictions or security checks.

  * **Blocked IPs**: Deny IPs or IP ranges access to your domain.
...
4\. Click **Add IP/IP Range**. To allow or block traffic for an IP range, enter its first and last IP address. 
...
6\. Click **Save** to add the IP to the list.
...
Additionally, all blocked and allowed IPs are displayed on the **Custom Rules** page, where you can temporarily disable or enable them if necessary. 

<Warning>
**Warning**

If you disable a firewall rule on the **Custom Rules** page, it will no longer be visible on the **Firewall** page. Once re-enabled, the rule will appear on the **Firewall** page automatically.
</Warning>
...
3\. Navigate to the needed section with the configured IP address—**Allowed IPs** or **Blocked IPs**. 

4\. Find the IP you want to manage and click the three-dot icon next to it. 

5\. Select the relevant action from the dropdown—**Edit** or **Delete**—and follow the instructions.
