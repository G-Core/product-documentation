...
| Access keys | Create and delete read-write or read-only S3 access keys. Read-only keys are supported on S3 Standard only. The secret is returned only when a key is created. |
...
<p>The initial key in the response has read-write access. Additional keys — read-write or read-only — can be created using the [access&nbsp;keys](/storage/manage-object-storage/manage-s3-access-keys) endpoint. Read-only keys are supported on S3 Standard only.</p>
