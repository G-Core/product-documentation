...
ai-navigation: Create S3 Standard, S3 Fast, or SFTP storage via Customer Portal, REST API, or Terraform - includes read-only S3 access keys.
---
...
<p>A **Details** dialog opens immediately, showing the storage credentials: access key, secret key, hostname, and location. Click **Copy object storage data** to copy all credentials at once. Store the secret key, because it is returned only when a key is created. Additional keys are created from **Access keys** in the storage actions menu.</p>
...
| Access keys | Create and delete S3 access keys, and copy a new secret when a key is created. |
...
<p>S3 access keys authenticate AWS CLI, SDKs, and other S3-compatible clients. S3 Standard storage holds up to 10 keys, and S3 Fast storage holds up to 2 keys, so a replacement can be created before the previous key is deleted.</p>

<p>A read-only access key is created on S3 Standard storage with the REST API. Permissions are fixed at creation, so delete a key and create another to change access.</p>

<Steps>
  <Step title="Open Access keys">
    Click **...** next to the storage name and select **Access keys**.
  </Step>
  <Step title="Create a key">
    Click **Create key**. The dialog shows the access key and secret key. Click **Copy credentials** and store both values, because the secret is shown only once.

    The new key is read-write, and existing keys appear in the list below.

    <Frame>![Access keys dialog after creating a key](/images/docs/storage/create-an-s3-or-sftp-storage/s3-access-keys-70.png)</Frame>
  </Step>
...
<p>S3-compatible and SFTP storages are provisioned with a single API call. For S3 storage, the secret key is returned only at creation time and when a new access key is created, so it cannot be retrieved later.</p>
...
<p>S3 Standard storage holds up to 10 access keys, and S3 Fast storage holds up to 2. Create a replacement key, update clients, then delete the old key. The secret key is returned only in the create response.</p>
...
<p>Permissions are fixed at creation, so delete a key and create another to change access.</p>

<p>**Create an access key.** An empty body creates a read-write key. The Python `extra_body` argument and the second curl request create a read-only key on S3 Standard:</p>
...
<p>Declares an S3 storage and an optional additional access key for zero-downtime credential rotation. The storage is provisioned with one key automatically; `gcore_storage_access_key` creates an additional key.</p>
