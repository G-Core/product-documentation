---
title: Create and manage an object or SFTP storage
sidebarTitle: Create Object Storage
ai-navigation: Create and manage S3 or SFTP storage via Customer Portal, REST API, or Terraform - includes listing by type and access key management.
---
...
<p>Use <code>PATCH /v4/sftp_storages/{storage_id}</code> to replace an SFTP storage's password. Set <code>password_mode</code> to <code>set</code> and send the new value in <code>password</code>. Typed SDK update parameters omit <code>set</code>; pass that body with <code>extra_body</code> (Python) or <code>option.WithJSONSet</code> (Go).</p>
...
<Tabs>
  <Tab title="Python SDK">
    ...
    client.storage.sftp_storages.update(
        sftp_id,
        extra_body={"password_mode": "set", "password": "MyNewPassword123"},
    )
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        _, err := client.Storage.SftpStorages.Update(
            ctx,
            sftpID,
            storage.SftpStorageUpdateParams{},
            option.WithJSONSet("password_mode", "set"),
            option.WithJSONSet("password", "MyNewPassword123"),
        )
        if err != nil {
...
<p>To set a custom SFTP password, set <code>password_mode</code> to <code>set</code> and pass the value in <code>sftp_password</code>. <code>password</code> is read-only: Terraform stores the value the API returns after apply.</p>
