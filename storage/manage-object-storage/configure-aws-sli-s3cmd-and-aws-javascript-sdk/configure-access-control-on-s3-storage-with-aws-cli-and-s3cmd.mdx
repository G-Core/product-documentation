...
ai-navigation: Configure Gcore Object Storage access control with AWS CLI and S3cmd using ACLs and JSON bucket policies, including IP-restricted anonymous access.
---
...
Replace the documentation addresses in the examples with the source IPs that should be allowed. The condition matches the client address seen by Object Storage for virtual-hosted and path-style requests, so a client-supplied `X-Forwarded-For` header cannot bypass the restriction.
