...
AWS CLI, S3cmd, and the AWS JavaScript SDK all use the same credentials to connect to Gcore Object Storage: an access key and secret key from [access keys](/storage/manage-object-storage/manage-s3-access-keys), and an [endpoint URL](/storage/manage-object-storage/s3-service-urls-and-default-region-names). The setup format differs by tool — select a tab for the specific steps.
...
<Tabs>
  <Tab title="AWS CLI">
    ...
    <Steps>
      ...
      <Step title="Enter storage credentials">
        ...
        - **Access Key**: the storage access key.
        - **Secret Key**: the storage secret key.
  ...
  <Tab title="S3cmd">
    ...
    <Accordion title="Without wizard">
      ...
      - `12*****6DEF` is the storage access key.
      - `EX*****` is the storage secret key.
  ...
  <Tab title="AWS JavaScript SDK">
    ...
    - `1234` is the storage access key.
    - `5678` is the storage secret key.
