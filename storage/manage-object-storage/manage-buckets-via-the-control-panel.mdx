...
<p>Storage creation generates one access key. Additional keys, including replacements, are created in [access keys](/storage/manage-object-storage/manage-s3-access-keys). Gcore does not store the secret after it is shown, so copy it when the key is created.</p>
