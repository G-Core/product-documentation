...
<p>S3 Standard storage holds up to 10 keys, and S3 Fast storage holds up to two keys, so a replacement can be created before the previous key is deleted. Create a second key, update applications, then delete the old key to rotate credentials without downtime.</p>
...
<p>An empty POST body creates a read-write key, and the response sets `read_only` to `false`. A JSON body with `read_only` set to `true` creates a read-only key on S3 Standard storage.</p>
...
<p>A GET request to the same `access_keys` path returns each key's `access_key`, `created_at`, and `read_only` fields, and omits the secret.</p>
