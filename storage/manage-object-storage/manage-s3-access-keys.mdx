...
<p>S3 Standard storage holds up to 10 keys, and S3 Fast storage holds up to 2 keys, so a replacement can be created before the previous key is deleted. The portal **Create key** control creates a read-write key, while a read-only access key is created on S3 Standard storage with the REST API.</p>
...
<Steps>
  ...
  <Step title="Create a key">
    ...
    <Frame>![Access keys dialog after creating a key](/images/docs/storage/manage-object-storage/manage-s3-access-keys/manage-s3-access-keys-image1.png)</Frame>
  </Step>
