...
- **Public bucket** (anonymous access allowed): In the **URL** tab, enter the bucket hostname in the **Origin source** field: `<bucket-name>.luxembourg-2.storage.gcore.dev`.
   - **Private bucket** (authentication required): Select the **S3 storage** tab. For Gcore Object Storage, select **Other** and enter **Hostname**, **Access Key ID**, **Secret access key**, **Region (optional)**, and **Bucket name**. Create keys in [access keys](/storage/manage-object-storage/manage-s3-access-keys).
...
The addressing scheme `<bucket-name>.luxembourg-2.storage.gcore.dev` hides the storage hostname from CDN requests, keeping the origin private.
...
Given a bucket `mybucket` at `luxembourg-2.storage.gcore.dev` and a CDN resource with custom domain `cdn.example.com`, request `picture.png` using:
