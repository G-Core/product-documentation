...
<p>The API returns a nested structure: <code>data.clients.{client_id}.locations.{location_name}.storages.{storage_name}</code>. Each storage entry contains per-metric time-series arrays, where each element is `[timestamp, value]`:</p>
