...
|-----------|--------|---------|
| `tags[key]` | repeated per value; tag values match case-insensitively; repeat the same key for OR logic | `?tags[env]=production&tags[env]=staging` |
...
<Tabs>
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        tagKV, err := json.Marshal(map[string]string{"env": "production"})
        if err != nil {
            log.Fatal(err)
        }

        page, err := client.Cloud.Instances.List(context.Background(), cloud.InstanceListParams{
            TagKeyValue: param.NewOpt(string(tagKV)),
        })
        if err != nil {
            log.Fatal(err)
        }
  ...
  <Tab title="curl">
    ```bash
    # URL-decoded: ?tag_key_value={"env":"production"}
...
<p>To filter by multiple tags simultaneously (AND logic), include all key-value pairs in the same JSON object:</p>
...
<Tabs>
  <Tab title="Python SDK">
    ...
    instances = list(client.cloud.instances.list(
        tag_key_value=json.dumps({"env": "production", "team": "backend"}),
    ))
  ...
  <Tab title="Go SDK">
    ```go
    tagKV, _ := json.Marshal(map[string]string{"env": "production", "team": "backend"})
  ...
  <Tab title="curl">
    ```bash
    # URL-decoded: ?tag_key_value={"env":"production","team":"backend"}
