...
| Version | Status | Latest release | Recommended for |
|---------|--------|----------------|-----------------|
| v0 | Stable | 0.34.0 | Production use |
| v2 | Alpha | 2.0.0-alpha.6 | Experimentation only |

Because both versions share the same registry namespace, using an open-ended version constraint such as `>= 0.34.0` in your configuration may cause Terraform to resolve the v2 alpha in future runs. Pin the provider version explicitly to avoid unintended upgrades.

**v0** is the current production-ready provider. It supports all resources described in this article and is the recommended choice for any live workload.

**v2** is a ground-up rewrite of the provider based on OpenAPI spec-driven code generation. It introduces a new authentication method, a redesigned provider configuration, and a different resource schema. Breaking changes are expected between alpha releases. See [Try provider v2 (alpha)](#try-provider-v2-alpha) if you want to test it.
...
6. Generate a permanent API token using [the guide](/account-settings/api-tokens) if you don't have one yet.
...
<Frame>
  ![output](/images/docs/cloud/manage-cloud-via-terraform/12966689446417.png)
</Frame>
...
Provider v2 is a complete rewrite of the Gcore Terraform provider, generated from the Gcore OpenAPI specification using [Stainless](https://www.stainless.com/). It uses a different authentication method and provider configuration compared to v0.

<Warning>
**Warning**

v2 is an alpha release. Breaking changes between releases are expected. Do not use v2 in production environments. Report issues on the [GitHub repository](https://github.com/G-Core/terraform-provider-gcore/issues).
</Warning>

To try v2, replace the provider configuration in `main.tf` with the following:
...
Provider v2 differs from v0 in the following ways:

- Terraform CLI 1.11 or later is required (v0 requires 0.13.0 or later).
- Authentication uses `api_key` instead of `permanent_api_token`.
- `cloud_project_id` and `cloud_region_id` are configured at the provider level rather than per resource.
- The version must be pinned exactly (`2.0.0-alpha.6`) to avoid unintended upgrades to future alpha releases.

The API key can also be set via the `GCORE_API_KEY` environment variable. Similarly, `cloud_project_id` and `cloud_region_id` accept `GCORE_CLOUD_PROJECT_ID` and `GCORE_CLOUD_REGION_ID` environment variables.

Available resources and data sources for v2 are documented on the [Terraform Registry](https://registry.terraform.io/providers/G-Core/gcore/2.0.0-alpha.6/docs). Additional examples are available in the [v2 branch on GitHub](https://github.com/G-Core/terraform-provider-gcore/tree/v2).
...
All examples in this section use provider v0. Provider v2 resources are under active development—refer to the [Terraform Registry](https://registry.terraform.io/providers/G-Core/gcore/2.0.0-alpha.6/docs) for the current v2 resource list.
</Info>
...
- If you select "any_subnet", specify the "network_id".
...
3. Configure your Bare Metal image.

Required attributes:

- "name" for your image.
- "url" where your image is located.
...
- Configure the "pool" for the Kubernetes cluster. Specify the following parameters:
  ...
  - boot_volume_size: Cluster pool boot volume size. Must be set only for VM pools. Changing the value of this attribute will trigger recreation of the cluster pool.
  - boot_volume_type: Cluster pool boot volume type. Must be set only for VM pools. Available values are "standard", "ssd_hiiops", "cold", "ultra". Changing the value of this attribute will trigger recreation of the cluster pool.
...
resource "gcore_lbpool" "pl" {
  ...
  listener_id     = gcore_loadbalancer.lb.listener.id
  health_monitor {
    type        = "PING"
    delay       = 60
    max_retries = 5
