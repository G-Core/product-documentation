...
The Gcore Terraform provider v0 supports the following Cloud resources: Virtual Machines, Bare Metal servers, Kubernetes clusters, Load Balancers, networks, volumes, snapshots, server groups, and reserved IP addresses. The [Terraform page](/developer-tools/terraform/overview) covers installation and provider configuration.
...
Provider v2 is a ground-up rewrite of the Gcore Terraform provider, auto-generated from the OpenAPI spec. It supports `terraform import` for most resources. The [Terraform page](/developer-tools/terraform/overview) covers v2 installation, authentication, and a full comparison with v0.
