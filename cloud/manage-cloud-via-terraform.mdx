...
ai-navigation: Configure Gcore Cloud resources using Terraform provider with API tokens, main.tf configuration files, and resource definitions for Virtual Machines, Kuberne
ai-navigation: Deploy Bare Metal servers, Virtual Machines, Managed Kubernetes clusters via Terraform provider with gcore_baremetal, gcore_instance resources.
---
...
6. Find out the latest version of the Terraform provider on [the page](https://registry.terraform.io/providers/G-Core/gcore/latest) and generate a permanent API token using [the guide](/account-settings/api-tokens), if you don't have one yet.
...
- 0.32.5—the Terraform provider version
