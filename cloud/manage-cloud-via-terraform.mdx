---
title: Manage Cloud resources via Terraform
...
The Gcore Terraform provider v0 supports the following Cloud resources: Virtual Machines, Bare Metal servers, Kubernetes clusters, Load Balancers, networks, volumes, snapshots, server groups, and reserved IP addresses. The [Terraform page](/developer-tools/terraform/manage-with-terraform) covers installation and provider configuration.

<Info>
**Info**

All examples use provider v0. Provider v2 resources are under active development; the current v2 resource list is in the [v2 alpha documentation on Terraform Registry.](https://registry.terraform.io/providers/G-Core/gcore/2.0.0-alpha.6/docs)
</Info>

Provider v2 is a ground-up rewrite of the Gcore Terraform provider, auto-generated from the OpenAPI spec. It is currently in alpha and supports `terraform import` for most resources. The [Terraform page](/developer-tools/terraform/manage-with-terraform) covers v2 installation, authentication, and a full comparison with v0.

For those already familiar with Terraform, the abridged workflow is:
...
4. To skip the subnetwork, proceed to step 6. To create a subnetwork, add the code below and customize the highlighted values:
...
- Specify whether to use the reserved IP address as a virtual IP (VIP) address ("is_vip = true") or not ("is_vip = false"). For details, see [Create and configure a virtual IP address.](/cloud/networking/ip-address/create-and-configure-a-virtual-ip-vip-address)
