...
ai-navigation: View, update, start, stop, retrieve logs, and delete CaaS containers via the Customer Portal or REST API.
---
...
<p>On the **Image & Port** tab, select the image type (Public or Private), add a container startup command, or update the port used to connect to the container. These settings can be changed at any time.</p>
...
<p>View and adjust memory (MiB), allocated mCPU, and autoscaling limits and triggers. Autoscaling rules configured during [container&nbsp;creation](/cloud/caas/create-a-container#step-4-configure-autoscaling) apply here and can be updated at any time.</p>
...
<Info>API Key authentication may be temporarily unavailable. If the tab shows a disabled state, contact [Gcore&nbsp;support](https://gcore.com/contact-us).</Info>
...
<p>Each log entry shows the pod name, timestamp, and the message written by the container process.</p>
...
<p>Activate the Logging service to store logs. [Managed&nbsp;Logging](/cloud/logging-as-a-service/about-logging-as-a-service) covers configuration and retention settings.</p>

<Info>Logging integration is not available in all regions. If the tab shows "Logging is currently not supported in this region," select a different region when creating the container.</Info>
