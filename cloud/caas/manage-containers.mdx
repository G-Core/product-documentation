...
ai-navigation: Manage container status, configuration, ports, protocols, memory, CPU, autoscaling limits, API key authentication, and environment variables in Gcore CaaS via the Customer Portal containers page.
---
...
The script below checks status, updates the container configuration, stops and restarts it, then retrieves the latest log entries.
...
print(f"Status: {c['status']}  Address: {c['address']}")
...
for entry in requests.get(
    f"{BASE}/{CONTAINER_NAME}/logs?limit=10", headers=HEADERS
).json()["logs"]:
...
var (
	...
	baseURL       = fmt.Sprintf(
		"https://api.gcore.com/cloud/v1/caas/%s/%s/containers",
		projectID, regionID,
	)
)

func call(method, url string, body any) map[string]any {
	var r io.Reader
	if body != nil {
...
func pollTask(id string) {
	for {
		r := call("GET", "https://api.gcore.com/cloud/v1/tasks/"+id, nil)
		if r["state"] == "FINISHED" {
			return
		}
...
func main() {
	// Get status.
	c := call("GET", baseURL+"/"+containerName, nil)
	fmt.Printf("Status: %s  Address: %s\n", c["status"], c["address"])

	// Update configuration.
	r := call("PATCH", baseURL+"/"+containerName, map[string]any{
		"flavor": "160mCPU-256MiB",
		"scale":  map[string]any{"min": 1, "max": 2},
		"envs":   map[string]any{"APP_ENV": "production"},
	})
	pollTask(r["tasks"].([]any)[0].(string))

	// Stop.
	r = call("PATCH", baseURL+"/"+containerName, map[string]any{"is_disabled": true})
	pollTask(r["tasks"].([]any)[0].(string))

	// Start.
	r = call("PATCH", baseURL+"/"+containerName, map[string]any{"is_disabled": false})
	pollTask(r["tasks"].([]any)[0].(string))

	// Get logs.
	logs := call("GET", baseURL+"/"+containerName+"/logs?limit=10", nil)
	for _, e := range logs["logs"].([]any) {
...
### Step 1. Get container status and details

This call returns the full container configuration along with the current status and the public HTTPS endpoint.
...
The PATCH response contains the task ID to poll:
...
This call returns the most recent log lines from all running container pods. Logs are removed when the container is deleted.
...
```
  ...
  <Tab title="Go">
```go
logs := call("GET", baseURL+"/"+containerName+"/logs?limit=20", nil)
for _, e := range logs["logs"].([]any) {
    entry := e.(map[string]any)
    fmt.Println(entry["time"], entry["message"])
}
...
```
  ...
  <Tab title="Go">
```go
r := call("DELETE", baseURL+"/"+containerName, nil)
pollTask(r["tasks"].([]any)[0].(string))
