...
The [get container](/api-reference/cloud#tag/Container-as-a-Service/operation/ContainerHandler.get) endpoint also returns `deploy_status.ready`, which shows how many pods are currently running.
...
The complete list of patchable fields is in the [update container](/api-reference/cloud#tag/Container-as-a-Service/operation/ContainerHandler.patch) endpoint spec.
...
The [get logs](/api-reference/cloud#tag/Container-as-a-Service/operation/LogsContainerHandler.get) endpoint supports a `limit` query parameter to control how many entries are returned.
