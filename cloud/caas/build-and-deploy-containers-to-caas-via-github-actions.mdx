...
ai-navigation: Deploy containers to Gcore CaaS using GitHub Actions workflow with deploy-container action, configuring API token, project ID, region, image, port, environme
ai-navigation: Deploy containers to Gcore CaaS via GitHub Actions workflow with API token, project ID, region, image name, port, and scaling parameters.
---
...
**Required fields** :
  ...
  * `api-token` is a [permanent API token](/account-settings/api-tokens) that authenticates the GitHub action to Gcore API.
