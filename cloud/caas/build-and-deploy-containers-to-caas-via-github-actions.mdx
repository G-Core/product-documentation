...
**Required fields** :
  ...
  * `project-id` is the ID of the Gcore project where the container should be deployed. You can use the [list projects](https://gcore.com/docs/api-reference/cloud/projects/list-projects) API request to retrieve this.
  * `region-id` is the ID of the region where the container should be deployed. This can be obtained using the [list regions](https://gcore.com/docs/api-reference/cloud/regions/list-regions) API request.
...
3\. Create a _.secrets_ file with your `GCLOUD_API_TOKEN` value obtained from the [Customer Portal](https://portal.gcore.com/accounts/profile/api-tokens).
