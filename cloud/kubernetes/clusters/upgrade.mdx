...
Select a `version` value from the returned list and pass it to the upgrade call. The [upgrade versions](https://api.gcore.com/docs/cloud#tag/Managed-Kubernetes/operation/K8sClusterUpdateVersionViewSet.get) endpoint accepts only path parameters - no request body is required.
...
<p>Upgrade a Kubernetes cluster by editing `version` in the cluster resource and running `terraform apply` with the [Terraform provider](/developer-tools/terraform/overview). Gcore permits upgrading one minor version at a time — verify the target version in the Customer Portal or REST API before updating the configuration.</p>
...
<Tip>
See the [Terraform Registry](https://registry.terraform.io/providers/G-Core/gcore/latest/docs) for the full provider reference. This tab uses the [gcore_cloud_k8s_cluster](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_k8s_cluster) resource.
</Tip>
