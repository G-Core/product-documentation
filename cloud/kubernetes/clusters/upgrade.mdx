...
<p>Upgrade a Kubernetes cluster by editing `version` in the cluster resource and running `terraform apply` with the [Gcore Terraform provider](/developer-tools/terraform/overview). Gcore permits upgrading one minor version at a time, so verify the target version in the Customer Portal or REST API before updating the configuration.</p>
