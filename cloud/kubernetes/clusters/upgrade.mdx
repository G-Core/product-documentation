...
<p>Upgrade a Kubernetes cluster by incrementing `version` in the [`gcore_cloud_k8s_cluster`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_k8s_cluster) resource and running `terraform apply` with the [Gcore Terraform provider v2](/developer-tools/terraform/overview). Gcore permits upgrading one minor version at a time; verify the target version in the Customer Portal or API before changing the configuration.</p>
