...
<p>Provision a Kubernetes cluster with one or more node pools using the [`gcore_cloud_k8s_cluster`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_k8s_cluster) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview). The cluster requires an existing network and subnet; node pool flavor and autoscaling settings are configured per pool.</p>
