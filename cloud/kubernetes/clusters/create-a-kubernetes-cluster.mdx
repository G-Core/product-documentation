...
<p>Under the **Pools** section, configure a pool. A pool is a set of cluster nodes with the same specifications. Select **Virtual instances** for most workloads, **Bare metal instances** for workloads that require dedicated hardware without hypervisor overhead, or **GPU Bare metal instances** for GPU-accelerated workloads.</p>
...
<p>If a [network](/cloud/networking/create-and-manage-a-network) already exists, select it from the **Network** dropdown and select a subnet from the **Subnet** dropdown.</p>
...
<p>(Optional) Enable **Enable IPv6 dual-stack** to assign both IPv4 and IPv6 addresses for network interfaces of worker nodes and pods. If the toggle is unavailable, at least one pool must have a public IPv4 address. If the cluster connects only to a private network, configure and add an IPv6 subnet.</p>

<p>Basic DDoS Protection is included for the cluster network.</p>

<p>**Interface order**</p>

<p>The interfaces configured in **Network settings** are assigned in order at cluster creation — the assignment cannot be changed afterward. The first interface has higher routing priority for outbound traffic.</p>
...
## Step 10. (Optional) Enable VAST storage

<p>In regions with GPU Bare Metal nodes, a **VAST storage** toggle is available. Enable it to provision a VAST file share alongside the cluster. The share is accessible from all worker nodes through a dedicated VAST network interface that is attached automatically.</p>
...
## Step 11. (Optional) Configure advanced settings
...
| `pools[].max_node_count` | No | Maximum nodes for autoscaling. Set equal to `min_node_count` to disable autoscaling |
| `pools[].boot_volume_size` | No | Boot volume size in GiB |
| `pools[].boot_volume_type` | No | Boot volume type: `standard`, `ssd_hiiops`, `ssd_lowlatency`, `cold`, or `ultra` |
| `pools[].servergroup_policy` | No | Node placement policy: `anti-affinity` (nodes on different hypervisors), `soft-anti-affinity`, or `affinity` |
| `pools[].auto_healing_enabled` | No | Automatically replace unhealthy nodes |
