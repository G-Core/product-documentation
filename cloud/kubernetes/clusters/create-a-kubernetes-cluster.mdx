...
sidebarTitle: Create
ai-navigation: Create a Managed Kubernetes cluster via the Customer Portal, REST API, or Terraform. Configure worker pools, CNI provider, network, and access settings.
---
...
<p>Create a Managed Kubernetes cluster and configure its worker pools, networking, and access settings in the [Gcore Customer Portal](https://portal.gcore.com).</p>

<Steps>
  <Step title="Open the create form">
    Navigate to **Cloud** > **Managed Kubernetes** and click **Create Cluster**.

    <Frame>![Kubernetes Clusters page with the Create Cluster button](/images/docs/cloud/kubernetes/clusters/create-a-kubernetes-cluster/managed-kubernetes-clusters-list.png)</Frame>
  </Step>

  <Step title="Select region and version">
    Select a **region** and the Kubernetes **version**.

    <Frame>![Region selector on the create cluster form](/images/docs/cloud/kubernetes/clusters/create-a-kubernetes-cluster/create-cluster-region.png)</Frame>

    <Frame>![Kubernetes cluster version dropdown on the create form](/images/docs/cloud/kubernetes/clusters/create-a-kubernetes-cluster/create-cluster-version.png)</Frame>
  </Step>

  <Step title="Configure worker pools">
    A pool is a set of cluster nodes with the same specifications. Select **Virtual instances** for most workloads, **Bare metal instances** for dedicated hardware without hypervisor overhead, or **GPU Bare metal instances** for GPU-accelerated workloads.

    Set **Minimum nodes** and **Maximum nodes** to configure [autoscaling](/cloud/kubernetes/clusters/autoscaling/about-autoscaling). Select a **Placement policy** and the appropriate flavor for the workload. For **Bare metal instances**, select a flavor from the [available&nbsp;configurations](/cloud/kubernetes/bare-metal-kubernetes#available-configurations).

    For Virtual instance pools, select the boot volume type based on required storage performance:
    ...
    | **SSD Low-Latency** | Real-time data processing requiring sub-millisecond latency (avg. 300 µs) | 75,000 | 1,500 MB/s |

    Enter a **Pool name** and configure labels, taints, or additional options under **Advanced pool settings** as needed. Use **Add pool** to add additional pools.

    <Frame>![Pools section with worker node type, flavor, volume, and placement policy fields](/images/docs/cloud/kubernetes/clusters/create-a-kubernetes-cluster/create-cluster-pools.png)</Frame>

    <Info>
    If [IPv6&nbsp;dual-stack](/cloud/kubernetes/clusters/networking#enable-ipv6-dual-stack) is required, at least one pool must have a public IPv4 address.
    </Info>
  </Step>

  <Step title="Select the CNI provider">
    <Warning>
    The CNI provider cannot be changed after the cluster has been created.
    </Warning>

    Select **Cilium** or **Calico** before creating the cluster. Cilium uses eBPF and provides a broader range of features (load balancing, advanced security, failure detection, and more). Calico uses a more conservative stack based on iptables.

    Configure **Pod CIDR** and **Service CIDR** as needed. Default ranges:
    ...
    <p>The CIDR range cannot overlap with 10.255.255.0/24 or 192.168.128.0/17 — these ranges are reserved for internal use.</p>

    <p>(Optional) Adjust **Node mask size** or enable **Hubble for network monitoring** when using Cilium.</p>

    <Frame>![CNI Provider section with Calico and Cilium options and CIDR fields](/images/docs/cloud/kubernetes/clusters/create-a-kubernetes-cluster/create-cluster-cni.png)</Frame>
  </Step>

  <Step title="Configure network settings">
    Under **Network settings**, configure the first interface for the cluster.

    If a [network](/cloud/networking/create-and-manage-a-network) already exists, select it from the **Network** dropdown and select a subnet from the **Subnet** dropdown.

    <Frame>![Network settings section with network and subnet dropdowns](/images/docs/cloud/kubernetes/clusters/create-a-kubernetes-cluster/create-cluster-network.png)</Frame>

    To add a new network, click **Add a new network** and configure the dialog:

    1. Enter the **Network name**.
    2. (Optional) Enable **Bare Metal Network** to connect Bare Metal servers to the network.
    3. Enable **Create router** — required for a Managed Kubernetes cluster and on by default.
    4. (Optional) Enable **Add tags** to add metadata to the network.
    5. Click **Create & Proceed**.

    <Frame>![Create network dialog with network name, Bare Metal Network, and Create router options](/images/docs/cloud/kubernetes/clusters/create-a-kubernetes-cluster/create-cluster-network-dialog.png)</Frame>

    <p>(Optional) Enable **IPv6 dual-stack** to assign both IPv4 and IPv6 addresses for network interfaces of worker nodes and pods. If the cluster connects only to a private network, configure and add an IPv6 subnet.</p>

    <p>The interfaces in **Network settings** are assigned in order at cluster creation — the order cannot be changed afterward. The first interface has higher routing priority for outbound traffic.</p>

    <Warning>
    Configuring interfaces in the wrong order can cause routing conflicts and make cluster nodes unreachable. Verify the intended interface order before creating the cluster.
    </Warning>

    <p>After the cluster is created, navigate to the **Networking** tab in the cluster details to verify the interface assignment. To confirm the OS-level mapping on Bare Metal worker nodes, connect via SSH and run `ip a`. The [Interface&nbsp;order](/cloud/bare-metal-servers/create-a-bare-metal-server#interface-order) section in the Bare Metal server documentation explains the bonding mechanism.</p>
  </Step>

  <Step title="Configure access and optional services">
    Enter a **Cluster name** and select or create an [SSH&nbsp;key](/cloud/virtual-instances/connect/connect-to-your-instance-via-ssh) for node access.

    <Frame>![SSH key section with key selection dropdown](/images/docs/cloud/kubernetes/clusters/create-a-kubernetes-cluster/create-cluster-ssh.png)</Frame>

    <Frame>![Cluster name field on the create form](/images/docs/cloud/kubernetes/clusters/create-a-kubernetes-cluster/create-cluster-name.png)</Frame>

    If required, enable [Managed&nbsp;Logging](/cloud/logging-as-a-service/configure-logging-and-view-your-logs), **VAST storage** (GPU-enabled regions only), or expand [OIDC&nbsp;authentication](/cloud/kubernetes/clusters/secure-cluster-with-oidc) or [Cluster&nbsp;Autoscaler](/cloud/kubernetes/clusters/autoscaling/advanced-cluster-autoscaler-settings) under **Advanced settings**.

    <Frame>![Logging section with Enable Logging checkbox and topic options](/images/docs/cloud/kubernetes/clusters/create-a-kubernetes-cluster/create-cluster-logging.png)</Frame>

    <Info>
    VAST storage is available only in GPU-enabled regions. If the toggle does not appear, the region does not support VAST file shares.
    </Info>
  </Step>

  <Step title="Create the cluster">
    Review the **Estimated cost** summary and click **Create Cluster**.
...
- **K8S\_FLAVOR\_ID**: the flavor identifier for the Kubernetes node pool. Use `GET /cloud/v1/k8s/{project_id}/{region_id}/flavors` to list flavors available in the region.
</Info>

## Quickstart

The scripts below create a private network and subnet, provision a Kubernetes cluster with one initial worker node and autoscaling up to three, and save the kubeconfig to a local file. Cluster creation takes approximately 5–20 minutes.
...
<Tabs>
  <Tab title="Python SDK">
    ...
    k8s_version = versions.results[-1].version
...
Poll `GET /cloud/v1/tasks/{task_id}` every 5 seconds until `state` is `FINISHED`. Export the network ID:

```bash
export NETWORK_ID="<created_resources.networks[0]>"
```
...
The API returns a task ID. Poll until `FINISHED` and export the subnet ID:

```bash
export SUBNET_ID="<created_resources.subnets[0]>"
```
...
| `pools[].flavor_id` | Yes | Node VM flavor. Use `GET /cloud/v1/k8s/{project_id}/{region_id}/flavors` to list available options |
| `pools[].min_node_count` | Yes | Minimum nodes for autoscaling |
| `pools[].max_node_count` | No | Maximum nodes. Set equal to `min_node_count` to disable autoscaling |
| `pools[].boot_volume_type` | No | `standard`, `ssd_hiiops`, `ssd_lowlatency`, `cold`, or `ultra` |
...
```json
{
  "tasks": ["61ee06b0-4705-4616-bb19-245bc510a8d0"]
}
```

Save the returned value as `TASK_ID`. Poll `GET /cloud/v1/tasks/{task_id}` every 5 seconds until `state` is `FINISHED`. Cluster creation takes approximately 5–20 minutes. When complete:
...
Set `NETWORK_ID` to the network ID printed by the Quickstart (or captured in Step 2) before running the examples below.
...
<p>Cluster creation takes approximately 5–20 minutes. A `cluster_status` output of `"Provisioned"` confirms the cluster is ready.</p>
