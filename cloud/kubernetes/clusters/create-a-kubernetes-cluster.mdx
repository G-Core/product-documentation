...
<p>Provision a Kubernetes cluster with its private network and node pool using the [Gcore Terraform provider](/developer-tools/terraform/overview) v2. A permanent [API token](/account-settings/api-tokens), [project ID](https://gcore.com/docs/api-reference/cloud/projects/list-projects), and [region ID](https://gcore.com/docs/api-reference/cloud/regions/list-regions) are required.</p>

## Create a cluster

<p>The configuration below provisions a cluster into an existing network and subnet. Pass the IDs as variables, or reference resources if provisioning the network in the same configuration.</p>
...
<Warning>
The cluster `name` is immutable. The API does not support renaming an existing cluster — changing `name` in the resource block forces a full replacement (destroy + recreate).
</Warning>
