...
<Tabs>
  <Tab title="Python SDK">
    ...
    task = client.cloud.k8s.clusters.create(
        ...
        pools=[{
            "name": "default-pool",
            "flavor_id": "g2-standard-2-4",  # region-specific
            "min_node_count": 1,
            "max_node_count": 3,
            "boot_volume_size": 10,
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        projectID := gcore.Int(int64(mustAtoi(os.Getenv("PROJECT_ID"))))
        regionID  := gcore.Int(int64(mustAtoi(os.Getenv("REGION_ID"))))
        ...
        cluster, err := client.Cloud.K8S.Clusters.NewAndPoll(ctx, cloud.K8SClusterNewParams{
            ...
            Pools: []cloud.K8SClusterNewParamsPool{{
                Name:               "default-pool",
                FlavorID:           "g2-standard-2-4",  // region-specific
                MinNodeCount:       1,
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl "https://api.gcore.com/cloud/v2/k8s/$PROJECT_ID/$REGION_ID/create_versions" \
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl -X POST "https://api.gcore.com/cloud/v2/k8s/clusters/$PROJECT_ID/$REGION_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "name": "my-k8s-cluster",
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl "https://api.gcore.com/cloud/v2/k8s/clusters/$PROJECT_ID/$REGION_ID/my-k8s-cluster/config" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    # Delete the cluster first
    curl -X DELETE "https://api.gcore.com/cloud/v2/k8s/clusters/$PROJECT_ID/$REGION_ID/my-k8s-cluster" \
      -H "Authorization: APIKey $GCORE_API_KEY"

    # Poll until FINISHED, then delete the network (removes subnet and router too)
    curl -X DELETE "https://api.gcore.com/cloud/v1/networks/$PROJECT_ID/$REGION_ID/$NETWORK_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
  </Tab>
</Tabs>
