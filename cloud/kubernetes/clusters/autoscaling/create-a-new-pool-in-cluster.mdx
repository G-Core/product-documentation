...
ai-navigation: Create and configure Kubernetes node pool settings, including autoscaling limits, node type, flavor, volume, placement policy, labels, taints, autohealing, and advanced Kubelet and CRI-O options.
---
...
A pool can be created [at cluster creation](/cloud/kubernetes/clusters/create-a-kubernetes-cluster#step-4-configure-pools) or added to an existing cluster in the cluster settings. The following steps describe adding a pool to an existing cluster:

1\. In the [Gcore Customer Portal](https://portal.gcore.com), navigate to **Cloud** > **Managed Kubernetes**.

2\. Find the required cluster and click its name to open the settings.

<Frame>![Managed Kubernetes clusters list showing cluster names, versions, and status](/images/docs/cloud/kubernetes/clusters/autoscaling/create-a-new-pool-in-cluster/mkaas-page-existing-cluster.png)</Frame>

3\. Open the **Pools** section and click **Add pool**.

<Frame>![Cluster overview page with the Pools tab and Add pool button](/images/docs/cloud/kubernetes/clusters/autoscaling/create-a-new-pool-in-cluster/add-pool-existing-cluster.png)</Frame>
...
- Select the worker node type: **Virtual instances** (VM), **Bare metal instances**, or **GPU Bare metal instances**. For VMs, select a flavor, volume size in GiB, and the [volume type](/cloud/virtual-instances/volumes/about-volumes#available-volume-types).
...
<Frame>![Add pool form showing pool name, autoscaling limits, node type, flavor, volume, and placement policy fields](/images/docs/cloud/kubernetes/clusters/autoscaling/create-a-new-pool-in-cluster/create-pool.png)</Frame>
...
<Frame>![Add pool form showing optional settings: Add labels, Add taints, Autohealing nodes, Public IPv4 address, and Advanced pool settings](/images/docs/cloud/kubernetes/clusters/autoscaling/create-a-new-pool-in-cluster/create-pool-toggles.png)</Frame>
...
A pool with advanced configuration can be added during cluster creation or in the settings of an existing cluster. To do so, find the **Advanced pool settings** section and click the **Configure settings** link.
...
Kubelet manages pod scheduling and execution on every node according to the [KubeletConfiguration](https://kubernetes.io/docs/reference/config-api/kubelet-config.v1beta1/#kubelet-config-k8s-io-v1beta1-KubeletConfiguration) API.
...
A list of default required capabilities is provided. Removing any of these capabilities may cause the cluster to stop working. Additional capabilities can be added from the Linux kernel [capabilities list](https://man7.org/linux/man-pages/man7/capabilities.7.html).
