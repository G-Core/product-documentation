...
For GPU-intensive workloads with fluctuating traffic in a Kubernetes cluster, configure the Kubernetes Event-Driven Autoscaling (KEDA) component to automatically scale GPU resources based on real-time demand. This approach ensures optimal performance and cost efficiency.
...
- **Type** : Choose **GPU Bare metal instances**.
...
6. (Optional) Add labels to include additional information about nodes. Taints can also be specified to define which Kubernetes pods can run on these nodes.
...
6. Verify that the node is ready by running:
...
The metric `DCGM_FI_DEV_GPU_UTIL` measures real-time GPU core utilization and is collected by the `nvidia-dcgm-exporter` installed in Step 3.

Adjust the `query` expression or use other available DCGM metrics depending on the use case.

<Tip>
  This autoscaling configuration is based on the `DCGM_FI_DEV_GPU_UTIL` metric (GPU utilization). Any available metric from the [DCGM exporter](https://docs.nvidia.com/datacenter/dcgm/latest/dcgm-api/dcgm-api-field-ids.html) can be used instead.
</Tip>

Customize the highlighted values:

- **workload-name**: Enter a unique name that identifies the workload. Use the same value in all name fields.
...
After the GPU workload is deployed, KEDA creates a Horizontal Pod Autoscaler (HPA) and injects GPU utilization metrics from Prometheus as an external metric source.

1. To get real-time information about the HPA, run `kubectl get hpa -w`.
...
The system created 8 pods to match the 8 GPUs available on the server.

After scaling by the number of GPUs, some pending pods are created by the HPA. The autoscaler triggers the addition of new nodes because the cluster has no free GPUs.

2. Fetch the events: `kubectl get events -w`.
...
3. Check all pods within the cluster: `kubectl get pods`.
...
8 pods are running and several pods have a Pending status while waiting for a new node to be provisioned.

After the new node is provisioned, the system creates new pods up to the `maxReplicaCount` value specified in Step 8.
...
4. Check the list of nodes and their statuses: `kubectl get nodes`.
...
5. Verify the HPA status: `kubectl get hpa`.
...
Once the new node is ready and GPUs are initialized (which might take 5 to 10 minutes), the pending pods will start running.
