...
<p>Cloud Router connects subnets to external networks and controls traffic between them using static routes. The list of routers is available in networking settings, on the **Routers** page.</p>
...
- When a [network](/cloud/networking/create-and-manage-a-network) is created, a new router is automatically provisioned to enable traffic routing. Automatically created routers support the same configuration options as manually created routers.
...
3. Select the network and subnet to configure routing. To add multiple subnets, click **Add Subnet**.
...
4. (Optional) Select the **Enable SNAT** checkbox to enable access to the external network. A separate public IP address is announced for a router with external network access. This setting can be changed at any time after the router is created.
...
6. Enter a name for the router and click **Create Router** to save the configuration.
...
2. Click the router name to access its settings. Alternatively, click the three-dot icon next to the router and select **Overview**.
...
3. The **Router Details** page shows the router ID, SNAT status, creation date, and external IPs. Use the **SNAT** checkbox to allow instances in a private network to access external networks through the router. Subnets and static routes can also be configured from this page.


<Frame>![Router Details page with SNAT toggle](/images/docs/cloud/networking/create-and-manage-a-router/router-details-snat.png)</Frame>



### Configure a subnet

<p>The **Subnets** section lists subnets attached to the router. New subnets can be added and existing ones deleted.</p>

<p>When creating a subnet, pay attention to the supported CIDR ranges:</p>
...
<Warning>
A static route's next-hop IP must fall within the CIDR range of an attached subnet. If the subnet covering that next-hop is removed, traffic to the route destination will fail.
</Warning>

<p>Both IPv4 and IPv6 subnets are supported, but there are key differences. IPv4 is fully supported without restrictions. IPv6 has limitations: Floating IPs are not supported, private IPv6 subnets are not publicly routable, and only public IPv6 subnets can access the internet.</p>
...
<p>Host routes operate at Layer 2 without relying on a router, controlling traffic flow within a subnet. They are delivered to instances through DHCP or cloud-init — DHCP dynamically assigns them based on subnet configuration, while cloud-init applies predefined settings at instance startup.</p>

### Router static routes (Layer 3)

<p>Router-level static routes operate at Layer 3 and require a router to forward traffic between networks. When an instance sends traffic to a destination that matches a configured route, the router forwards it to the specified next-hop. When a router is present, it handles forwarding for all connected instances automatically.</p>
...
<Info>
**SUBNET\_ID** is the UUID of the subnet to attach to the router. To create a subnet or find its ID, see [Create and manage a subnet](/cloud/networking/create-and-manage-a-subnet).
</Info>

## Quickstart

<p>The scripts below create a router with SNAT enabled, attach a subnet, add a static route, then verify the result.</p>
...
<p>Static routes direct traffic to a destination CIDR via a specified next-hop IP. The `PATCH` call replaces the full routes list — include all routes on every update.</p>
...
<Info>
`var.subnet_id` references a subnet from [Create and manage a subnet](/cloud/networking/create-and-manage-a-subnet).
</Info>
