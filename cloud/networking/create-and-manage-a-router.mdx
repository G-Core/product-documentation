...
<Frame>![Add interfaces to router](/images/docs/cloud/networking/create-and-manage-a-router/create-router-add-interfaces.png)</Frame>



4\. (Optional) The **Enable SNAT** toggle is turned on by default to provide access to the external network. Turn it off if you don't need external network access. A separate public IP address is announced for a router with SNAT enabled. 
...
3\. On the router overview page, you can find general information such as router ID, SNAT status, creation dates, and external IPs. Here you can also enable or disable SNAT, configure and add new subnetworks, and update static routes.
...
You can enable or disable SNAT for an existing router at any time. In the **Router Details** section, use the **Enable SNAT** toggle to change the setting. When SNAT is enabled, instances in a private network can access the external network through the router.
...
5\. Enter Destination and next hop address.
