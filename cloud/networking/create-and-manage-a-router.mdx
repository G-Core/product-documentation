...
You can enable or disable SNAT for an existing router at any time. In the **Router Details** section, use the **SNAT** toggle to change the setting. When SNAT is enabled, instances in a private network can access the external network through the router.
