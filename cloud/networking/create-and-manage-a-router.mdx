...
<p>Provision a cloud router with external gateway SNAT and static routes using the [`gcore_cloud_network_router`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_network_router) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview). After the first `terraform apply`, replace `type = "default"` with `type = "manual"` and the resolved network ID to prevent configuration drift on subsequent runs.</p>
