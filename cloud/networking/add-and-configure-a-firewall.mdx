---
title: Add and configure a security group
sidebarTitle: Security Group
ai-navigation: Create and configure security groups and rules for Virtual Machines via Customer Portal, REST API, or Terraform.
---
...
<p>A security group is a set of rules that controls incoming and outgoing network traffic for Virtual Machines. Rules define which traffic is allowed or blocked based on protocol, port, and source or destination address.</p>
...
1. Open the creation form in one of two ways:
   ...
   - In the Cloud menu, navigate to **Networking** > **Security Groups** > **Create Security Group**.
   - When creating a Virtual Machine, navigate to **Network settings**, expand an interface, and configure **Security Groups** for that interface.

<Frame>![Security Groups list with Create Security Group button](/images/docs/cloud/networking/add-and-configure-a-firewall/security-groups-list-v2.png)</Frame>

2. Enter a name for the security group.

3. Set **Inbound rules** to define the allowed incoming traffic.
<p>Click **Add rule** and select one of the template rules, or select **Custom** to apply custom settings. A security group supports up to 250 rules total across inbound and outbound directions.</p>
   ...
   - Custom rule: for a custom rule, specify the protocol, port, Ethernet type (IPv4 or IPv6), and optionally a description.

<Frame>![Add rule dropdown showing template options including RDP TCP and RDP UDP](/images/docs/cloud/networking/add-and-configure-a-firewall/firewall-add-rule-templates-v2.png)</Frame>

<p>For **Sources**, specify the allowed sources for inbound traffic:</p>

- **IP with mask**: Specify an IP address range in CIDR format (192.168.1.0/24). If left empty with "All IPv4" selected, the rule applies to all IP addresses.
- **Select security groups**: Select existing security groups to allow traffic from the instances associated with those groups.
...
<Info>
By default, all outbound traffic is allowed. Add outbound rules only if specific destinations or protocols need to be restricted.
</Info>

<p>Click **Add rule** and select one of the template rules, or select **Custom** to apply custom settings.</p>

   - Template rules (All TCP/All UDP/SSH/HTTP/HTTPS/MySQL/DNS TCP/DNS UDP/PostgreSQL/RDP TCP/RDP UDP): template rules come with pre-configured protocols and ports for typical connections
   - Custom rule: for a custom rule, specify the protocol, port, Ethernet type (IPv4 or IPv6), and optionally a description.

<Frame>![Add firewall rule dialog showing protocol, port, and sources configuration](/images/docs/cloud/networking/add-and-configure-a-firewall/firewall-add-rule-sources-v2.png)</Frame>

<p>For **Destinations**, specify the allowed destinations for outbound traffic:</p>

- **IP with mask**: Specify an IP address range in CIDR format (192.168.1.0/24). If left empty with "All IPv4" selected, the rule applies to all IP addresses.
- **Select security groups**: Select existing security groups to allow traffic to the instances associated with those groups.
...
<Info>
Instances in the same security group no longer communicate with each other automatically. The self-referential rule that previously allowed intra-group traffic has been removed. If the setup relies on it, add an inbound rule that allows traffic from the security group itself.
</Info>
...
<p>Security groups protect cloud infrastructure by filtering network traffic based on defined rules. Rules, assignments to Virtual Machines, and deletion are all managed from the same security group view.</p>
...
2. Find the required security group, click the **...** menu on the right and select **Rules**.

<Frame>![Security group context menu showing Duplicate, Rules, and Instances options](/images/docs/cloud/networking/add-and-configure-a-firewall/security-group-context-menu-v2.png)</Frame>

### Assign and detach from Virtual Machines

<p>Security groups can be duplicated, assigned to Virtual Machines, or detached when no longer needed. To copy an existing security group with all its rules, click the **...** menu and select **Duplicate**. If the target security group is already applied to the Virtual Machine, no changes are made: the system automatically detects the current state and skips unnecessary updates.</p>

1. Navigate to **Networking** > **Security Groups**.

2. Find the required security group, click the **...** menu on the right and select **Instances**.

3. To assign the security group, click **Add Instances** and select one or more Virtual Machines from the list.

4. To detach the security group, select the checkbox next to the Virtual Machine and click **Delete**.

<Frame>![Security group instances list showing assigned Virtual Machines](/images/docs/cloud/networking/add-and-configure-a-firewall/security-group-instances-v2.png)</Frame>
...
2. Find the required security group, click the **...** menu on the right and select **Delete**.

### Manage tags

<p>Tags assigned at creation can be viewed and edited after the security group is created. Open the security group and switch to the **Tags** tab.</p>
...
<Frame>![Restore Default option for resetting firewall settings](/images/docs/cloud/networking/add-and-configure-a-firewall/restore-default-option-v2.png)</Frame>

## Security group on Bare Metal servers

<p>Security groups are not supported for Bare Metal servers.</p>

<Info>
Security groups are not supported for Bare Metal servers. Unlike Virtual Machines, Bare Metal servers operate directly on physical hardware and do not integrate with cloud-native security group management.
</Info>

<p>For network security, Bare Metal servers can use the following alternatives:</p>

- Configure network security manually using `iptables`, `nftables`, or similar tools at the OS level. The following commands allow SSH, HTTP, and HTTPS traffic while blocking everything else:
...
- **INSTANCE\_ID**: the UUID of a virtual machine in the region — create one via [Virtual&nbsp;Machines](/cloud/virtual-instances/create-an-instance).
...
- **SG\_ID**: the UUID of an existing security group, created via [Create&nbsp;security&nbsp;group](#create-a-security-group) below, or list existing ones with `GET /cloud/v2/security_groups/{project_id}/{region_id}`.
</Info>
...
| `remote_group_id` | No | UUID of another security group. Allows traffic from all instances assigned to that group. Use this instead of `remote_ip_prefix` when the traffic source is defined by group membership rather than an IP range. |
...
If the VM already exists, attach the security group by name:
...
<Tabs>
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        err := client.Cloud.Instances.AssignSecurityGroup(ctx, os.Getenv("INSTANCE_ID"), cloud.InstanceAssignSecurityGroupParams{
            Name: gcore.String("web-firewall"),
        })
        if err != nil {
            panic(err.Error())
...
<Tabs>
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        rule, err := client.Cloud.SecurityGroups.Rules.NewAndPoll(ctx, os.Getenv("SG_ID"), cloud.SecurityGroupRuleNewParams{
            Direction:      "ingress",
            Protocol:       "tcp",
...
<Tabs>
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        client.Cloud.SecurityGroups.Delete(ctx, os.Getenv("SG_ID"), cloud.SecurityGroupDeleteParams{})
    }
    ```
  </Tab>
  <Tab title="curl">
