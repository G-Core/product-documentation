...
<p>A firewall is a network security device used to protect servers from network threats. The firewall monitors incoming and outgoing network traffic and decides whether to allow or block specific traffic based on a defined set of security rules. Rules can be set for all connections except outbound traffic on port 25, which is blocked by default for spam prevention.</p>
...
3. Set **Inbound rules** to define the allowed incoming traffic.
   Click **Add rule** and select one of the template rules, or choose **Custom** to apply custom settings. A security group supports up to 250 rules total across inbound and outbound directions.
...
<p>To unblock port 25 for outbound SMTP traffic, [contact the Gcore support team](https://gcore.com/contact-us) with a business justification.</p>
...
<p>If no security group is specified when creating a Virtual Machine, the default security group is applied automatically.</p>

<p>The default security group contains inbound rules that allow the following traffic for both IPv6 and IPv4 addresses:</p>
...
<p>All outgoing connections are allowed.</p>
...
<p>The system creates a default security group automatically in the default project when the service is activated.</p>
...
<Frame>![Security group instances list showing assigned Virtual Machines](/images/docs/cloud/networking/add-and-configure-a-firewall/13258087088401.png)</Frame>
...
<Frame>![Security group context menu with Delete option selected](/images/docs/cloud/networking/add-and-configure-a-firewall/13258132640145.png)</Frame>

### Restore a default security group

<p>To restore the original rules, click **Restore Default** in the security group Rules view or use the API. All custom rules are replaced with the default configuration.</p>
