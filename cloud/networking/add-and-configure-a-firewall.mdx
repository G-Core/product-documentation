...
<p>Control inbound and outbound traffic by declaring a [`gcore_cloud_security_group`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_security_group) and its rules with the [Gcore Terraform provider v2](/developer-tools/terraform/overview). Each rule is a separate `gcore_cloud_security_group_rule` resource — outbound traffic is unrestricted by default unless you explicitly add egress rules.</p>
