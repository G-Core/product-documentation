...
A [security group](https://gcore.com/docs/api-reference/cloud/security-groups/create-security-group) controls which traffic is allowed in and out of a virtual machine. The steps below create a group with SSH, HTTP, HTTPS, and ICMP inbound rules, then attach it to a VM.
...
The scripts below create a security group with SSH, HTTP, HTTPS, and ICMP inbound rules, then attach it to a running VM.
...
<Tabs>
  <Tab title="Python SDK">
    ...
    # Step 2. Verify rules
    ingress_rules = [r for r in sg.security_group_rules if r.direction == "ingress"]
    print(f"Ingress rules: {len(ingress_rules)}")

    # Step 3. Attach to a running VM
...
### Step 2. Verify the security group

Retrieve the security group to confirm all rules were applied correctly. The [security groups API](https://gcore.com/docs/api-reference/cloud/security-groups) also supports listing all groups, updating rules in bulk, copying a group, and reverting to the default configuration.

<Tabs>
  <Tab title="Python SDK">
    ```python
    sg = client.cloud.security_groups.get(group_id=sg_id)
  ...
  <Tab title="Go SDK">
    ```go
    fetched, err := client.Cloud.SecurityGroups.Get(context.TODO(), sgID, cloud.SecurityGroupGetParams{
        ProjectID: gcore.Int(projectID),
        RegionID:  gcore.Int(regionID),
    })
    if err != nil {
    ...
    for _, r := range fetched.SecurityGroupRules {
        if r.Direction == "ingress" {
            fmt.Printf("ingress: %s port %d-%d\n", r.Protocol, r.PortRangeMin, r.PortRangeMax)
        }
    }
    ```
  </Tab>
...
A new security group is automatically populated with default egress rules that allow all outbound traffic. These do not need to be added manually.

### Step 3. Attach the security group to a VM
...
<Tabs>
  ...
  <Tab title="Go SDK">
    ...
    instance, err := client.Cloud.Instances.NewAndPoll(context.TODO(), cloud.InstanceNewParams{
        ...
        SecurityGroups: []cloud.InstanceNewParamsSecurityGroup{{ID: sgID}},
    })
    if err != nil {
        panic(err.Error())
    }
...
<Tabs>
  ...
  <Tab title="Go SDK">
    ```go
    rule, err := client.Cloud.SecurityGroups.Rules.NewAndPoll(context.TODO(), sgID, cloud.SecurityGroupRuleNewParams{
        ProjectID:      gcore.Int(projectID),
        RegionID:       gcore.Int(regionID),
        Direction:      "ingress",
        Protocol:       "tcp",
...
<Tabs>
  <Tab title="Python SDK">
    ...
    client.cloud.security_groups.delete(group_id=sg_id)
    ```
  </Tab>
  <Tab title="Go SDK">
    ```go
    ...
    client.Cloud.SecurityGroups.Delete(context.TODO(), sgID, cloud.SecurityGroupDeleteParams{
        ProjectID: gcore.Int(projectID),
        RegionID:  gcore.Int(regionID),
    })
    ```
  ...
  <Tab title="curl">
    ...
    # Detach from VM
    curl -X POST \
      "https://api.gcore.com/cloud/v1/instances/$PROJECT_ID/$REGION_ID/$INSTANCE_ID/delsecuritygroup" \
      -H "Authorization: APIKey $GCORE_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"name": "web-firewall"}'
