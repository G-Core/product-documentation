...
<p><strong>Outbound rules</strong> — all traffic is allowed except SMTP on ports TCP/25 and UDP/25, which are blocked by default to prevent spam. To unblock port 25, <a href="https://gcore.com/contact-us">contact the Gcore support team</a> with a business justification.</p>
...
<p>To restore the original rules, click **Restore Default** in the security group Rules view or use the API. This resets any custom changes to the current default rule set: inbound ICMP, ICMPv6, SSH (TCP/22), and RDP (TCP/3389 and UDP/3389); outbound unrestricted except SMTP (TCP/25 and UDP/25).</p>
