...
- **Public**: Add a public interface to give the VM internet access. Multiple public interfaces can be attached to the same VM — each receives a separate external IP address. To use a specific IP, enable **Use reserved IP** when adding the interface.
