...
Networks (also known as network interfaces) can be attached to [Virtual Machines (VMs)](/cloud/virtual-instances/types-of-virtual-machines), [Bare Metal](/cloud/bare-metal-servers/about-bare-metal-servers), [Load Balancers](/cloud/networking/create-and-configure-a-load-balancer), and [Managed Kubernetes](/cloud/kubernetes/about-gcore-kubernetes) clusters.

## Types of networks

The following network types are available for Cloud resources:
...
Both public and private networks can be attached to VMs, Bare Metal, Load Balancers, and Managed Kubernetes clusters.
...
For each resource type, the network interface is configured separately:
...
New network interfaces can be created in the [Gcore Customer Portal](https://portal.gcore.com), on the **Networks** page.
...
To configure a private network, add a [subnetwork](/cloud/networking/create-and-manage-a-subnetwork#create-a-subnetwork-from-the-networks-page) as well.
...
To add more networks to an existing cloud resource, go to the resource settings:
...
The network interface is now attached to the resource.

## Detach a network from a VM or Bare Metal

To remove an interface from a cloud resource, detach all subnetworks within the network interface:
...
Detach any connected Floating IP before removing subnetworks.
...
All configured network interfaces and their subnetworks appear in the [Gcore Customer Portal](https://portal.gcore.com), on the **Networks** page.

Network details, renaming, deletion, and subnetwork configuration are all available from this page.
...
[Subnetworks](/cloud/networking/create-and-manage-a-subnetwork) can also be created and removed from this page.

### Port security

Port Security is enabled by default for all VMs, allowing only authorized traffic through network interfaces. To establish connectivity, a firewall must be configured. In some cases, such as specific network configurations or applications that require unrestricted access, turning off Port Security may be necessary.
