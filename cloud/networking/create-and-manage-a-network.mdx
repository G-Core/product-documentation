...
For curl, poll <code>GET&nbsp;/cloud/v1/tasks/{task_id}</code> every 5 seconds until `state` is `FINISHED`:
...
<Tabs>
  ...
  <Tab title="Python SDK">
    ```python
    vm1 = client.cloud.instances.create_and_poll(
        project_id=int(os.environ["PROJECT_ID"]),
        region_id=int(os.environ["REGION_ID"]),
        name="vm-1",
        flavor="g1-standard-2-4",
...
For curl, poll until `FINISHED` and extract the instance ID from `created_resources.instances[0]`.
...
For curl, poll both tasks to `FINISHED` before deleting network resources.
