...
A [network](/api-reference/cloud#tag/Networks/operation/NetworkCreateV1.post) is the top-level container. Creating it also provisions a router that can later connect the subnet to external networks.
...
A [subnet](/api-reference/cloud#tag/Subnets/operation/SubnetCreateV1.post) defines the IP range and DHCP settings for the network. VMs attached to this network receive addresses from this range.
