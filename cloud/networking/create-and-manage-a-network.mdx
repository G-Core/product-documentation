...
<p>To configure a private network, add a [subnet](/cloud/networking/create-and-manage-a-subnet#create-a-subnet-from-the-networks-page) as well.</p>
...
- **Public**: Add a public interface to give the VM internet access. Multiple IP addresses from the same external network can be attached to the same VM — each receives a separate external IP address. A VM can only be connected to one external network at a time; adding interfaces from two different external networks is not supported. To use a specific IP, enable **Use reserved IP** when adding the interface.

<Info>
  [Floating IPs](/cloud/networking/ip-address/create-and-configure-a-floating-ip-address) can also provide additional public addresses when applied to a private interface.
</Info>

- **Private**: Choose a network from the list or create a new one, then [configure a subnet](/cloud/networking/create-and-manage-a-subnet) as required.
...
<p>To remove an interface from a cloud resource, detach all subnets within the network interface:</p>
...
5. Click the three-dot icon next to each subnet within the network and select **Detach Subnet**.

<Frame>
  ![Network settings with the detach subnet option](/images/docs/cloud/networking/create-and-manage-a-network/detach-subnetwork.png)
</Frame>

<p>Detach any connected Floating IP before removing subnets.</p>

6. Confirm the action by clicking **Detach**.

<p>Repeat this step for all subnets within the network.</p>
...
<p>All configured network interfaces and their subnets appear in the [Gcore Customer Portal](https://portal.gcore.com), on the **Networks** page.</p>

<p>Network details, renaming, deletion, and subnet configuration are all available from this page.</p>
...
<p>[Subnets](/cloud/networking/create-and-manage-a-subnet) can also be created and removed from this page.</p>
...
<p>A rename runs as a background task. The SDK waits for the task and returns the network. curl polls the task, then GET returns the updated name.</p>

<Tabs>
  <Tab title="Python SDK">
    ```python
    updated = client.cloud.networks.update_and_poll(network_id, name="my-network-renamed")
    print(f"New name: {updated.name}")
    ```
  </Tab>
  <Tab title="Go SDK">
    ```go
    updated, err := client.Cloud.Networks.UpdateAndPoll(ctx, networkID, cloud.NetworkUpdateParams{
        Name: gcore.String("my-network-renamed"),
    })
    if err != nil {
        panic(err.Error())
    }
  ...
  <Tab title="curl">
    ```bash
    curl -X PATCH "https://api.gcore.com/cloud/v2/networks/$GCORE_CLOUD_PROJECT_ID/$GCORE_CLOUD_REGION_ID/$NETWORK_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY" \
      -H "Content-Type: application/json" \
    ...
    ```json
    {
      "tasks": ["02b255b4-5888-460f-8402-acfe629333f9"]
    }
    ```

    Poll <code>GET /cloud/v1/tasks/{task_id}</code> until `state` is `FINISHED`. Then GET the network:

    ```bash
    curl "https://api.gcore.com/cloud/v1/networks/$GCORE_CLOUD_PROJECT_ID/$GCORE_CLOUD_REGION_ID/$NETWORK_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
