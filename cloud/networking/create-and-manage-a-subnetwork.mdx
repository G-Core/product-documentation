---
title: "Create and manage a subnet"
sidebarTitle: "Subnet"
ai-navigation: Create, update, and delete subnets in Gcore Cloud via the Customer Portal, REST API, or Terraform.
---
...
<p>A subnet is a range of IP addresses in a cloud network. Addresses from this range will be assigned to Virtual Machines in the cloud.</p>

## Create a subnet

<p>A subnet can be created [during Virtual Machine creation](/cloud/virtual-instances/create-an-instance) or from the **Networks** page.</p>
...
<Frame>
  ![Create a subnet dialog](/images/docs/cloud/networking/create-and-manage-a-subnetwork/create-subnetwork.png)
</Frame>

#### CIDR requirements

- The CIDR must not overlap with other subnets in the same network — overlapping ranges cause network conflicts and break connectivity.
- The CIDR cannot be changed after the subnet is created.
...
- Subnets with a /31 or /32 mask require both the gateway and DHCP to be disabled — DHCP is incompatible with these prefix lengths.
...
<Warning>
  For IPv6 networks, DHCP can only be enabled or disabled during subnet creation. Changing this setting later requires recreating the IPv6 subnet.
</Warning>
...
<p>The gateway IP must meet these requirements before saving the subnet configuration.</p>

- The gateway IP must fall within the subnet's CIDR range. For 192.168.0.0/24, the valid range is 192.168.0.1–192.168.0.254. The network address (.0) and broadcast address (.255) are not valid.
- Assigning a **Specific** gateway to more than one subnet in the same network causes routing conflicts — only one subnet should use a specific gateway IP.
- When a Gcore Virtual Machine or Bare Metal server has both public and private interfaces, disable the default gateway on all private subnets — a conflict with the server's default gateway will prevent access.

<Frame>
  ![Edit subnet dialog](/images/docs/cloud/networking/create-and-manage-a-subnetwork/disable-router-gateway.png)
</Frame>
...
<Frame>
  ![Create subnet dialog showing non-routable option](/images/docs/cloud/networking/create-and-manage-a-subnetwork/enable-non-routable-subnetwork.png)
</Frame>

### Step 4. Configure DNS, host routes, and tags

1. (Optional) Specify custom DNS servers for the subnet. If left blank, DNS servers are assigned automatically based on the subnet configuration.
...
3. (Optional) Select **Add tags** to add metadata to the subnet.
4. Click **Create subnet**.

<Frame>
  ![Custom DNS and host routes fields in the Create Subnet dialog](/images/docs/cloud/networking/create-and-manage-a-subnetwork/create-subnetwork-custom-dns.png)
</Frame>
...
<p>Each subnet supports up to 20 host routes. If the next-hop IP is unreachable from the subnet, traffic to that destination will fail.</p>

## Manage subnets

<p>Subnets are managed from the **Networking** page of the Customer Portal.</p>
...
2. Find the network containing the subnet and click its name to open it.
3. Click the three-dot icon next to the subnet, then click **Edit**.
...
4. Update the subnet's name.
5. Click **Save**.

<p>The subnet has been renamed.</p>

### Delete a subnet

<p>A subnet cannot be deleted if any of its IP addresses are in use — for example, if a Virtual Machine, load balancer, or other resource still has a port in that subnet.</p>

1. In the Customer Portal, navigate to **Cloud** > **Networking**.
2. Find the network containing the subnet and click its name to open it.
3. Click the three-dot icon next to the subnet, then click **Delete**.
4. Click **Delete** again to confirm.

<Frame>
  ![Delete subnet confirmation dialog](/images/docs/cloud/networking/create-and-manage-a-subnetwork/confirm-subnetwork-deletion.png)
</Frame>

<p>The subnet has been removed.</p>
...
<Info>
**NETWORK\_ID** is the UUID of the [network](/cloud/networking/create-and-manage-a-network) in which to create the subnet.
</Info>
...
| `ip_version` | No | IP version: `4` (default) or `6`. |
...
| `router_id_to_connect` | No | UUID of a specific router to connect this subnet to. Requires `connect_to_network_router: true`. If omitted, the router created with the network is used. |
