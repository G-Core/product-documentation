...
<p>7. (optional) Select the **Enable DHCP** checkbox to assign IP addresses to machines in the subnet automatically.</p>
...
<p>9. (optional) Specify custom DNS servers for the subnetwork. If left blank, DNS servers are assigned automatically based on the subnet configuration.</p>
...
The default gateway controls how traffic leaves a subnetwork. Leave the **Gateway IP** set to **Automatically** unless a specific IP is required. Misconfiguring it can cause connectivity loss, so review the following guidelines before making changes.
