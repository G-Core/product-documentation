...
<p>9. (optional) Specify custom DNS servers for the subnetwork. If left blank, the system assigns defaults automatically: subnets with DHCP enabled use the local DHCP server as resolver; subnets with DHCP disabled but a gateway receive `95.85.95.85` and `8.8.8.8`.</p>
...
Subnetworks are routable by default. To restrict machines from external connections, turn off the **Connect to network router** toggle during subnet creation.
...
When configuring a gateway in a private subnetwork, verify that only one subnetwork is routable by checking that only one subnetwork has the **Gateway IP** set to **Specific**.
