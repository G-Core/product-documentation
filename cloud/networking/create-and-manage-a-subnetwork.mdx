...
<p>Create a subnet inside an existing [network](/cloud/networking/create-and-manage-a-network) and configure its CIDR range, DHCP settings, DNS servers, and gateway using the [`gcore_cloud_network_subnet`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_network_subnet) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview). Changing any field recreates the subnet — Virtual Machines attached to it will briefly lose connectivity.</p>
