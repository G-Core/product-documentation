...
<Frame>
 ...
 9. (optional) Specify custom DNS servers for the subnetwork.

    If left blank, the system assigns defaults automatically based on the DHCP setting:

    - **DHCP enabled** — no explicit DNS servers are set. Instances receive the local DHCP server as their resolver, which handles both internal hostnames and external queries.
    - **DHCP disabled with a gateway** — `95.85.95.85` and `8.8.8.8` are assigned automatically.

    To disable DNS resolution entirely, enter an empty list explicitly after creating the subnet via the API (`dns_nameservers: []`).
