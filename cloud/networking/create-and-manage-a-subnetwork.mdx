...
Set the mask between 16 and 24. See [Set the IP range](#set-the-ip-range) below for details.
...
Your subnetwork's IP range must not overlap with others in the same network. Overlapping subnets cause network conflicts and break connectivity.

Virtual machines in the same network communicate using private IP addresses, but subnetworks in different networks cannot communicate unless you configure routing.
...
For a full list of parameters and response schemas, see the [Subnets API reference](https://api.gcore.com/docs/cloud#tag/Subnets).
