...
A subnetwork can be created [during Virtual Machine creation](/cloud/virtual-instances/create-an-instance) or from the **Networks** page, as described below.
...
4. A new **Create subnetwork** page will open.
...
Set the mask between 16 and 29. See [IP range](#ip-range) below for details.
...
8. (optional) Turn on the **Non-routable subnetwork** toggle to block access to the subnet from external networks and other subnets. For routable networks, specify a custom **Gateway IP** address, or leave it blank to assign one automatically.
...
<Frame>
 ...
 9. (optional) Specify custom DNS servers for the subnetwork. If left blank, the system assigns defaults automatically: subnets with DHCP enabled use the local DHCP server as resolver; subnets with DHCP disabled but a gateway receive `95.85.95.85` and `8.8.8.8`.
...
The address range is specified in CIDR format when creating a subnetwork.

<Frame>
  ![Create a subnetwork dialog](/images/docs/cloud/networking/create-and-manage-a-subnetwork/create-subnetwork.png)
</Frame>

The subnetwork's IP range must not overlap with others in the same network — overlapping subnets cause network conflicts and break connectivity.
...
Subnetworks are routable by default. To restrict machines from external connections, enable the **Non-routable subnetwork** option during subnetwork creation.
...
The default gateway controls how traffic leaves a subnetwork. Misconfiguring it can cause connectivity loss, so verify the settings below before making changes.
...
When configuring a gateway in a private subnetwork, verify that only one subnetwork is routable by checking that only one subnetwork has the **Enable router gateway** toggle active.

<Frame>
  ![Edit subnetwork dialog](/images/docs/cloud/networking/create-and-manage-a-subnetwork/disable-router-gateway.png)
</Frame>

When a private interface is added after creating a server, some operating systems may not activate it automatically — this is especially common for Bare Metal servers. In such cases, configure the interface manually.

**Gateway IP validation**

Gateway IP validation prevents misconfiguration during creation or updates. The gateway IP must fall within the subnetwork's CIDR range — for a CIDR of 192.168.0.0/24, the gateway IP must be between 192.168.0.0 and 192.168.0.255.
...
| `dns_nameservers` | No | DNS servers to advertise via DHCP. Omit or pass `null` to auto-assign defaults: DHCP-enabled subnets use the local DHCP server as resolver; subnets with DHCP disabled but a gateway receive `95.85.95.85` and `8.8.8.8`. Pass `[]` to explicitly disable DNS. |
