...
Reserve a shared IP address and attach it to multiple Virtual Machines to enable high-availability failover configurations.
...
Setting `is_vip = true` reserves the IP with VIP mode already active.
...
`port_id` is the identifier used in all subsequent operations — keep it in the Terraform output or pass it to the API.

## Enable or disable VIP mode

Changing `is_vip` is an in-place update — the resource is not recreated:
...
Run `terraform apply` after the change.
...
To bring an existing VIP under Terraform management:

```bash
terraform import gcore_cloud_reserved_fixed_ip.vip '{PROJECT_ID}/{REGION_ID}/{PORT_ID}'
```

Replace `{PROJECT_ID}`, `{REGION_ID}`, and `{PORT_ID}` with the values from the Customer Portal or from the reserved IPs API response.
