...
<p>If **Private** is selected, specify the network and subnetwork. In the **IP address to reserve** section, select **Custom** to specify a particular IP address from the valid CIDR, or leave **Any available IP** selected to have one assigned automatically.</p>
...
<p>The virtual IP address is now reserved and can be assigned to multiple machines to create a fault-tolerant system, or added as a secondary address for a network interface.</p>
...
<p>3\. Install and configure Keepalived on the Virtual Machines using [the official Keepalived documentation](https://keepalived.readthedocs.io/en/latest/installing_keepalived.html).</p>
...
<p>5\. Run Keepalived on the Virtual Machine and add it to the autostart list.</p>
...
<p>1\. Connect to the Virtual Machine.</p>
...
<p>For example, to add the virtual IP address _192.168.1.200_ with a subnet mask of _24_ to the _eth0_ interface, use the following command:</p>
...
<p>Note: **the command assigns the VIP to the machine until it restarts**. After rebooting, run the command again to re-assign the VIP. To permanently assign a VIP, refer to the official documentation of the OS to add the IP address to the system network settings.</p>
...
<p>7\. In the next window, enter the VIP and the subnet mask from the command line. Then click **Add**.</p>
