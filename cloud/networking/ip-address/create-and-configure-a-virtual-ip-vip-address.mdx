...
The `reservation.status` of `"attached"` confirms VIP mode is active. Additional creation options such as specifying a private network or a fixed IP address are described in the [reserved IPs API](/api-reference/cloud/reserved-ips/create-reserved-fixed-ip).
