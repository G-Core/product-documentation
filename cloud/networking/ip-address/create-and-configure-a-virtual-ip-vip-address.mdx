...
<p>A Virtual Machine can have multiple public interfaces, each receiving a separate external IP address from the public network. Floating IPs from private network interfaces are also supported, subject to quotas. In a multi-network setup, each subnet can have one Floating IP. Within a single network, multiple subnets can exist, and each subnet of a single machine (VM/BM) is allowed one Floating IP.</p>
