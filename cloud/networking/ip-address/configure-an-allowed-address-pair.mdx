...
- **INSTANCE\_ID**: the UUID of the virtual machine whose port you want to configure. To create a VM or find its ID, see [Create a virtual machine](/cloud/virtual-instances/create-an-instance).
- **PORT\_ID**: the UUID of the VM's **private** network interface port. The external (public) interface does not support allowed address pairs. See the [Find the port ID](#find-the-port-id) section below for how to retrieve it.
</Info>
