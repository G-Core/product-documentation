...
ai-navigation: Assign additional IP addresses to VM network interfaces using the allowed address pairs API endpoint with MAC-IP binding configuration.
ai-navigation: Assign allowed address pairs to Virtual Machine ports via API to enable traffic routing to multiple IP addresses.
---
...
In Gcore, you can add an allowed address pair via an [API request](/api-reference/cloud/instances/assign-allowed-address-pairs-for-instance-port).

## Assign an allowed address pair

1\. Generate an authorization token or use a [permanent API token](/account-settings/api-tokens).
