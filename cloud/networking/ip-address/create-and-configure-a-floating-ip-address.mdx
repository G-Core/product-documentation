...
<p>When a floating IP is reserved and assigned to a Virtual Machine, the router directs incoming traffic to the device by forwarding packets intended for that IP. Outgoing traffic from the instance will still use its private IP, while incoming traffic will be forwarded through the Floating IP.</p>
...
<p>Before assigning a floating IP, confirm that the target subnet has routing enabled. The steps below show how to check and enable this setting.</p>
...
1. Open the **Networking** section, select the required network, and navigate to the subnet settings.
...
1. Select the Region where the machine is located to assign a floating IP and navigate to the **Networking** section.

<Frame>
  ![floating-ip-04.png](/images/docs/cloud/networking/ip-address/create-and-configure-a-floating-ip-address/12391851006353.png)
</Frame>

2. Open the Floating IPs section. A floating IP can be created without assigning it to a machine by clicking **Create a new floating IP.**

<Frame>
  ![floating-ip-05.png](/images/docs/cloud/networking/ip-address/create-and-configure-a-floating-ip-address/12392006241425.png)
</Frame>

3. Additionally, a floating IP can be created and immediately assigned to a specific Virtual Machine. To do this, enable the **Attach to existing instance** toggle, select the machine and interface, and click **Create a new floating IP.**
...
<p>When creating a VM, in the "Network settings" section, select the "**Private** " network type. Specify the network and subnet of the interface to assign the floating IP. Enable the "**Use floating IP** " toggle, click "**Create a new floating IP** ", and save the settings using the "**Add Interface** " button.</p>
...
<p>A floating IP address can be detached in the "Floating IPs" section or in the Virtual Machine menu. In the "Floating IP" section, click the selector next to the address and select **Detach from Instance**, and the IP will no longer be assigned to the machine.</p>
...
<p>In the "Floating IPs" section, click the selector next to the desired address and select **Delete**. The IP will be removed and billing stops.</p>
