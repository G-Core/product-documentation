...
<p>Reserve a public static IP and optionally bind it to a Virtual Machine's private interface in a single configuration using the [`gcore_cloud_floating_ip`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_floating_ip) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview). The VM's subnet must have a router with an external gateway — without it, assignment succeeds but traffic cannot route.</p>
