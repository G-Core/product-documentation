...
The scripts below reserve a public IPv4 address, create a Virtual Machine that uses it as its network interface, then delete both resources. Set the required environment variables before running:
...
The reserved IP's `port_id` is the identifier passed in the `interfaces` block during VM creation. Set `type` to `"reserved_fixed_ip"` and supply `port_id` — the VM receives the reserved address as its network interface.
...
After creation, adding the reserved IP to a running VM requires attaching a new interface through the [instance API](https://api.gcore.com/docs/cloud#tag/Instances/operation/InstanceAddInterfaceV2.post). The same operation is available in the Customer Portal via the VM **Networking** tab — see the [Portal tab](#portal) for step-by-step instructions.
...
<p>Declare a `gcore_cloud_reserved_fixed_ip` resource to allocate a dedicated IP address that persists independently of Virtual Machines and can be reassigned without changing DNS records or firewall rules.</p>
...
`port_id` and `id` contain the same UUID — this is the identifier passed to a Virtual Machine's interface configuration (see below).
...
To request a specific address within the subnet, add `ip_address = "192.168.1.100"` and change `type` to `"ip_address"` with `network_id` instead of `subnet_id`.
...
The reserved IP remains allocated even if the instance is destroyed. Reassign it to a new VM by updating `port_id` in a different instance resource and running `terraform apply`.

<Warning>
To reserve an IP that will be used as a [Virtual IP address](/cloud/networking/ip-address/create-and-configure-a-virtual-ip-vip-address), set `is_vip = true` when creating the resource. This flag cannot be changed after creation.
</Warning>
...
The IP must not be attached to any resource before deletion.
