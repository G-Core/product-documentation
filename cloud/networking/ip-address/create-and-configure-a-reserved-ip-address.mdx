...
<Tabs>
  ...
  <Tab title="After VM creation">
    ...
    <Warning>
A reserved IP is a standalone network port. If the VM already has an interface on the same public network (pub_net), adding a second reserved IP from that network will be rejected — the reserved IP port cannot be merged with the existing port. To assign a specific reserved IP when the network is already in use, detach the existing public subnetwork first, then add the new interface with the reserved IP.

This restriction applies to the standard public network. In a dedicated external network, additional IP addresses can be added without detaching the existing interface — they are assigned to the same port automatically.
</Warning>
  </Tab>
</Tabs>
