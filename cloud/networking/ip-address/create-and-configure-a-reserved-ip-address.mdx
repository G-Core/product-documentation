...
<p>Allocate a dedicated IP address that persists independently of Virtual Machines using the [`gcore_cloud_reserved_fixed_ip`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_reserved_fixed_ip) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview). The IP can be reassigned between Virtual Machines without updating DNS records or firewall rules.</p>
