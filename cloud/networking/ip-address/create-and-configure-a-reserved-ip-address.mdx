...
<p>After creation, adding the reserved IP to a running VM requires attaching a new interface through the [instance API](/api-reference/cloud/instances/attach-interface). The same operation is available in the Customer Portal via the VM **Networking** tab — see the [Portal tab](#portal) for step-by-step instructions.</p>
