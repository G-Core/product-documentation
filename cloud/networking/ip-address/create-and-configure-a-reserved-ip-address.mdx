...
- **Public** (for external network). A public IP address can be created immediately and used for external connectivity.
- **Private** (for internal network). For this option, select the network and subnetwork in which the address will be created.

<Info>

The system automatically filters out regional external networks from the available network list.

</Info>

<Info>

A reserved private IP can only be used in the specified subnetwork. To request a reserved IP in a different subnetwork, create a separate reservation.
