...
<p>After creation, adding the reserved IP to a running VM requires attaching a new interface through the [instance API](https://api.gcore.com/docs/cloud#tag/Instances/operation/InstanceAddInterfaceV2.post). The same operation is available in the Customer Portal via the VM **Networking** tab — see the [Portal tab](#portal) for step-by-step instructions.</p>
