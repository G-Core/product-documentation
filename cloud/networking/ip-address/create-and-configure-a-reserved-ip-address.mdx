...
4. In the **Reserve IP for** section, keep **Network interface** selected to allocate a new address. Select **Specific port on a resource** to mark an existing Virtual Machine interface as reserved â€” then choose the instance type, instance, and network interface.

5. In the **Network type** section, select the network type:

- **Public**: External network for internet-facing IP addresses.
- **Private**: Internal network within the project. For this option, select the [network](/cloud/networking/create-and-manage-a-network) and optionally a specific [subnet](/cloud/networking/create-and-manage-a-subnet).
- **Dedicated public**: Dedicated public network (if configured for the project).

<p>When a network is selected without specifying a subnet, the IP address is allocated in any available subnet of that network.</p>

<Info>
A reserved private IP works within the specified subnet. To request a specific address, in the **IP address to reserve** section, select **Custom** and enter the IP that belongs to that subnet.
</Info>
...
<Tabs>
  ...
  <Tab title="After VM creation">
    ...
    2. Click **Add Interface**, select **Public**, and enable the **Use reserved IP** toggle.
    ...
    <p>The reserved IP is assigned as a new public interface alongside any existing interfaces.</p>

    <Warning>
A reserved IP is a standalone network port. If the VM already has an interface on the same public network (pub_net), adding a second reserved IP from that network will be rejected â€” the reserved IP port cannot be merged with the existing port. To assign a specific reserved IP when the network is already in use, detach the existing public subnet first, then add the new interface with the reserved IP.

This restriction applies to the standard public network. In a dedicated external network, additional IP addresses can be added without detaching the existing interface â€” they are assigned to the same port automatically.
</Warning>
  </Tab>
</Tabs>
...
4. Click the three-dot menu next to the subnet row.
5. Select **Detach subnet**.
...
<Tabs>
  <Tab title="Python SDK">
    ...
    client.cloud.instances.delete_and_poll(instance.id)
    client.cloud.volumes.delete_and_poll(volume.id)
    client.cloud.reserved_fixed_ips.delete_and_poll(rip.port_id)
    print("Cleaned up.")
    ```
  </Tab>
  <Tab title="Go SDK">
...
Reserving an IP allocates a static address from the region pool exclusively for this project â€” no other account can claim it.
...
| `is_vip` | No | Set to `true` to create as a [Virtual IP address](/cloud/networking/ip-address/create-and-configure-a-virtual-ip-vip-address) |
    ...
    <Tabs>
      ...
      <Tab title="curl">
        ...
        Save `created_resources.ports[0]` as `PORT_ID` â€” this is the identifier used in all subsequent calls for this reserved IP.
      </Tab>
    </Tabs>
...
<Info>
`SUBNET_ID` is the UUID of the subnet from which to allocate the private IP. To create a subnet or find its ID, see [Create and manage a subnet](/cloud/networking/create-and-manage-a-subnet).
</Info>
...
<Tabs>
  <Tab title="Python SDK">
    ```python
    rip = client.cloud.reserved_fixed_ips.get(os.environ["PORT_ID"])
...
<Tabs>
  <Tab title="Python SDK">
    ```python
    client.cloud.reserved_fixed_ips.delete_and_poll(os.environ["PORT_ID"])
...
<p>`port_id` and `id` contain the same UUID â€” this is the identifier passed to a Virtual Machine's interface configuration (see below).</p>
...
<Warning>
To reserve an IP for use as a [Virtual IP](/cloud/networking/ip-address/create-and-configure-a-virtual-ip-vip-address), set `is_vip = true` when creating the resource. This field can be changed after creation via PATCH — the resource is updated in place without recreation.
</Warning>

## Delete a reserved IP

<p>Remove the resource block â€” Terraform releases the address and stops billing on the next `terraform apply`.</p>
