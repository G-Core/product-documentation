...
<Steps>
  ...
  <Step title="Initialize logging">
    ...
    {/* NOTE for reviewers (not for publication):
        The screenshot below is currently broken because both the image file
        and the article path use the extensionless name `load-balancers-moitoring-4`.
        Web images without a file extension (.png, .jpg, etc.) do not render,
        so fixing this correctly requires renaming the underlying file to include
        the proper extension and updating the path together in a follow-up change.
    */}
