...
<p>Creating and managing TLS secrets is fully supported via `gcore_cloud_secret`:</p>
...
<p>For a full walkthrough including secret rotation, see [Configure secrets for HTTPS load balancer](/cloud/secrets-manager/upload-a-pkcs12-file).</p>

<p>Attaching a certificate to a `TERMINATED_HTTPS` listener is not currently supported by the Terraform provider v2. Use the [REST API tab](#api) or the [Customer Portal tab](#portal) for that step.</p>
