...
ai-navigation: Upload TLS certificates to Gcore Secrets Manager and attach them to TERMINATED HTTPS load balancer listeners to enable SNI-based multi-domain termination; create listeners with a default certificate and add per-domain SNI certificates via the REST API.
---
...
TLS termination on a load balancer requires two resources: a certificate stored in Secrets Manager and a `TERMINATED_HTTPS` listener that references it. A listener accepts one default certificate and any number of SNI certificates - the load balancer selects the correct certificate for each connection based on the domain name in the TLS handshake.

<Info>
A permanent [API token](/account-settings/api-tokens) is required, along with a [project ID](https://api.gcore.com/docs/cloud#tag/Projects/operation/ProjectsListV1.get), a [region ID](https://api.gcore.com/docs/cloud#tag/Regions/operation/RegionListV1.get), and the ID of an existing [load balancer](/cloud/networking/load-balancers/manage-load-balancers).
</Info>
...
The scripts below upload a TLS certificate to Secrets Manager and create a `TERMINATED_HTTPS` listener on an existing load balancer, then attach a second SNI certificate for an additional domain.
...
<Tabs>
  <Tab title="Python SDK">
    ...
    # Step 3. Upload an SNI certificate for an additional domain
    sni_cert_pem = open("/path/to/sni-certificate.pem").read()
    sni_key_pem = open("/path/to/sni-private-key.pem").read()
    sni_chain_pem = open("/path/to/sni-chain.pem").read()

    sni_secret = client.cloud.secrets.upload_tls_certificate_and_poll(
        name="my-sni-cert",
        payload=secret_upload_tls_certificate_params.Payload(
            certificate=sni_cert_pem,
            private_key=sni_key_pem,
            certificate_chain=sni_chain_pem,
        ),
    )
    print(f"SNI secret: {sni_secret.id}")

    # Step 4. Add the SNI certificate to the listener
  ...
  <Tab title="Go SDK">
    ...
    func main() {
    	...
    	fmt.Printf("SNI secret: %s\n", sniSecret.ID)

    	// Step 4. Add the SNI certificate to the listener
    	updated, err := client.Cloud.LoadBalancers.Listeners.UpdateAndPoll(ctx, listener.ID, cloud.LoadBalancerListenerUpdateParams{
...
<Tabs>
  ...
  <Tab title="curl">
    ...
    curl -X POST "https://api.gcore.com/cloud/v2/secrets/$PROJECT_ID/$REGION_ID" \
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl "https://api.gcore.com/cloud/v1/secrets/$PROJECT_ID/$REGION_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
...
<Tabs>
  ...
  <Tab title="curl">
    ...
    ```bash
    curl "https://api.gcore.com/cloud/v1/lblisteners/$PROJECT_ID/$REGION_ID/$LISTENER_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl -X PATCH "https://api.gcore.com/cloud/v2/lblisteners/$PROJECT_ID/$REGION_ID/$LISTENER_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY" \
      -H "Content-Type: application/json" \
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    # Delete the listener first
    curl -X DELETE "https://api.gcore.com/cloud/v1/lblisteners/$PROJECT_ID/$REGION_ID/$LISTENER_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
    ...
    ```bash
    curl -X DELETE "https://api.gcore.com/cloud/v1/secrets/$PROJECT_ID/$REGION_ID/$SECRET_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"

    curl -X DELETE "https://api.gcore.com/cloud/v1/secrets/$PROJECT_ID/$REGION_ID/$SNI_SECRET_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
  </Tab>
</Tabs>
