...
<p>Attach TLS certificates to a load balancer listener using the [`gcore_cloud_load_balancer_listener`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_load_balancer_listener) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview). Pass certificates via `sni_secret_id` as a list of secret UUIDs; adding or removing a certificate replaces the full SNI list.</p>
