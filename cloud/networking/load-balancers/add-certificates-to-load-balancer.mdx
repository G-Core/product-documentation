...
<p>Use `gcore_cloud_secret` to provision a TLS certificate and output its ID. Increment `payload_wo_version` to rotate the certificate — Terraform replaces the secret and issues a new ID.</p>
...
<p>To attach the certificate to a `TERMINATED_HTTPS` listener, use the [REST API tab](#api) or the [Customer Portal tab](#portal).</p>
