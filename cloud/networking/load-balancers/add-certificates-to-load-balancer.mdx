...
<p>Multiple TLS (Transport Layer Security) certificates can be configured for a single Load Balancer, enabling several secure websites on a single IP address.</p>

<p>Instead of deploying separate Load Balancers for each website, a single Load Balancer can be configured with multiple TLS certificates. The Load Balancer serves the correct certificate based on the requested domain name.</p>
...
<p>Each listener can also be configured with its own TLS certificate, corresponding to a specific hostname or domain.</p>

## Add multiple certificates

<p>Multiple certificates can be added to listeners that use the Terminated HTTPS and Prometheus protocols.</p>

### 1\. Add certificates to the Secrets Manager

<p>Add the required certificates to Gcore Secrets Manager. If no certificates exist yet, follow the [upload guide](/cloud/secrets-manager/upload-a-pkcs12-file) to configure secrets for HTTPS Load Balancers.</p>

### 2\. Add certificates to a listener

<p>After configuring the certificates, add them to the relevant listener. This can be done during Load Balancer creation or in the settings of an existing Load Balancer.</p>
...
<Tabs>
  <Tab title="During Load Balancer creation">
    ...
    <Frame>
      <img src="/images/docs/cloud/networking/load-balancers/add-certificates-to-load-balancer/add-listener-name-port.png" alt="Create a listener dialog" width="70%"/>
    </Frame>

    4\. (Optional) To identify the origin of the user's IP address connecting to a web server via a Load balancer, select the **Add headers X-Forwarded-For, X-Forwarded-Port, X-Forwarded-Proto to requests** checkbox.
    ...
    <Frame>
      <img src="/images/docs/cloud/networking/load-balancers/add-certificates-to-load-balancer/add-listener-headers-cni.png" alt="Create a listener dialog" width="70%"/>
    </Frame>

    8\. (Optional) In the **Allowed CIDRs** field, specify the IP addresses that can access the content. IP addresses outside the specified range will be denied access.
    ...
    <p>After configuring the listener, proceed with the remaining steps in the [Create a Load Balancer](/cloud/networking/create-and-configure-a-load-balancer) guide to complete the creation.</p>

    <Frame>
      <img src="/images/docs/cloud/networking/load-balancers/add-certificates-to-load-balancer/create-listener.png" alt="Create a listener dialog" width="70%"/>
    </Frame>
  </Tab>
  <Tab title="From the Load Balancer settings">
    ...
    <Frame>
      <img src="/images/docs/cloud/networking/load-balancers/add-certificates-to-load-balancer/load-balancers-page.png" alt="Load Balancers page in Customer Portal" width="70%"/>
    </Frame>
    ...
    <Frame>
      <img src="/images/docs/cloud/networking/load-balancers/add-certificates-to-load-balancer/listeners-tab.png" alt="Listener tab in the Load Balancer settings" width="70%"/>
    </Frame>
...
<p>When configuring basic authentication, an encrypted password can be specified instead of a plain-text password.</p>

<p>Any preferred encryption method can be used. To generate a hashed password with the [MKPasswd](https://www.unix.com/man-page/linux/1/mkpasswd/) utility (dockerized version):</p>
...
2\. Select an encryption method and generate the password hash. For example, to create a bcrypt hash, run:
...
<Frame>
  <img src="/images/docs/cloud/networking/load-balancers/add-certificates-to-load-balancer/encrypted-password-field.png" alt="Encrypted password in listener settings" width="70%"/>
</Frame>
