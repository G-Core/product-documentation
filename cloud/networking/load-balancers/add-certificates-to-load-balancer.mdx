...
The [secrets API](/api-reference/cloud/secrets/create-secret) confirms creation with `status: ACTIVE` and `secret_type: certificate`.
...
The [secrets API](/api-reference/cloud/secrets/list-secrets) returns all secrets regardless of type; filter by `secret_type: certificate` for TLS certificates.
...
The [create listener](/api-reference/cloud/load-balancers/create-load-balancer-listener) endpoint also supports the `PROMETHEUS` protocol for TLS-secured metrics endpoints.
...
To add more SNI certificates later, include all existing IDs plus the new ones in `sni_secret_id` - the field replaces the full list, not appends to it. The [update listener](/api-reference/cloud/load-balancers/update-load-balancer-listener) endpoint also accepts `secret_id` to replace the default certificate.
