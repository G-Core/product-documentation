...
ai-navigation: Configure multiple TLS certificates on Load Balancer listeners using SNI to serve different domains on a single IP address via Terminated HTTPS or Prometheus
ai-navigation: Attach multiple TLS certificates to Load Balancer listeners using Secrets Manager and SNI for domain-based certificate selection.
---
