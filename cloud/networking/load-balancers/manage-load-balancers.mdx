...
The scripts below list all load balancers, fetch details for one, and rename it.
...
<Tabs>
  <Tab title="Python SDK">
    ...
    # Step 1. List load balancers
    page = client.cloud.load_balancers.list()
    for lb in page.results:
        print(f"  {lb.id}  {lb.name}  {lb.operating_status}  {lb.vip_address}")

    # Step 2. Get load balancer details
    lb = client.cloud.load_balancers.get(load_balancer_id=lb_id)
    print(f"flavor: {lb.flavor.flavor_name}  provisioning: {lb.provisioning_status}")
    print(f"listeners: {len(lb.listeners)}  tags: {lb.tags_v2}")

    # Step 3. Rename the load balancer and update tags
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        // Step 2. Get load balancer details
        lb, err := client.Cloud.LoadBalancers.Get(ctx, lbID, cloud.LoadBalancerGetParams{})
        if err != nil {
            log.Fatal(err)
        }
        fmt.Printf("flavor: %s  provisioning: %s\n", lb.Flavor.FlavorName, lb.ProvisioningStatus)
        fmt.Printf("listeners: %d\n", len(lb.Listeners))

        // Step 3. Rename the load balancer and update tags
        tagVal := func(s string) *string { return &s }
        updated, err := client.Cloud.LoadBalancers.Update(ctx, lbID, cloud.LoadBalancerUpdateParams{
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl "https://api.gcore.com/cloud/v1/loadbalancers/$PROJECT_ID/$REGION_ID" \
    ...
    ```json
    {
      ...
      "results": [
        {
          "id": "ae7def79-f507-4306-93be-7784edad9211",   // save as LB_ID
...
### Step 3. Rename a load balancer and update tags
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl -X PATCH "https://api.gcore.com/cloud/v2/loadbalancers/$PROJECT_ID/$REGION_ID/$LB_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY" \
    ...
    The API returns a task ID — poll <code>GET&nbsp;/cloud/v1/tasks/{task_id}</code> every 5 seconds until `state` is `FINISHED`:
...
<Tabs>
  <Tab title="Python SDK">
    ...
    task_id = t.tasks[0]
    print(f"failover task: {task_id}")

    client.cloud.tasks.poll(task_id)
  ...
  <Tab title="Go SDK">
    ...
    taskID := t.Tasks[0]
    fmt.Printf("failover task: %s\n", taskID)

    if err = client.Cloud.Tasks.Poll(ctx, taskID); err != nil {
        log.Fatal(err)
    }
  ...
  <Tab title="curl">
    ```bash
    curl -X POST "https://api.gcore.com/cloud/v1/loadbalancers/$PROJECT_ID/$REGION_ID/$LB_ID/failover" \
      -H "Authorization: APIKey $GCORE_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{}'
    ```

    The API returns a task ID — poll <code>GET&nbsp;/cloud/v1/tasks/{task_id}</code> every 5 seconds until `state` is `FINISHED`. Failover takes 1-2 minutes:
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl -X DELETE "https://api.gcore.com/cloud/v1/loadbalancers/$PROJECT_ID/$REGION_ID/$LB_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
