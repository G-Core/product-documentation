...
ai-navigation: List, inspect, rename, update tags, initiate failover, delete Load Balancers, and manage pool members via the Customer Portal, REST API, or Terraform.
---
...
<p>Listing returns all members in a pool with their addresses, ports, weights, and health statuses. Use `limit` and `offset` query parameters to paginate large member lists.</p>

<Tabs>
  <Tab title="curl">
    ```bash
    curl "https://api.gcore.com/cloud/v1/loadbalancers/$GCORE_CLOUD_PROJECT_ID/$GCORE_CLOUD_REGION_ID/pools/$POOL_ID/members" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
    ...
    <p>To retrieve a specific page, append `?limit=50&offset=0` to the URL.</p>
  </Tab>
</Tabs>

## Get a pool member

<p>Fetching a single member returns its full configuration, including weight, backup status, and optional health check overrides.</p>

<Tabs>
  <Tab title="curl">
    ```bash
    curl "https://api.gcore.com/cloud/v1/loadbalancers/$GCORE_CLOUD_PROJECT_ID/$GCORE_CLOUD_REGION_ID/pools/$POOL_ID/members/$MEMBER_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
...
<p>A PATCH request updates a member's traffic distribution settings in place, without removing and re-adding it. Any combination of the fields below is accepted; omitted fields remain unchanged.</p>

| Parameter | Required | Description |
|-----------|----------|-------------|
| `weight` | No | Relative weight for traffic distribution, range 1–256 |
| `backup` | No | `true` marks the member as a backup — traffic reaches it only when all primary members are unreachable |
| `admin_state_up` | No | `false` drains the member without removing it from the pool |
| `monitor_address` | No | Override IP for health checks when it differs from the backend address |
| `monitor_port` | No | Override port for health checks when it differs from `protocol_port` |

<Tabs>
  <Tab title="curl">
    ```bash
    curl -X PATCH "https://api.gcore.com/cloud/v1/loadbalancers/$GCORE_CLOUD_PROJECT_ID/$GCORE_CLOUD_REGION_ID/pools/$POOL_ID/members/$MEMBER_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"weight": 3, "backup": false}'
    ```
