...
A pool is the list of VMs or Bare Metal to which the listener will redirect incoming traffic. Each pool in the listener has the following configuration:
  ...
  * **Proxy Protocol** : When enabled, the Load Balancer prepends a Proxy Protocol header (v1 or v2) to each connection so that backend servers can read the original client IP address. This setting can be configured during pool creation or updated after the Load Balancer is created.
