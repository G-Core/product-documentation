...
Annotations add custom metadata to Kubernetes objects to configure behavior beyond the standard settings. For Gcore Load Balancers in [Managed Kubernetes clusters](/cloud/kubernetes/clusters/create-a-kubernetes-cluster), annotations control type, flavor, IP addressing, listener limits, pool routing, logging, and DDoS protection. For a general introduction, see the [official Kubernetes documentation](https://kubernetes.io/docs/concepts/overview/working-with-objects/annotations/).
