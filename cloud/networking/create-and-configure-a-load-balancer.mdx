...
- **Standard flavors** — general-purpose configurations suitable for most workloads, with network throughput up to 1.5 Gbps.
- **High-performance flavors (lb2)** — built on network-optimised infrastructure with SR-IOV on the VIP port. These deliver significantly higher packet rates and lower latency, with guaranteed bandwidth of 5 Gbps or 10 Gbps per instance.

| **Flavor** | **vCPU** | **RAM (GB)** | **Network bandwidth (VIP)** |
| --- | --- | --- | --- |
...
Use lb2 flavors for services that handle large volumes of concurrent connections or high-bandwidth traffic, such as media streaming, CDN origin servers, and API gateways.
