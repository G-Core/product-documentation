...
A single pool can be used by multiple listeners within the same Load Balancer. This is useful when you want to route traffic from different ports to the same set of backend servers, reducing configuration overhead and simplifying pool member management.

<Info>
  **Info**

  This feature is available via API only. The Gcore Customer Portal does not support creating listeners with shared pools.
</Info>

To configure a shared pool via API:

1. Create an empty Load Balancer.
2. Create a pool specifying only `loadbalancer_id` (without `listener_id`).
3. Create multiple listeners, each referencing the pool via the `default_pool_id` parameter.

For example, you can create one pool and attach it to listeners on ports 80 and 8080, so both listeners direct traffic to the same backend servers.
...
For API details, refer to the [Load Balancer API documentation](https://apidocs.gcore.com/cloud#tag/Load-Balancers).
