...
<p>The Gcore Terraform provider v2 manages the full load balancer lifecycle using three resources: `gcore_cloud_load_balancer`, `gcore_cloud_load_balancer_listener`, and `gcore_cloud_load_balancer_pool`. Pool members are declared inline in the pool resource using the `members` attribute.</p>
...
<p>For HTTP and TCP listeners, use the `gcore_cloud_load_balancer_listener` resource directly. For `TERMINATED_HTTPS`, the Terraform provider v2 does not support attaching a TLS certificate. Create the HTTPS listener via the REST API after `terraform apply`:</p>
