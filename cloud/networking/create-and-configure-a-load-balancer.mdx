...
<p>For HTTP and TCP listeners, use `gcore_cloud_load_balancer_listener` directly. For `TERMINATED_HTTPS`, create the listener via the REST API after `terraform apply` to include `secret_id` — the Terraform provider v2 handles HTTP and TCP natively:</p>
