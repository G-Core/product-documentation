...
<p>Provision a load balancer with listeners and backend pools using the [Gcore Terraform provider v2](/developer-tools/terraform/overview). A permanent [API token](/account-settings/api-tokens), [project ID](https://gcore.com/docs/api-reference/cloud/projects/list-projects), and [region ID](https://gcore.com/docs/api-reference/cloud/regions/list-regions) are required.</p>

## Quickstart

<p>The configuration below provisions a load balancer with an HTTP listener and a round-robin pool with two backend members.</p>
...
<p>The sections below cover each resource's key attributes and options.</p>
...
<p>For HTTP and TCP listeners, use `gcore_cloud_load_balancer_listener` directly. For `TERMINATED_HTTPS`, attach the certificate via the REST API after `terraform apply` — the Terraform provider v2 does not support the `secret_id` field on listeners:</p>
...
<p>Set `LB_ID` to the value of `gcore_cloud_load_balancer.example.id` from Terraform output, and `SECRET_ID` to the value of `gcore_cloud_secret.tls.id`.</p>
