...
A Load Balancer distributes incoming requests across Virtual Machines to improve infrastructure fault tolerance.

## Step 1. Initiate Load Balancer creation

In the project, navigate to **Load Balancers** in the **Networking** section and click **Create Load Balancer**.
...
- **Network-Optimized** - built on network-optimised infrastructure with SR-IOV on the VIP port. These deliver higher packet rates and lower latency, with guaranteed bandwidth up to 10 Gbps per instance.
...
L2 (Layer 2) and L3 (Layer 3) connectivity options are available. This setting determines the preferred connectivity method the Load Balancer uses to connect to backend pool members. If the preferred connectivity is not feasible, traffic will automatically route via the alternative method:
...
<Tip>
  For most cases, such as a single subnet setup, use **L2 connectivity** for optimal performance. For deployments with multiple subnets or complex networking requirements, contact support to determine the best configuration.
</Tip>
...
When enabled, the Logging service stores logs. To learn how it works and how to configure it, refer to the article about [Logging](/cloud/logging-as-a-service/about-logging-as-a-service).
...
Create tags for the load balancer by entering "Key" and "Value".
...
A [load balancer](/api-reference/cloud/load-balancers/create-load-balancer) distributes incoming requests across backend VMs, routing traffic only to healthy members. Creating one requires four API calls: create the balancer, add a listener, create a pool, and register the backend VMs as pool members.
...
A [flavor](/api-reference/cloud/load-balancers/list-load-balancer-flavors) determines the vCPU count, RAM, and maximum network throughput of the load balancer instance. Flavor availability is region-specific — run this call to see what's available in the target region.
...
A [listener](/api-reference/cloud/load-balancers/create-load-balancer-listener) watches for incoming connections on a specific protocol and port, then forwards them to a pool.
...
A [pool](/api-reference/cloud/load-balancers/create-load-balancer-pool) groups the backend VMs and distributes requests across them according to the chosen algorithm.
...
Each [pool member](/api-reference/cloud/load-balancers/create-load-balancer-pool-member) is a VM identified by its IP address and the port where the application listens. Repeat this call for each VM.
...
Members show `operating_status: NO_MONITOR` when no health monitor is configured — this is expected. The load balancer still distributes traffic. To enable automatic member removal when a backend becomes unhealthy, add a [health monitor](/api-reference/cloud/load-balancers/create-load-balancer-pool-health-monitor) to the pool.

### Step 6. Verify the load balancer

After adding both members, confirm the load balancer is active and traffic is flowing. The [load balancers](/api-reference/cloud/load-balancers) endpoint also supports updating listeners, resizing the flavor, enabling logging, and configuring DDoS protection.
