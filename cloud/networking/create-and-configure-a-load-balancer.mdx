...
3. Select a pool protocol. The protocol defines how the Load Balancer communicates with backend servers:

- **HTTP** / **HTTPS** — for HTTP-based listeners.
- **TCP** — forwards raw TCP traffic without modification.
- **PROXY** — enables Proxy Protocol v1 (text-based). The Load Balancer prepends a Proxy Protocol header to each connection so that backend servers can read the original client IP address instead of the Load Balancer's IP.
- **PROXY V2** — enables Proxy Protocol v2 (binary format). Use this if your backend application is configured to parse the binary header format.

   <Info>
     **Info**

     To use PROXY or PROXY V2, the listener must use the TCP protocol. Your backend application must be explicitly configured to parse Proxy Protocol headers. Using PROXY or PROXY V2 on a pool whose members do not expect these headers will cause connection failures.
   </Info>
