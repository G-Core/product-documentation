...
<p>The Gcore Terraform provider v2 manages the full load balancer lifecycle using three resources: `gcore_cloud_load_balancer`, `gcore_cloud_load_balancer_listener`, and `gcore_cloud_load_balancer_pool`. Pool members are declared inline in the pool resource.</p>

<Warning>
The separate `gcore_cloud_load_balancer_pool_member` resource does not work in provider v2 due to a provider bug. Always define pool members using the `members` attribute inside `gcore_cloud_load_balancer_pool`.
</Warning>
...
<p>Each resource below explains the key parameters and available options.</p>
...
| `flavor` | No | Compute flavor. Omit to use the platform default. |
...
| `secret_id` | No | TLS certificate secret ID. Required for `TERMINATED_HTTPS`. |

<p>Example HTTPS listener using a TLS certificate stored in Secrets Manager:</p>
...
resource "gcore_cloud_load_balancer_listener" "https" {
  ...
  protocol_port    = 443
  secret_id        = var.tls_secret_id  # from gcore_cloud_secret resource
}
```

<p>To store and manage TLS certificates, see [Configure secrets for HTTPS load balancer](/cloud/secrets-manager/upload-a-pkcs12-file).</p>
...
<p>Terraform removes the pool, listener, and load balancer in the correct order automatically.</p>
