...
4. (Optional) Enable **Proxy Protocol** to forward the original client IP address and connection details to backend servers. When enabled, the Load Balancer prepends a Proxy Protocol header to each connection, allowing backend servers to read the real client IP instead of seeing the Load Balancer's IP.

   Gcore Load Balancers support Proxy Protocol **v1** (text-based) and **v2** (binary). Select the version that matches your backend application's configuration.

   <Info>
     **Info**

     Proxy Protocol is supported for TCP listeners only. Your backend application must be configured to parse Proxy Protocol headers. Enabling this option on a pool whose members do not expect Proxy Protocol headers will cause connection failures.
   </Info>

5. Select **App Cookie** and fill in the "Cookie" field. [A special module](https://code.google.com/p/nginx-sticky-module/) creates a cookie and then uses it to forward requests to the same server.
