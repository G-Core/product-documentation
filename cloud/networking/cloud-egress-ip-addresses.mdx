...
ai-navigation: Retrieve and allowlist Gcore Cloud egress IP ranges (IPv4/IPv6 CIDR) via public API for webhooks, AMQP, asset delivery, and firewall rules.
ai-navigation: Retrieve Cloud egress IP ranges in CIDR notation via public API for firewall, webhook, AMQP allowlisting.
---
...
| **AMQP messaging** | Cloud connects to your AMQP message brokers when you [subscribe to user-action logs via AMQP](/api-reference/cloud#tag/User-Actions). Allowlist these ranges to receive event messages. |
