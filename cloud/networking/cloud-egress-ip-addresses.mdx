...
Gcore Cloud services use specific IP addresses for outbound (egress) traffic. The API returns both IPv4 and IPv6 address ranges in CIDR notation. If the infrastructure restricts incoming connections by source IP, add these ranges to the allowlist.
...
<Warning>
If a new IP range is not added to the allowlist, Cloud instances using that range cannot reach the target systems. Run the update script on a schedule (for example, hourly via cron).
</Warning>
