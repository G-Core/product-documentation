...
##### **Enable DHCP for automatic configuration (recommended)**

<Info>
  Enabling DHCP is the recommended method for configuring VAST network interfaces. It automatically provides IP address, CIDR, gateway, and routing information, eliminating the need for manual configuration.
</Info>

To enable DHCP for your VAST subnet, configure the following parameter:

- `enable_dhcp` (boolean, optional): Enables automatic IP address and network configuration via DHCP. Set to `true` to use DHCP.
  Default: `false`.

Once DHCP is enabled, configure the interface using a DHCP client:

**For VMs and GPU clusters:**

1. Run the DHCP client on the interface:

   ```bash
   sudo dhcpcd enp8s0
   ```

**For Bare Metal servers with Arista switches (Trunks):**

1. Create the VLAN interface on the bond:
...
Replace `enp8s0` or `bond0.100` with your actual interface name, and replace `100` with the VLAN ID shown in the **Networking** tab. The DHCP client will automatically obtain the IP address, netmask, gateway, and routing information.

<Note>
  If you prefer manual configuration or DHCP is not available, follow the steps below.
</Note>
