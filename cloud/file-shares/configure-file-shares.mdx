...
<p>Provision an NFS file share and its per-CIDR access rules using the [`gcore_cloud_file_share`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_file_share) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview). The file share and any Virtual Machine that mounts it must be on the same private subnet — pass the network and subnet IDs as input variables.</p>
