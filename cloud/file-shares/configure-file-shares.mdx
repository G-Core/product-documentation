...
DHCP is enabled by default on VAST networks. Use a DHCP client to automatically obtain the IP address, CIDR, gateway, and routing configuration. This eliminates the need for manual network configuration, which can be complex as routing information is not displayed in the control panel.
...
<Info>
  Replace `<interface_name>` with the name identified in the previous step (e.g., `enp8s0` or `enp4s0`).
</Info>
