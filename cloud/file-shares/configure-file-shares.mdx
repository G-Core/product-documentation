...
##### **DHCP-based configuration (recommended)**

<Info>
  DHCP is the recommended method for configuring VAST network interfaces. It automatically handles IP address assignment, CIDR configuration, and routing, eliminating the need for manual network setup.
</Info>

For VMs and GPU clusters, use the following commands:

bash
sudo dhcpcd <interface_name>


Replace `<interface_name>` with the interface name identified in Step 3 (such as `enp8s0` or `enp4s0`).

For Bare Metal servers with Arista switches (Trunks), create the VLAN interface first, then use DHCP:

bash
sudo ip link add link bond0 name bond0.<vlan_id> type vlan id <vlan_id>
sudo dhcpcd bond0.<vlan_id>


Replace `<vlan_id>` with the VLAN ID shown in the **Networking** tab of the server details page.

<Tip>
  The DHCP client will automatically obtain the IP address, subnet mask, gateway, and routing configuration. No manual IP or route configuration is required.
</Tip>

##### **Manual configuration (alternative)**

If DHCP is not available or you prefer manual configuration, follow the steps below.
