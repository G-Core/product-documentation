...
##### Configure VAST interface using DHCP (recommended)

<Info>
  Using DHCP eliminates the need for manual configuration, which required network information (such as routing details) that was not visible in the interface.
</Info>

DHCP is now enabled on VAST subnets. This is the recommended method as it automatically provides the IP address, CIDR, gateway, and routing configuration to the instance.

###### For VMs and GPU clusters

Run the DHCP client on the VAST interface identified in the previous step. Replace `enp8s0` with your actual interface name.

```bash
sudo dhcpcd enp8s0
```

###### For Bare Metal servers (Arista/Trunks)

For Bare Metal servers using bond interfaces, you must first create the VLAN interface on the bond, then run the DHCP client. Replace `100` with your VLAN ID.

```bash
sudo ip link add link bond0 name bond0.100 type vlan id 100
sudo dhcpcd bond0.100
```

<Warning>
  Manual configuration is only required if DHCP is not available or not working. This method requires specific network information that may not be visible in the interface.
</Warning>

If you must configure the interface manually, follow the steps below. Note that manual configuration requires the IP address, CIDR range, and gateway information from the previous step.
