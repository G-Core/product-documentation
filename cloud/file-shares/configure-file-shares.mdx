...
<Info>
  This is the recommended method for configuring VAST interfaces on existing resources. DHCP automatically handles IP assignment, routing, and network configuration, eliminating the need for manual setup.
</Info>

Before attaching the VAST interface, ensure DHCP is enabled on the VAST subnet. Set `enable_dhcp` to `true` for the VAST subnet.

After attaching the VAST network interface to your compute resource, connect to the server via the [Gcore Customer Portal](/cloud/virtual-instances/connect/connect-to-your-instance-via-control-panel) or SSH and configure the interface using DHCP.
...
<AccordionGroup>
  <Accordion title="Bluefield DPU">
    In the commands below, replace `ens10f0v0` with the interface name identified above.
  ...
  <Accordion title="Arista (Trunks)">
    ...
    In the commands below, replace `100` with the VLAN ID shown in the **Networking** tab of the server details page.
...
#### Configure attached VAST network interface (manual)
