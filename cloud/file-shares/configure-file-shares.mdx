...
<p>Provision a Standard NFS file share and its access rules using the [Gcore Terraform provider](/developer-tools/terraform/overview). Pass an existing network and subnet as variables — the file share and any VM that mounts it must share the same private subnet.</p>
...
<p>Edit the `size` field and run `terraform apply` — size increases are applied in place. Standard shares must be unmounted before resizing; downgrading the size is not supported.</p>
