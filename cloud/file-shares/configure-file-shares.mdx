...
##### Configure using DHCP (recommended)

<Info>
  DHCP is enabled on VAST subnets by Gcore infrastructure. This automatically provides the IP address, CIDR range, gateway, and routing configuration, eliminating the need for manual network configuration.
</Info>

###### For VMs and GPU clusters

Run the DHCP client on the VAST interface to obtain all network configuration automatically.

```bash
sudo dhcpcd enp8s0
```

Replace `enp8s0` with the actual interface name identified in the previous step.

###### For Bare Metal servers

Bare Metal servers require creating a VLAN interface before running the DHCP client.
