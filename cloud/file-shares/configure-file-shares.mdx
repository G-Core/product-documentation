...
<Info>
  The `enable_dhcp` parameter is set to `true` by default on VAST subnets. This automatically provides the IP address, CIDR, gateway, and routing configuration, eliminating the need for manual network configuration which can be complex as routing information is not displayed in the control panel.
</Info>
...
Replace `<interface_name>` with the actual interface name (such as `enp8s0` or `bond0.100`).
