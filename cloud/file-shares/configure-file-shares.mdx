...
Gcore has enabled DHCP on VAST subnets to simplify network configuration. Use automatic DHCP configuration (recommended) or configure the interface manually.

##### Automatic configuration with DHCP (recommended)

<Info>
  DHCP automatically provides IP address, subnet mask, gateway, and routing configuration. This eliminates the need to manually determine network parameters that were previously unavailable.
</Info>

For Bare Metal servers with Arista switches (Trunks), first create the VLAN interface, then run the DHCP client on that interface. The VLAN interface must exist before DHCP can configure it. Replace `100` with the VLAN ID shown in the **Networking** tab of the server details page:

```bash
# Create VLAN interface on bond
sudo ip link add link bond0 name bond0.100 type vlan id 100

# Get automatic configuration via DHCP
sudo dhcpcd bond0.100
```

For VMs, GPU clusters, and Bare Metal servers with Bluefield DPU, DHCP configuration is typically handled automatically when the interface is attached during resource creation.

##### Manual configuration

If automatic DHCP configuration doesn't work for the setup, configure the interface manually as described in the following sections.
