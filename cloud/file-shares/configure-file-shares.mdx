...
While the VAST interface can be attached to an already-provisioned GPU cluster or compute resource, this order is generally **not recommended** due to its greater complexity.

1. Go to the **Resource** settings ([VM](/cloud/virtual-instances/create-an-instance), [Bare Metal](/cloud/bare-metal-servers/create-a-bare-metal-server), or [GPU cluster](/edge-ai/ai-infrastructure/create-an-ai-cluster)).
...
6. Note the assigned **IP** address and **MAC** identifier for use in the following step.

### Step 3: Configure attached VAST network interface
...
2. Identify the VAST interface by matching the **link/ether** MAC identifier to the one from the prevous step.
3. Note the interface name, such as `enp8s0` or `enp4s0`, for use in subsequebt commands.
4. Set interface parameters, using the correct interface name from above:

   `sudo ip link set enpXs0 mtu 9000`

   `sudo ip link set enpXs0 up` 
4. Assign an IP adress to the interface, using the IP adress from Step 2 and the interface name from above:

   `sudo ip addr add 172.18.XXX.XXX/22 dev enpXs0`
4. Add a static route for this interface, using....

   `sudo ip route add 172.19.252.0/22 via 172.18.XXX.XXX dev enpXs0`
