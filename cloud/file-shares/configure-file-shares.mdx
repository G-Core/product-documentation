...
<Info>
  DHCP is enabled on VAST subnets. We recommend using DHCP to automatically obtain the IP address, CIDR, gateway, and routing configuration, eliminating the need for manual network configuration.
</Info>

###### **For VMs and GPU clusters**

To configure the interface automatically, run the DHCP client on the identified interface:

```bash
sudo dhcpcd <interface_name>
```

Replace `<interface_name>` with the actual interface name (e.g., `enp8s0`).

###### **For Bare Metal servers (Arista/Trunks)**

For Bare Metal servers using Arista switches, you must first create the VLAN interface on the bond, then run the DHCP client:

```bash
# Create the VLAN interface (replace 100 with your VLAN ID)
sudo ip link add link bond0 name bond0.100 type vlan id 100

# Activate the interface
sudo ip link set bond0.100 up

# Obtain configuration via DHCP
sudo dhcpcd bond0.100
```

Replace `100` with the VLAN ID shown in the **Networking** tab of the server details page.

##### **Configuring VAST interface for VMs and GPU clusters (manual)**
...
##### **Configuring VAST interface for Bare Metal servers (manual)**
