...
<AccordionGroup>
  ...
  <Accordion title="Arista (Trunks)">
    For Bare Metal servers with Arista switches (Trunks), two methods configure the VAST network interface: automatic configuration with DHCP (recommended) and manual configuration.

    #### Automatic configuration with DHCP (recommended)

    <Info>

    DHCP is now enabled on VAST subnets and automatically provides IP address, CIDR, gateway, and routing information.

    </Info>

    With DHCP enabled, first create the VLAN interface, then run the DHCP client on that interface to obtain the full network configuration:
    ...
    The VLAN interface must exist before the DHCP client can configure it with network settings.

    Replace `2998` with the actual VLAN ID and `bond0` with the network interface name.

    After running these commands, the DHCP client automatically configures the interface with the IP address, subnet mask, and routing information provided by the VAST subnet.

    #### Manual configuration (alternative method)

    Manual configuration was the only option before DHCP was enabled on VAST subnets. It requires specific IP addresses, CIDR blocks, and gateway information that is not available through standard network discovery tools like `host` or `dig`. Previously, clients needed to contact support to obtain routing details, making network setup time-consuming and error-prone.

    <Warning>

    Manual configuration requires specific IP addresses, CIDR blocks, and gateway information that may not be readily available through standard network discovery tools.

    </Warning>

    A bond (link aggregation) combines multiple physical network interfaces into a single logical interface, providing higher throughput and redundancy.
