...
##### **Automatic configuration using DHCP (recommended)**

DHCP is enabled on VAST subnets. Use DHCP to automatically configure the network interface, as it provides the IP address, CIDR, gateway, and routing configuration without manual setup.

###### **For VMs and GPU clusters**

Run the DHCP client on the identified interface. Replace `enp8s0` with your interface name.
...
<AccordionGroup>
  <Accordion title="Bluefield DPU">
    Run the DHCP client on the identified interface. Replace `ens10f0v0` with your interface name.
  ...
  <Accordion title="Arista (Trunks)">
    For Arista switches, you must create the VLAN interface manually, but can use DHCP for the IP and routing configuration.

    1. Create the VLAN interface on the bond. Replace `100` with the VLAN ID from the **Networking** tab.

    ```bash
    sudo ip link add link bond0 name bond0.100 type vlan id 100
    ```

    2. Run the DHCP client on the new VLAN interface.
...
##### **Manual configuration for VMs and GPU clusters**
...
##### **Manual configuration for Bare Metal servers**
