...
<p>A Standard NFS file share requires a private network to connect to Virtual Machines or Bare Metal servers.</p>

<p>Network type requirements differ by resource:</p>

- **Virtual Machines** — a regular private network (VXLAN-based) is sufficient. Create one in **Cloud** > **Networking** > **Networks**.
- **Bare Metal servers** — the network must be created with the **Bare Metal Network** toggle enabled, which provisions a dedicated VLAN. If needed, create a new network and enable the **Bare Metal Network** toggle during configuration.
...
<Info>
  After attaching a Bare Metal server to a new network, manually update the OS network settings to activate the new interface.
</Info>

## Configure Standard NFS file shares for Linux VMs and Bare Metal
...
<p>VAST shares can be resized while mounted. Standard shares must be unmounted before resizing:</p>

```bash
umount -lf /mount/path
```

<p>After the resize, remount the Standard share as described above.</p>

### Resize file share

<p>In the Customer Portal, the resize steps are the same for both share types:</p>
