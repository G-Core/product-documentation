...
6. Add static route to the VAST network:
   - Replace `198.51.100.1` with the IP adress from Step 2, modifying the last octet to `1` for network gateway, and use the same interface name.
...
`sudo ./build.sh bin --dkms`

`sudo dpkg -i /dist/vastnfs-dkms_4.0.36-vastdata_all.deb`

chmod +x vast_download.sh
