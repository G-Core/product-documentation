...
Once the VAST file share is created, you can add it directly when [creating a GPU cluster](/edge-ai/ai-infrastructure/create-an-ai-cluster) (recommended), or attach the VAST network interface manually to an existing compute resource.
...
<Tabs>
  <Tab title="VM / GPU VM">
    ...
    - Replace `198.51.100.25` with the IP address from Step 2.
    ...
    - Replace `198.51.100.1` with an address formed by the first three octets from the CIDR range in Step 2, keeping `1` as the last octet for the gateway IP, and using the same interface name.
  ...
  <Tab title="Bare Metal with Bluefield DPU">
    Using commands below:
    - Replace `198.51.100.25` with the IP address from Step 2.
    - Replace `ens10f0v0` with the interface name from above, if different.
  ...
  <Tab title="Bare Metal with Arista (Trunks)">
    For Bare Metal servers with Arista switches (Trunks), create a VLAN interface on the bond.

    Using commands below:
    - Replace `2998` with the VLAN ID from the UI or API.
    - Replace `198.51.100.25` with the IP address from Step 2.

    Create VLAN interface on bond:
    ...
    ```bash
    sudo ip addr add 198.51.100.25/22 dev bond0.2998
    ```

    Add static route to the VAST network:
    - Replace `198.51.100.1` with an address formed by the first three octets from the CIDR range in Step 2, keeping `1` as the last octet for the gateway IP.

    ```bash
    sudo ip route add 172.19.252.0/22 via 198.51.100.1 dev bond0.2998
    ```
  </Tab>
</Tabs>
