...
##### Use DHCP for automatic configuration (recommended)

<Info>
  DHCP automatically assigns the IP address, CIDR, gateway, and routing, eliminating the need for manual static route configuration.
</Info>

###### For VMs and GPU clusters

Run the DHCP client on the identified interface to obtain the full network configuration:
...
For Bare Metal servers using Arista switches (Trunks), create the VLAN interface on the bond, then run the DHCP client:
...
<Note>
  DHCP is the recommended method for configuring VAST interfaces. Use the DHCP configuration above for automatic setup. Manual configuration below is only required for specific use cases.
</Note>
