...
<p>The [create secret](/api-reference/cloud/secrets/create-secret) endpoint accepts PEM-formatted certificate data directly — no PKCS12 packaging or base64 encoding required.</p>
...
<p>The [list secrets](/api-reference/cloud/secrets/list-secrets) endpoint returns all secrets in the region — useful for finding the ID of an existing secret before attaching it to a load balancer.</p>
...
<p>The [get secret](/api-reference/cloud/secrets/get-secret) endpoint returns full metadata for a single secret by ID, including its status, type, and expiration date.</p>
