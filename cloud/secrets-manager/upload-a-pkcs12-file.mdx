...
<p>The [create secret](https://api.gcore.com/docs/cloud#tag/Secrets/operation/SecretsViewSetV2.post) endpoint accepts PEM-formatted certificate data directly — no PKCS12 packaging or base64 encoding required.</p>
...
<p>The [list secrets](https://api.gcore.com/docs/cloud#tag/Secrets/operation/SecretsViewSet.get) endpoint returns all secrets in the region — useful for finding the ID of an existing secret before attaching it to a load balancer.</p>
...
<p>The [get secret](https://api.gcore.com/docs/cloud#tag/Secrets/operation/SecretViewSet.get) endpoint returns full metadata for a single secret by ID, including its status, type, and expiration date.</p>
...
<p>Declare TLS certificate secrets from PEM files on disk using the [Gcore Terraform provider v2](/developer-tools/terraform/overview). A permanent [API token](/account-settings/api-tokens), [project ID](https://gcore.com/docs/api-reference/cloud/projects/list-projects), and [region ID](https://gcore.com/docs/api-reference/cloud/regions/list-regions) are required. The certificate data is write-only — Terraform sends it to the API but never stores it in state.</p>
...
<p>After running `terraform apply`, attach the secret to a `TERMINATED_HTTPS` listener via the REST API — use the [REST API tab](#api) for that step, or run the following command with `SECRET_ID` from the Terraform output:</p>

```bash
export SECRET_ID=$(terraform output -raw secret_id)
