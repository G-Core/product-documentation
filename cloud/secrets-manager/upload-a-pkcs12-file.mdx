...
<p>Store TLS certificates as secrets using the [`gcore_cloud_secret`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_secret) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview). Secrets are immutable once created — rotate a certificate by declaring a new secret version and updating the listener reference.</p>
...
<p>For the full workflow including listener creation and SNI configuration, see [Add TLS certificates to a load balancer](/cloud/networking/load-balancers/add-certificates-to-load-balancer).</p>
