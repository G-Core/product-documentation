...
<p>The Terraform provider v2 does not support attaching a secret to a listener resource. After running `terraform apply` to create the secret, attach it to a `TERMINATED_HTTPS` listener using the REST API:</p>
...
<p>For a full load balancer setup, see [Create and configure a load balancer](/cloud/networking/create-and-configure-a-load-balancer).</p>
