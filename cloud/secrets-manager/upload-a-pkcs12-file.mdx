...
<p>To upload a secret as a PKCS12 file:</p>
  ...
  1. In the [Gcore Customer Portal](https://portal.gcore.com), navigate to **Cloud**.
...
<Frame>![Enable Upload as PKCS12 file](/images/docs/cloud/secrets-manager/upload-a-pkcs12-file/upload-as-pkcs12-file.png)</Frame>
  ...
  6. Paste the base64-encoded PKCS12 content into the **Secret content** field.
...
<p>Three files are needed: the SSL/TLS certificate for the domain, the certificate chain, and the private key. The domain must point to the virtual IP address of the Load Balancer via a DNS A record. When using Gcore DNS, create the A record following the [instructions](/dns/dns-records/manage-dns-records-advanced-interface-mode-with-balancing).</p>
...
<p>A new base64-encoded file is created. Open it in a text editor and delete the first and last lines — `----- BEGIN CERTIFICATE -----` and `----- END CERTIFICATE -----`. These header and footer lines are added automatically by certutil and must be removed; the remaining text is the Secret content.</p>
...
<p>For instructions on configuring a Load Balancer with an HTTPS listener, see [adding TLS certificates to a Load Balancer](/cloud/networking/load-balancers/add-certificates-to-load-balancer).</p>
