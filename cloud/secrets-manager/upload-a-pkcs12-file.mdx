...
<p>Declare TLS certificate secrets from PEM files on disk using the [Gcore Terraform provider v2](/developer-tools/terraform/overview) — certificate data is write-only and not stored in state. A permanent [API token](/account-settings/api-tokens), [project ID](https://gcore.com/docs/api-reference/cloud/projects/list-projects), and [region ID](https://gcore.com/docs/api-reference/cloud/regions/list-regions) are required.</p>
...
<p>After running `terraform apply`, attach the secret to a `TERMINATED_HTTPS` listener via the [REST API](#api) tab, or run the following command with `SECRET_ID` from the Terraform output:</p>
