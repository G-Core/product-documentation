...
<p>Declare a custom image using the [Gcore Terraform provider v2](/developer-tools/terraform/overview). Gcore fetches the image from the URL and stores it in the selected region.</p>
...
<Warning>Any change to the resource block triggers a replace — Terraform deletes the existing image and re-uploads from the URL. There is no in-place update: rename, format change, and flag change all result in a new image ID.</Warning>
