...
sidebarTitle: Upload
ai-navigation: Upload images to Gcore Cloud storage via URL import, supporting raw, vhd, vhdx, vdi, ploop, qcow2, and ami formats with VirtIO drivers and cloud-init package for Virtual Instances, Bare Metal, Virtual GPU Clusters, and Baremetal GPU Clusters using x86 or ARM architecture selection and VM quick start toggle configuration.
---
...
client := gcore.NewClient(
    ...
    option.WithCloudProjectID(mustParseInt(os.Getenv("PROJECT_ID"))),
    option.WithCloudRegionID(mustParseInt(os.Getenv("REGION_ID"))),
)
...
```
  ...
  <Tab title="curl">
```bash
curl -X POST "https://api.gcore.com/cloud/v1/downloadimage/$PROJECT_ID/$REGION_ID" \
...
```
  ...
  <Tab title="curl">
```bash
curl "https://api.gcore.com/cloud/v1/images/$PROJECT_ID/$REGION_ID?visibility=shared" \
  -H "Authorization: APIKey $GCORE_API_KEY"
```
...
<Tabs>
  <Tab title="Python SDK">
```python
IMAGE_ID = "e418cec8-cf26-4146-aad8-82e3ca805d2e"
...
```
  ...
  <Tab title="Go SDK">
```go
imageID := "e418cec8-cf26-4146-aad8-82e3ca805d2e"
...
curl -X PATCH "https://api.gcore.com/cloud/v1/images/$PROJECT_ID/$REGION_ID/$IMAGE_ID" \
  -H "Authorization: APIKey $GCORE_API_KEY" \
  -H "Content-Type: application/json" \
...
curl -X DELETE "https://api.gcore.com/cloud/v1/images/$PROJECT_ID/$REGION_ID/$IMAGE_ID" \
  -H "Authorization: APIKey $GCORE_API_KEY"
```
