...
<p>Upload a custom OS image from a public URL using the [`gcore_cloud_instance_image`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_instance_image) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview) — Gcore fetches and stores the image in the selected region. Changing the image source recreates the resource and assigns a new image ID — update any references that depend on it.</p>
