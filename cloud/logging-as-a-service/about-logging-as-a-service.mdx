...
ai-navigation: Collect and store logs from Virtual Machines and Bare Metal servers via Kafka-compatible shippers, accessed through OpenSearch Dashboards for error detection
ai-navigation: Configure log shippers (Fluent Bit, Filebeat, Logstash) to export logs to Kafka; access via OpenSearch Dashboards.
---
...
## How managed logging works
  ...
  1. Install and configure any log shipper that supports Kafka. You can use [Fluent Bit](https://docs.fluentbit.io/manual/installation/getting-started-with-fluent-bit), [Filebeat](https://elastic.co/beats/filebeat), [Logstash](https://www.elastic.co/logstash), or any other log source that can export logs to Kafka, such as a Python application with configured Kafka logger output.
