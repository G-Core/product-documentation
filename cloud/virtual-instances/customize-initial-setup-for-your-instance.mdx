...
<Warning>
`user_data` runs on first boot only. Changing the value in an existing configuration does not re-run cloud-init on a running instance — it takes effect only when the instance is recreated.
</Warning>
...
<p>The full VM configuration with boot volume and SSH key is covered in [Create a Virtual Machine](/cloud/virtual-instances/create-an-instance). The [Cloud-init directives](#cloud-init-directives) section below lists all available YAML options.</p>
