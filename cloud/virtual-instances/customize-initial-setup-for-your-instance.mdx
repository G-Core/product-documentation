...
<p>Customize a Virtual Machine on its first boot with cloud-init using the [Gcore Terraform provider v2](/developer-tools/terraform/overview). Pass the YAML document through the `user_data` field on [`gcore_cloud_instance`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_instance) — Terraform's `base64encode()` function handles encoding automatically.</p>
