---
title: Customize the initial setup for a Virtual Machine
...
<p>When [creating](/cloud/virtual-instances/create-an-instance) a Virtual Machine, the **Additional options** section includes a **User data** field that passes a `cloud-init` YAML script to the VM on first boot. Use it to set passwords, create users, install packages, write files, and configure network interfaces on Linux-based Virtual Machines. Windows Virtual Machines do not support `cloud-init`.</p>

## Enable User data

<p>By default, the **User data** field is disabled for new Virtual Machines. Enable it by switching on the **User data** toggle. The portal pre-fills a sample password configuration template — edit the template or replace it with a custom cloud-init YAML document.</p>

<Frame>
  ![User data field enabled with default cloud-init template](/images/docs/cloud/virtual-instances/customize-initial-setup-for-your-instance/user-data-enabled.png)
</Frame>

</MethodSection>
<MethodSection id="api" label="REST API">

<p>The `user_data` field in the [instance creation](/api-reference/cloud#tag/Instances/operation/InstanceCreateSetV2.post) request passes a `cloud-init` YAML script to the VM on first boot. The value must be base64-encoded — the API rejects plain-text YAML.</p>
...
- **IMAGE\_ID**: the UUID of a public or private image. Use `GET /cloud/v1/images/{project_id}/{region_id}` to list available images; the [image upload](/cloud/images/upload-an-image-to-the-storage) article covers custom uploads.
...
| `flavor` | Yes | [Flavor](/api-reference/cloud#tag/Flavors/operation/FlavorListV2.get) ID; IDs are region-specific. |
...
<Tabs>
  ...
  <Tab title="curl">
    ...
    Poll `GET /cloud/v1/tasks/{task_id}` every 5 seconds until `state` is `FINISHED`, then read the instance ID from `created_resources.instances[0]`.
  </Tab>
</Tabs>
...
<p>The [Virtual Machine](/cloud/virtual-instances/create-an-instance) article covers the full configuration including boot volume and SSH key.</p>
...
#cloud-config
ssh_authorized_keys:
  - ssh-rsa <YOUR_PUBLIC_KEY> user@host
```
