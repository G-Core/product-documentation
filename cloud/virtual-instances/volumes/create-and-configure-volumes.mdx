...
<p>Declares a blank data volume. Set `source = "new-volume"` for empty storage, or `"snapshot"` to restore from an existing snapshot.</p>
...
<p>Increase `size` and run `terraform apply`. Volume size can only be increased — decreasing is not supported.</p>
