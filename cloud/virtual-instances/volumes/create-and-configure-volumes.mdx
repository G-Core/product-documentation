...
Volumes in Gcore Cloud are block storage devices that attach to Virtual Machines over the network. This guide covers creating a blank volume, attaching and detaching it from a VM, resizing it, and deleting it - all through the REST API.
...
Poll until `FINISHED`. The volume `status` field changes to `in-use` once attached.

## Detach from a Virtual Machine

Boot volumes cannot be detached from a running VM. Only regular (non-boot) volumes can be detached. Detaching does not delete the volume - it remains available for reattachment.
...
Poll until `FINISHED`. The volume `status` returns to `available`.

## Resize a volume

Volume size can only be increased, not decreased. The new size must be greater than the current size. Resizing does not require detaching - it can be done while the volume is attached.
...
Poll until `FINISHED`. After resizing, extend the filesystem inside the OS to use the new space (for example, `resize2fs` for ext4 or `xfs_growfs` for XFS).
...
Poll until `FINISHED`. Deleting a volume stops billing for that volume immediately.
