...
<Info>
SSD Low-Latency volumes don't support resizing. This operation applies to High IOPS SSD and Standard volumes only.
</Info>
...
<Info>
Snapshots are supported for `standard` and `ssd_hiiops` volume types. Snapshot creation is not supported for `ssd_lowlatency` volumes.
</Info>
...
| `type_name` | No | `"standard"` (default), `"ssd_hiiops"`, `"ssd_lowlatency"`, `"cold"`, or `"ultra"`. Available types vary by region. `"ssd_local"` is not available as a standalone volume — it can only be created together with a Virtual Machine. |
...
<Info>
SSD Low-Latency volumes don't support resizing. Use this step for High IOPS SSD and Standard volumes only.
</Info>
