...
<p>Provision a block storage volume using the Gcore [Terraform provider](/developer-tools/terraform/overview).</p>

<Tip>
The [gcore_cloud_volume](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_volume) reference on Terraform Registry includes additional configuration examples.
</Tip>

## Create a volume

<p>Set `source = "new-volume"` for a blank data volume, `"snapshot"` to restore from an existing snapshot, or `"image"` to create a boot volume from an OS image.</p>
