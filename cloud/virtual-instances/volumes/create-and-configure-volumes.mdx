...
<p>The following sections describe how to create and manage volumes. For a general overview of volumes and available volume types, see the [About volumes](/cloud/virtual-instances/volumes/about-volumes) guide.</p>

## Create a volume

<p>Volumes can be created in several ways: [when creating a Virtual Machine](/cloud/virtual-instances/create-an-instance), from the **Volumes** page, or from a snapshot. The latter two approaches are described in the following sections.</p>
...
<Tabs>
  <Tab title="From the Volumes page">
    ...
    6\. (Optional) Attach the volume to a Virtual Machine. The VM must exist in the same region — see [Create a virtual machine](/cloud/virtual-instances/create-an-instance).
  ...
  <Tab title="From a volume snapshot">
    To duplicate or back up Virtual Machine data quickly or provision new resources from pre-existing data, create a volume from an existing volume snapshot. A snapshot must exist first — see [Take a snapshot of your file system](/cloud/virtual-instances/snapshots/take-a-snapshot-of-your-file-system). 
...
<p>A regular volume that is not attached to a Virtual Machine can be deleted. Boot volumes can only be deleted together with the VM.</p>
