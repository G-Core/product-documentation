...
Volumes are persistent network-attached disks for Virtual Machines. Data survives VM reboots, and a volume can be detached from one VM and re-attached to another in the same region.
...
Set `INSTANCE_ID` to the UUID of an existing VM in the same region; it is needed only for the attach and detach steps.

## Quickstart

The scripts below create a 10 GiB standard volume, attach it to the VM identified by `INSTANCE_ID`, resize it to 20 GiB, then detach and delete it.
...
<Tabs>
  <Tab title="Python SDK">
    ...
    # Step 4. Detach the volume from the VM.
    ...
    # Step 5. Resize the volume to 20 GiB.
...
### Step 3. Detach from a Virtual Machine
...
Only detached volumes can be deleted. Boot volumes are deleted together with the VM when the instance delete call is issued.
