...
The [snapshot](/api-reference/cloud/snapshots/create-snapshot) is created asynchronously — the API returns a task ID immediately, and the actual snapshot becomes available after the task finishes.
...
Retrieve the [snapshot](/api-reference/cloud/snapshots/get-snapshot) to confirm its status and record its properties.
...
Delete a snapshot with the [delete snapshot](/api-reference/cloud/snapshots/delete-snapshot) endpoint when it is no longer needed.
