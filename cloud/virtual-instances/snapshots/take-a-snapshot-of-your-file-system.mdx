...
<Info>
A permanent [API token](/account-settings/api-tokens) is required, along with a project ID, region ID, and the ID of an existing volume to snapshot.
</Info>
...
Both SDK methods block until the snapshot reaches `available` status and return a typed snapshot object — `id`, `status`, `size`, `volume_id`, and `created_at` are available immediately on the returned value. When using curl, poll the task endpoint every 5 seconds until `state` is `FINISHED`, then read the snapshot ID from `created_resources.snapshots[0]`.

<Tabs>
  <Tab title="Python SDK">
    ```python
    # create_and_poll blocks until the snapshot is ready and returns a Snapshot object.
  ...
  <Tab title="Go SDK">
    ```go
    // NewAndPoll blocks until the snapshot is ready and returns *cloud.Snapshot.
    fmt.Printf("ID: %s\n", snapshot.ID)
    fmt.Printf("Status: %s\n", snapshot.Status)
