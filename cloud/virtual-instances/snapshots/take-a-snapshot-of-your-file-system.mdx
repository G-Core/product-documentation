...
Both SDK methods (`create_and_poll` / `NewAndPoll`) block until the snapshot is ready. When using curl, poll the task endpoint (`GET /cloud/v1/tasks/$TASK_ID`) every 5 seconds until `state` is `FINISHED`.

<Tabs>
  <Tab title="Python SDK">
    ```python
    # create_and_poll handles polling automatically — no extra code needed.
    ```
  </Tab>
  <Tab title="Go SDK">
    ```go
    // NewAndPoll handles polling automatically — no extra code needed.
    ```
  </Tab>
  <Tab title="curl">
    ```bash
...
Save the snapshot ID from `created_resources.snapshots[0]` for subsequent requests.
