...
ai-navigation: Compare Gcore VM types: Standard, GPU, CPU, Memory, High-frequency, Basic, TPM, and Network-optimised instances with network bandwidth specifications.
ai-navigation: Select Virtual Machine type: Standard, GPU, CPU, Memory, High-frequency, Basic, TPM, or Network-optimised for workload requirements.
---
...
| Other VMs (Standard,<br />CPU, Memory,<br />GPU, High-frequency,<br />TPM) | Up to 1 Gbps | Free, unlimited | Free, unlimited |

Bandwidth values indicate the maximum achievable speed. Actual throughput may vary depending on network conditions and workload. Gcore does not apply artificial throttling.

For workloads that require high and consistent bandwidth (10 Gbps or higher), use Bare Metal servers with dedicated network configurations.
...
These VMs run on dedicated infrastructure with hardware-level network acceleration, delivering near bare-metal networking performance while maintaining full security feature support including security groups and anti-spoofing protection.
