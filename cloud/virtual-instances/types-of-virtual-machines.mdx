...
ai-navigation: Configure Gcore Virtual Machine types including Standard, GPU, CPU, Memory, High-frequency, Basic VM, TPM, and Network optimised instances with network bandwidth specifications up to 1 Gbps and traffic policies.
---
...
| Standard, CPU, Memory,<br />GPU, High-frequency, TPM | Up to 1 Gbps | Free, unlimited | Free, unlimited |
| Network optimised (gX-net) | 5 Gbps or 10 Gbps (guaranteed) | Free, unlimited | Free, unlimited |

Bandwidth values indicate the maximum achievable speed for standard flavors. Network optimised VMs have guaranteed bandwidth enforced through QoS policies. Actual throughput for standard VMs may vary depending on network conditions and workload.
...
Virtual Machines designed for workloads that require high network throughput and low latency. Network optimised VMs provide guaranteed bandwidth through QoS policies: 5&nbsp;Gbps for smaller configurations and 10&nbsp;Gbps for larger configurations, compared to up to 1&nbsp;Gbps on standard flavors.

These VMs run on dedicated hypervisor nodes with hardware-level network offloading via Open vSwitch TC offloading on Mellanox ConnectX NICs. Network flows are processed directly in NIC hardware, delivering near bare-metal networking performance while maintaining full security feature support including security groups and anti-spoofing protection.
...
| **Flavor size** | **Guaranteed bandwidth** | **Burst** |
| --- | --- | --- |
...
- Network optimised nodes operate at up to 2x CPU overcommit compared to standard hypervisors.
- VxLAN-encapsulated networks receive partial hardware offload. VLAN-backed networks achieve close to 100% hardware offload.

<Tabs>
  <Tab title="G3A">
    Machines powered with **AMD EPYC processors**. Flavors up to 8 vCPU / 32 GB RAM use the 5 Gbps tier; larger flavors use 10 Gbps.
