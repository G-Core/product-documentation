...
SSH key authentication is the recommended connection method for Virtual Machines. It provides better security compared to password authentication.
...
<Warning>
They cannot be downloaded again after creation. Save the private key in a secure location immediately after generating it.
</Warning>

### Add multiple SSH keys to a resource

To allow multiple users or devices to access the same VM, add several keys through the [Gcore Customer Portal](https://portal.gcore.com) when [creating a VM](/cloud/virtual-instances/create-an-instance). In the **SSH key** section, select multiple keys from the saved keys list.

Upload additional keys from the [SSH Keys](/cloud/virtual-instances/connect/connect-to-your-instance-via-ssh#add-a-public-ssh-key-to-the-customer-portal) section.
...
SSH is not supported for Windows-based VMs. Use RDP (Remote Desktop Protocol) instead. On Windows PCs, the Remote Desktop client is installed by default.

## Network appliance images (FortiGate, MikroTik)

Network appliance images use their own authentication methods, not standard keys.
