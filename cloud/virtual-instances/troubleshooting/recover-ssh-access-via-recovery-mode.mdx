...
ai-navigation: Open a root shell on a Linux Virtual Machine or Bare Metal server from the VNC console when the private key is lost, then install a public key only when SSH must accept a new key.
---
...
<Steps>
  <Step title="Remount the root filesystem">
    ...
    <Frame>![Root shell after remounting the filesystem read-write](/images/docs/cloud/virtual-instances/troubleshooting/recover-ssh-access-via-recovery-mode/remount-read-write.png)</Frame>
  </Step>
