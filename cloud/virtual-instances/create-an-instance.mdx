...
<Tabs>
  ...
  <Tab title="Python SDK">
    ```python
    # create_and_poll() in step 4 polls automatically.
    # instance is already ACTIVE when step 4 returns - skip to step 6.
    ```
  </Tab>
</Tabs>
...
For curl, poll <code>GET&nbsp;/cloud/v1/tasks/{task_id}</code> until `state` is `FINISHED`. The boot volume ID is available in the instance details under `volumes[0].id`.
