...
<Warning>
`gcore_cloud_volume` is an independent resource. Removing the instance from your configuration does not delete its boot volume — you must remove `gcore_cloud_volume.boot` from the configuration as well and run `terraform destroy`.
</Warning>
