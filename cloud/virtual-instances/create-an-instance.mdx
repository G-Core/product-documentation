...
### Interface order and routing priority

When adding multiple interfaces, the order determines how the OS assigns routing metrics after the instance starts. The interface added first gets the lowest metric — meaning the highest routing priority — and that order is fixed at creation time.

Adding a public interface first and a private interface second places them in that order — after the instance is created, the **Networking** tab shows the interfaces the same way:

![Networking tab showing Interface 1 and Interface 2 in creation order](/images/docs/cloud/virtual-instances/create-an-instance/vm-networking-tab.png)

In the OS, the routing table reflects this order through metric values. Lower metric means higher priority:

```bash
$ ip route
default via 192.168.0.1 dev ens3 proto dhcp src 85.234.66.94 metric 100
default via 10.0.0.1 dev ens4 proto dhcp src 172.19.78.149 metric 200
192.168.0.0/24 dev ens3 proto kernel scope link src 85.234.66.94
10.0.0.0/24 dev ens4 proto kernel scope link src 172.19.78.149
```

In this example, `ens3` (Interface 1, public) has metric 100 and `ens4` (Interface 2, private) has metric 200. The OS routes outbound traffic through the public interface by default. The interface order cannot be changed after the instance is created.
