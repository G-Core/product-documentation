...
<p>Provisioning a VM in Terraform requires two separate resources: a `gcore_cloud_volume` for the boot disk (created from an OS image) and a `gcore_cloud_instance` that references it by volume ID — declare the volume first.</p>

<Tip>
See the [Terraform Registry](https://registry.terraform.io/providers/G-Core/gcore/latest/docs) for the full provider reference. This tab uses the [gcore_cloud_instance](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_instance) resource.
</Tip>
...
<p>To find the image ID for the region, query the [Images API](https://api.gcore.com/docs/cloud#tag/Images/operation/ImageListForProjectRegion.get):</p>
