---
title: "Create a virtual instance"
sidebarTitle: "Create an instance"
ai-navigation: Create Linux or Windows Virtual Instances in Gcore Cloud via Customer Portal, REST API (including Basic VMs), or Terraform.
---
...
1. In the [Gcore Customer Portal](https://portal.gcore.com/accounts/reports/dashboard), navigate to **Virtual Instances** and click **Create Instance**.

<Frame>
  ![The Create VM button](/images/docs/cloud/virtual-instances/create-an-instance/1-create-instancebutton.png)
</Frame>

2. Select the region where the Virtual Instance will be deployed.
...
<Frame>
  ![Choose x86-64 or ARM architecture](/images/docs/cloud/virtual-instances/create-an-instance/configure-image.png)
</Frame>
...
- **Distributions** — standard OS distributions: Ubuntu, Debian, CentOS, Windows, and others.
...
<p>Enter a volume name, select its type, and set its size in GiB.</p>
...
<Accordion title="Available volume types">
  ...
  - **SSD Low-Latency**. SSD block storage designed for applications that require low-latency storage and real-time data processing. 50 IOPS per 1 GB; minimum 1,000 IOPS; maximum 75,000 IOPS. Bandwidth limit: 1,500 MB/s. Average latency: 300 µs.
</Accordion>
...
<p>To assign multiple public IP addresses, add additional public interfaces after the instance is created: open the **Networking** tab in the VM settings and click **Add Interface**.</p>

<Frame>
  ![Network settings section with Public network interface selected](/images/docs/cloud/virtual-instances/create-an-instance/vm-network-section.png)
</Frame>

<Info>
  The default security group allows inbound traffic over ICMP, SSH (TCP/22), and RDP (TCP/3389 and UDP/3389). All outbound traffic is allowed. [Custom&nbsp;security&nbsp;groups](/cloud/networking/add-and-configure-a-firewall) can override these defaults.
</Info>

<p>For a **private** interface, configure a network and a subnet as described below.</p>

<Tip>
  If both a public and private interface are needed, disable the default gateway on the private network's subnet and assign a floating IP to the private interface.
</Tip>
...
<Info>
  When a Virtual Instance has several subnets, keep only one routable — [default&nbsp;gateway&nbsp;conflicts](/cloud/networking/create-and-manage-a-subnet#set-up-the-default-gateway) otherwise prevent connecting to the instance.
</Info>

<p>Select an existing subnet from the dropdown list or create a new one by clicking **Add a new subnet**.</p>

<Frame>
  ![Add a new subnet](/images/docs/cloud/virtual-instances/create-an-instance/add-new-subnetwork.png)
</Frame>

<Accordion title="Create a new subnet">
  1. Click **Add a new subnet**.
  ...
  9. (Optional) Enable **Add tags** to attach metadata to the subnet.
  10. Click **Create subnet**.

  <Frame>
    ![Add a new subnet dialog](/images/docs/cloud/virtual-instances/create-an-instance/create-subnetwork-annotated.png)
  </Frame>
</Accordion>
...
<Accordion title="Interface naming when using multiple interfaces">
  ...
  <p>The first interface (`enp3s0`) is the public interface; the second interface (`enp4s0`) is the private interface. The order set at creation determines routing priority: the first interface has higher priority for outbound traffic than subsequent ones. The interface order cannot be changed after the instance is created.</p>
</Accordion>
...
<Tabs>
  ...
  <Tab title="Windows Virtual Machine">
    ...
    <Info>
      The password must contain 8-16 characters, including at least one lowercase letter (`a-z`), one uppercase letter (`A-Z`), one number (`0-9`), and one special character (`!#$%&'()*+,-./:;<=>?@[]^_{|}~`).
    </Info>
...
- `{ip_octets}` — the placeholder is replaced with the last two octets of the assigned IP address — `my-vm-{ip_octets}` becomes `my-vm-10-0-5`.
...
<Tabs>
  ...
  <Tab title="Go SDK">
    ```go
    sshKeyCreated, err := client.Cloud.SSHKeys.New(ctx, cloud.SSHKeyNewParams{
        Name: "my-ssh-key",
    })
    if err != nil {
        panic(err.Error())
...
<Tabs>
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        // Step 2. Pick a flavor (IDs are region-specific; this selects 2 vCPU / 4 GB)
        flavorList, err := client.Cloud.Instances.Flavors.List(ctx, cloud.InstanceFlavorListParams{})
        if err != nil {
            panic(err.Error())
        }
        var flavorID string
...
<Tabs>
  ...
  <Tab title="Go SDK">
    ```go
    instanceFlavorList, err := client.Cloud.Instances.Flavors.List(ctx, cloud.InstanceFlavorListParams{})
    if err != nil {
        panic(err.Error())
    }
...
<p>Basic VMs boot from a system-managed local SSD — no volume configuration is accepted on create. Networking is references-only: the create call attaches existing ports by ID rather than creating interfaces inline. The region is specified in the request body, so a single endpoint handles all regions without encoding the region in the URL path.</p>

<Info>
An [API token](/account-settings/api-tokens) is required, along with a [project ID](/api-reference/cloud/projects/list-projects) and a pre-created [port](/cloud/networking/ip-address/create-and-configure-a-reserved-ip-address) in the target region.
</Info>

<p>Set the required environment variables. `REGION_ID` is the numeric ID of a region that supports Basic VMs — query the [regions API](/api-reference/cloud/regions/list-regions) and filter for regions where `has_basic_vm` is `true`.</p>
...
<p>Only Linux images are accepted on Basic VM create. Query the images endpoint for the target region and save the ID of the chosen image.</p>

```bash
curl "https://api.gcore.com/cloud/v1/images/$GCORE_CLOUD_PROJECT_ID/$REGION_ID?visibility=public" \
  -H "Authorization: APIKey $GCORE_API_KEY"
```
...
### Step 2. List Basic VM flavors

<p>Basic VM flavors use the `g1s-shared-*` or `g2s-shared-*` naming scheme. Query the standard flavors endpoint and filter by name prefix.</p>

```bash
curl "https://api.gcore.com/cloud/v1/flavors/$GCORE_CLOUD_PROJECT_ID/$REGION_ID" \
  -H "Authorization: APIKey $GCORE_API_KEY"
```
...
{
  "results": [
    {"flavor_id": "g2s-shared-1-2-25",  "vcpus": 1, "ram": 2048},
    {"flavor_id": "g2s-shared-2-4-50",  "vcpus": 2, "ram": 4096},
    {"flavor_id": "g2s-shared-4-8-80",  "vcpus": 4, "ram": 8192}
  ]
}
```

<p>The number after the RAM value in the flavor name is the local SSD size in GiB. Set `FLAVOR_NAME` to the chosen flavor.</p>

```bash
export FLAVOR_NAME="g2s-shared-1-2-25"
```

### Step 3. Create the Basic VM

<p>Pass `region_id` in the request body rather than the URL path. The boot disk is created automatically; no volume fields are accepted.</p>

| Parameter | Required | Description |
|-----------|----------|-------------|
| `region_id` | Yes | Numeric ID of the target region. |
| `flavor_name` | Yes | Shared-CPU flavor name from Step 1. |
| `image_id` | Yes | ID of a Linux OS image. Only Linux images are accepted. |
| `interfaces[].port_id` | Yes | ID of a pre-created port in the same region. Pass multiple entries to attach several NICs; the array order sets the NIC boot order. |
| `name` | No | Display name. Auto-generated if omitted. |
| `ssh_key_id` | No | UUID of an SSH key registered in the project. |
| `user_data` | No | Base64-encoded cloud-init script. |
| `tags` | No | Key-value map of labels. |

```bash
curl -X POST "https://api.gcore.com/cloud/v1/projects/$GCORE_CLOUD_PROJECT_ID/basic_vms" \
  -H "Authorization: APIKey $GCORE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "region_id": '"$REGION_ID"',
...
<p>Poll `GET /cloud/v1/tasks/{task_id}` every 5 seconds until `state` is `FINISHED`, then read the instance ID from `created_resources`.</p>
...
```bash
export BASIC_VM_ID="{BASIC_VM_ID}"
```

### Step 4. Get Basic VM details

<p>The response includes `boot_volume_id` referencing the system-managed local disk, which is destroyed together with the VM on delete.</p>

```bash
curl "https://api.gcore.com/cloud/v1/projects/$GCORE_CLOUD_PROJECT_ID/basic_vms/$BASIC_VM_ID" \
  -H "Authorization: APIKey $GCORE_API_KEY"
```
...
```json
{
  ...
  "status": "ACTIVE",
  "region_id": 26,
  "flavor_name": "g2s-shared-1-2-25",
  "image_id": "bf221f42-...",
  "boot_volume_id": "e5f6a7b8-...",
  "addresses": {
    "doc-test-net": [{"addr": "10.10.10.5", "type": "fixed"}]
  }
}
```

### Step 5. Update the Basic VM

<p>Rename the VM or replace its interface list. Updating `interfaces[]` is a full replace — the API diffs the submitted list against the current state and attaches or detaches ports accordingly. Interface order is immutable after creation.</p>

| Field | Description |
|-------|-------------|
| `name` | New display name. |
| `tags` | Merge-patch map — a `null` value removes the key. |
| `interfaces[]` | Full list of port IDs. Ports in the list but not currently attached are attached; ports currently attached but not in the list are detached. |

```bash
curl -X PATCH "https://api.gcore.com/cloud/v1/projects/$GCORE_CLOUD_PROJECT_ID/basic_vms/$BASIC_VM_ID" \
  -H "Authorization: APIKey $GCORE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"name": "my-basic-vm-renamed"}'
```
...
<p>Poll the task until `state` is `FINISHED`.</p>

### Step 6. Delete the Basic VM

<p>Deleting a Basic VM also destroys its system-managed boot disk. Ports attached to the VM are detached but not deleted.</p>

```bash
curl -X DELETE "https://api.gcore.com/cloud/v1/projects/$GCORE_CLOUD_PROJECT_ID/basic_vms/$BASIC_VM_ID" \
  -H "Authorization: APIKey $GCORE_API_KEY"
```
...
| `security_groups` | List of security group IDs. If omitted, the default security group is applied. |
