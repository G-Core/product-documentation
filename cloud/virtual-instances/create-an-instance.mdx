...
Creating a Linux VM via the REST API takes three calls: list available images, list flavors, then submit the create request. The instance is reachable via SSH within about 30 seconds of the task finishing.

<Info>
A permanent [API token](/account-settings/api-tokens) is required, along with a project ID and region ID.

- **Project ID**: visible in the Customer Portal on the **Projects** page - switch to grid view to see IDs next to each project name.
- **Region ID**: use the [list regions](https://gcore.com/docs/api-reference/cloud/regions/list-regions) reference page to find the numeric ID for the region.
- To retrieve both programmatically, see [list projects](https://gcore.com/docs/api-reference/cloud/projects/list-projects) and list regions.

An SSH key must already be registered in the project. To add one, see the [SSH key](#add-an-ssh-key) section below.
</Info>
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl "https://api.gcore.com/cloud/v1/images/$PROJECT_ID/$REGION_ID" \
...
A flavor sets the vCPU count and RAM for the VM. Flavor IDs are strings, not UUIDs.
...
Instance creation is asynchronous. For curl, poll <code>GET&nbsp;/cloud/v1/tasks/{task_id}</code> every 5 seconds until `state` is `FINISHED`. The Python SDK `create_and_poll()` in Step 3 handles this automatically.

<Tabs>
  <Tab title="Python SDK">
    ```python
    # create_and_poll() in Step 3 polls automatically.
    # instance is already ACTIVE when Step 3 returns.
    ```
  </Tab>
  <Tab title="curl">
    ```bash
...
For curl, poll <code>GET&nbsp;/cloud/v1/tasks/{task_id}</code> every 5 seconds until `state` is `FINISHED`. If a floating IP was assigned to the instance, add `delete_floatings=true` to the query string.

## Advanced options

The steps below cover less common scenarios that are not required for the basic flow.
