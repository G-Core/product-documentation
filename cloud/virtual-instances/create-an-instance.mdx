...
ai-navigation: Create a Linux or Windows Virtual Machine in Gcore Cloud via Customer Portal, REST API, or Terraform.
---
...
Creating a Linux VM via the REST API consists of three API calls: list available images, list flavors, then submit the create request. The instance is reachable via SSH within about 30 seconds of the task finishing.
...
<p>Creating a VM in Terraform requires two separate resources: `gcore_cloud_volume` for the boot disk (created from an OS image) and `gcore_cloud_instance` that references it. Unlike the API, you cannot inline the image source directly inside the instance resource — the volume must be provisioned first.</p>

## Create a VM

<p>The following configuration creates a boot volume from an Ubuntu 22.04 image and a VM with a public IPv4 interface. An existing SSH key is referenced by name; to manage a key through Terraform, see [Add an SSH key](#add-an-ssh-key-1).</p>
...
To find the image ID for your region, query the [Images API](https://api.gcore.com/docs/cloud#tag/Images/operation/ImageListForProjectRegion.get):
...
<Warning>
`gcore_cloud_volume` is an independent resource. Removing `gcore_cloud_instance.example` from your configuration does not delete its boot volume. To deprovision both, remove `gcore_cloud_volume.boot` from the configuration as well, then run `terraform apply`.
</Warning>
...
When `public_key` is omitted, Gcore generates a key pair. The private key is returned once in the response and is not stored — save it before applying.
...
Example with `user_data` and `tags`:
...
<p>Remove both `gcore_cloud_instance.example` and `gcore_cloud_volume.boot` from your configuration, then run:</p>

```bash
terraform destroy
```

Terraform destroys the instance first, then the volume.

## Import a VM

<p>To bring an existing VM under Terraform management, import the instance and its boot volume separately:</p>
...
To find the boot volume ID, look for `"is_root_volume": true` in the instance details:
