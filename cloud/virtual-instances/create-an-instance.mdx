...
<Frame>
  <img
    src="/images/docs/cloud/virtual-instances/create/configure-image.png"
...
The diagram below shows how the selection order in the creation form carries through to the **Networking** tab and then to the OS interface names and routing priority:

![Diagram showing how interface order set during VM creation maps to the Networking tab and OS routing priority](/images/docs/cloud/virtual-instances/create-an-instance/vm-network-interface-order-diagram.png)

In the OS, the routing table reflects this order through metric values. Lower metric means higher priority:

```bash
$ ip route show
default via 85.234.84.1 dev enp3s0 proto dhcp src 85.234.84.186 metric 1000
default via 10.0.0.1 dev enp4s0 proto dhcp src 10.0.0.132 metric 2000
10.0.0.0/24 dev enp4s0 proto kernel scope link src 10.0.0.132 metric 2000
85.234.84.0/24 dev enp3s0 proto kernel scope link src 85.234.84.186 metric 1000
...
<Frame>
  ![Terminal output showing ip route and ip addr commands: enp3s0 public interface has metric 1000, enp4s0 private interface has metric 2000](/images/docs/cloud/virtual-instances/create-an-instance/vm-ip-route-metrics.png)
</Frame>

In this example, `enp3s0` (Interface 1, public) has metric 1000 and `enp4s0` (Interface 2, private) has metric 2000. The OS routes outbound traffic through the public interface by default. The interface order cannot be changed after the instance is created.
