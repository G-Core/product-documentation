...
<p>Provisioning a VM in Terraform requires two separate resources: `gcore_cloud_volume` for the boot disk (created from an OS image) and `gcore_cloud_instance` that references it. The instance resource does not accept an inline image source — the volume must exist first.</p>
...
<p>Remove both `gcore_cloud_instance.example` and `gcore_cloud_volume.boot` from the configuration, then run:</p>

```bash
terraform destroy
```

<p>Terraform destroys the instance first, then the volume.</p>
