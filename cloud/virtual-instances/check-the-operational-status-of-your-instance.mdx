...
<Info>A permanent API token, a project ID, and a region ID are required to authenticate API requests.</Info>
...
The scripts below retrieve the status of all Virtual Machines in a region and the status of a specific VM by ID.
...
# Step 1. List all instances and their statuses.
for instance in client.cloud.instances.list():
    print(f"{instance.name}: {instance.status}")

# Step 2. Get the status of a specific instance.
...
### Step 1. List all instances in the region
...
```
  ...
  <Tab title="curl">
```bash
curl -X GET "https://api.gcore.com/cloud/v1/instances/${PROJECT_ID}/${REGION_ID}" \
...
```
  ...
  <Tab title="curl">
```bash
curl -X GET "https://api.gcore.com/cloud/v1/instances/${PROJECT_ID}/${REGION_ID}/${INSTANCE_ID}" \
  -H "Authorization: APIKey ${GCORE_API_KEY}"
```
