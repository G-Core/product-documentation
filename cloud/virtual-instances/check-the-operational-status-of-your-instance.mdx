...
Run `terraform apply` — Terraform queries the API and prints:
...
For a full list of `status` values and what each means, see [Status values](#status-values) below.
...
To sync state with the live API:
