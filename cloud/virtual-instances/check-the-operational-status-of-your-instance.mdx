...
<p>Read the current operational state of a Virtual Machine without managing it as a Terraform resource. The [`gcore_cloud_instance`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/data-sources/cloud_instance) data source retrieves `status` and `vm_state` directly from the API and can expose them as outputs or pass them to dependent resources.</p>
