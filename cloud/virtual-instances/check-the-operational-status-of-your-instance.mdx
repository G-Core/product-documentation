...
ai-navigation: Retrieve Virtual Machine operational status via the Customer Portal Status column, the REST API using GET /cloud/v1/instances to read ACTIVE, BUILD, SHUTOFF, and ERROR status values, or Terraform using the gcore_cloud_instance data source.
---
...
<p>For a full list of status values and their meanings, see [Status values](#status-values) below.</p>
...
<Info>An [API token](/account-settings/api-tokens) is required, along with a [project ID](https://gcore.com/docs/api-reference/cloud/projects/list-projects) and [region ID](https://gcore.com/docs/api-reference/cloud/regions/list-regions).</Info>
...
<p>The `data "gcore_cloud_instance"` data source reads a VM's current status from the API and exposes it as a Terraform output or passes it to other resources in the configuration.</p>

<Tip>
See the [Terraform Registry](https://registry.terraform.io/providers/G-Core/gcore/latest/docs) for the full provider reference. This tab uses the [gcore_cloud_instance](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/data-sources/cloud_instance) data source.
</Tip>

## Get instance status

<p>Reads a single instance by ID and surfaces its `status`, `vm_state`, and `task_state`. Use in configurations where the live state of an existing VM must be known before provisioning dependent resources.</p>
...
<p>For instances declared as `gcore_cloud_instance` resources, `terraform show` prints the last known state without an API call:</p>

```bash
terraform show
```

<p>To sync state with the live API:</p>
...
| `SUSPENDED` | `suspended` | — | VM is suspended by the platform. |
