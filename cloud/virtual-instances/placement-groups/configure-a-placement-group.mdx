...
ai-navigation: Create and manage Virtual Machine placement groups via the Customer Portal, REST API, or Terraform, covering Affinity, Anti-Affinity, and Soft Anti-Affinity policies, VM assignment during creation, and moving VMs between groups.
---
...
<Frame>
  ![Create Placement Group dialog with Name and Policy fields](/images/docs/cloud/virtual-instances/placement-groups/configure-a-placement-group/create-placement-group-dialog.png)
</Frame>
...
<Frame>
  ![Add to placement group checkbox and Placement Group dropdown on the create instance form](/images/docs/cloud/virtual-instances/placement-groups/configure-a-placement-group/create-instance-placement-group.png)
</Frame>
...
<Frame>
  ![Placement Group tab with group selector and Add button](/images/docs/cloud/virtual-instances/placement-groups/configure-a-placement-group/placement-group-selector.png)
</Frame>
...
<Frame>
  ![Placement Group tab showing assigned group and Remove button](/images/docs/cloud/virtual-instances/placement-groups/configure-a-placement-group/placement-group-assigned.png)
</Frame>

<Frame>
  ![Placement Group dropdown after removal, ready to select a new group](/images/docs/cloud/virtual-instances/placement-groups/configure-a-placement-group/placement-group-selector.png)
</Frame>
</MethodSection>
...
A [placement group](/api-reference/cloud/placement-groups/create-placement-group) defines the policy that controls how VMs assigned to it are distributed across physical hosts. The group name and policy are set at creation and cannot be changed.
...
| `region_id` | Yes | Numeric ID of the region where the group is created. |
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl -X POST "https://api.gcore.com/cloud/v1/projects/$GCORE_CLOUD_PROJECT_ID/placement_groups" \
      -H "Authorization: APIKey $GCORE_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"name": "my-group", "policy": "anti-affinity", "region_id": '$GCORE_CLOUD_REGION_ID'}'
    ```
    ...
    ```json
    {
      "tasks": ["3445e188-452f-4968-bc8d-9da28f41ce7c"]
    }
    ```

    Poll <code>GET&nbsp;/cloud/v1/tasks/{task_id}</code> every 5 seconds until `state` is `FINISHED`. The group ID is in `created_resources.servergroups[0]`.

    ```bash
    export GROUP_ID="{created_resources.servergroups[0]}"
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl "https://api.gcore.com/cloud/v1/projects/$GCORE_CLOUD_PROJECT_ID/placement_groups/$GROUP_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
    ...
    ```json
    {
      "id": "390bf8f5-c2e6-490a-b8a6-b80341a61d18",
      "name": "my-group",
      "policy": "anti-affinity",
      "region_id": 76,
      "managed_by": "user",
      "member_ids": [
        "7710bd12-3c1c-4566-b2d0-76e3ff51a1a3"
      ],
      "created_at": "2026-09-10T11:06:23.456117",
      "updated_at": "2026-09-10T11:06:23.477125"
    }
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl -X DELETE \
      "https://api.gcore.com/cloud/v1/projects/$GCORE_CLOUD_PROJECT_ID/placement_groups/$GROUP_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
