...
<p>Control VM scheduling on physical hosts using the [`gcore_cloud_placement_group`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_placement_group) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview). Choose `anti-affinity` to spread instances across different hosts, or `affinity` to co-locate them.</p>
