...
The Gcore Terraform provider v2 supports the following Cloud resources: Virtual Machines, Bare Metal servers, Kubernetes clusters, Load Balancers, networks, volumes, security groups, floating IPs, and reserved IPs.

Provider v2 uses numeric `project_id` and `region_id` fields directly in each resource instead of data source lookups. For provider installation and authentication, see the [Terraform overview](/developer-tools/terraform/overview).

<Info>
**Project and region IDs** are account-specific. Find your project ID and region IDs in the Customer Portal URL: `portal.gcore.com/cloud/project/<project_id>/region/<region_id>/...`. Image IDs, flavor names, and Bare Metal image IDs are region-specific — use the [Images API](https://api.gcore.com/docs/cloud#tag/Images/operation/ImageListV1ProjectsProjectIdRegionsRegionIdImagesGet) or the Portal to look them up for your region. The examples below use Luxembourg-2 (region ID `76`).
</Info>
...
`direction` accepts `ingress` or `egress`. `ethertype` accepts `IPv4` or `IPv6`. To allow all protocols, omit `protocol`, `port_range_min`, and `port_range_max`. Reference the security group ID in instance `interfaces` or the top-level `security_groups` block.
...
Private networks for Bare Metal require `type = "vlan"` on the network resource.
