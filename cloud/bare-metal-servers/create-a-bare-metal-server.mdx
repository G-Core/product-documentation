...
ai-navigation: Create a Bare Metal server on dedicated physical hardware by selecting region, image template, and server type (Basic, High-Frequency, Infrastructure) in the Gcore Cloud portal, with quota increase required for physical server provisioning. Includes multi-bond network configuration, where each bond (Trunk Interface) groups LACP-aggregated ports with assignable sub-interfaces, available on request.
---
...
3. (Optional) Enable **Use reserved IP** to assign a [reserved IP address](/cloud/networking/ip-address/create-and-configure-a-reserved-ip-address) to your Bare Metal.
