...
<p>Provision a dedicated Bare Metal server using the [Terraform provider](/developer-tools/terraform/overview). Bare Metal servers have per-region [quota limits](/cloud/getting-started/request-a-quota-increase) — verify usage before applying.</p>
...
<Info>
Flavor IDs and image IDs are region-specific — use the Customer Portal or the [flavors API](https://api.gcore.com/docs/cloud#tag/Baremetal/operation/BaremetalFlavorListV1ProjectsProjectIdRegionsRegionIdBmflavorsGet) and [images API](https://api.gcore.com/docs/cloud#tag/Baremetal/operation/BareMetalImageListV1ProjectsProjectIdRegionsRegionIdBmimagesGet) to find available values for the target region. Pass them as `var.flavor_id` and `var.image_id` in `terraform.tfvars`.
</Info>
...
<Warning>
The interface order is set at creation and cannot be changed. With both a public and a private interface, disable the default gateway on the private subnet to avoid routing conflicts.
</Warning>
