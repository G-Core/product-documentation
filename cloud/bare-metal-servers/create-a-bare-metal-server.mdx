...
<Tabs>
  ...
  <Tab title="Private">
    ...
    <Warning>
      With both a public and a private interface, disable the default gateway on the private subnet. Assign a floating IP to the private interface to avoid routing conflicts.
    </Warning>
...
<p>Each interface block shows the sub-interfaces assigned to that bond — a sub-interface is the logical port that carries traffic for a specific network. The table for each sub-interface includes the IP address, network, subnet, MAC address, CIDR, and floating IP assignment.</p>
