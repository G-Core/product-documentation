...
- **Sub-Interface 1** (public) is served directly by `bond0`.
- **Sub-Interface 2** (private) is served by a VLAN sub-interface named `bond0.<vlan_id>` (for example, `bond0.2853`).
...
2: enp1s0f0: <BROADCAST,MULTICAST,SLAVE,UP,LOWER_UP> mtu 9000 ... master bond0
3: enp1s0f1: <BROADCAST,MULTICAST,SLAVE,UP,LOWER_UP> mtu 9000 ... master bond0
5: bond0: <BROADCAST,MULTICAST,MASTER,UP,LOWER_UP> mtu 1500 qdisc noqueue state UP
    link/ether b4:96:91:cd:b8:c4 brd ff:ff:ff:ff:ff:ff
    inet 85.234.84.212/24 brd 85.234.84.255 scope global bond0
6: bond0.2853@bond0: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc noqueue state UP
    link/ether b4:96:91:cd:b8:c4 brd ff:ff:ff:ff:ff:ff
    inet 10.0.0.212/24 metric 100 brd 10.0.0.255 scope global dynamic bond0.2853
```

<Frame>
  ![Terminal output showing ip a command: bond0 has public IP 85.234.84.212/24 with no metric, bond0.2853 private sub-interface has metric 100](/images/docs/cloud/bare-metal-servers/create-a-bare-metal-server/bm-ip-addr-metrics.png)
</Frame>

The private sub-interface gets `metric 100`, which means the OS prefers the public interface (`bond0`) for outbound traffic by default. The sub-interface order cannot be changed after the server is created.
