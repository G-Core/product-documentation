...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl "https://api.gcore.com/cloud/v1/bmimages/$PROJECT_ID/$REGION_ID" \
      -H "Authorization: APIKey $GCORE_API_KEY"
    ```
...
Bare Metal servers have per-region quotas tracked separately for `baremetal_hf`, `baremetal_basic`, `baremetal_storage`, `baremetal_network`, and `baremetal_infrastructure` categories. Before creating a server, verify that the usage is below the limit. For the full API workflow, see [Check resource quotas](/cloud/getting-started/check-resource-quota).
