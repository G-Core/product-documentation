...
<p>Provision a dedicated Bare Metal server using the [`gcore_cloud_baremetal_server`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/resources/cloud_baremetal_server) resource from the [Gcore Terraform provider v2](/developer-tools/terraform/overview). Bare Metal servers have per-region [quota limits](/cloud/getting-started/request-a-quota-increase) and take 4–6 minutes to provision — verify quota availability before running `terraform apply`.</p>
