...
After provisioning, retrieve the server details with `GET /cloud/v1/bminstances/{project_id}/{region_id}/{server_id}` — each trunk appears as a separate interface block in the `addresses` map.
