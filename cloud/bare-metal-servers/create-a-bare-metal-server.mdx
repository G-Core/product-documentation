...
When adding multiple interfaces, the order determines how the OS assigns routing metrics after the server starts. The interface added first gets the lowest metric — meaning the highest routing priority — and that order is fixed at creation time.

After the server is created, the **Networking** tab shows the interfaces in the same order as they were added during setup.

In the OS, the routing table reflects this through metric values. Lower metric means higher priority. For a server with a public interface added first (metric 100) and a private interface added second (metric 200):
...
The OS routes outbound traffic through the public interface by default. The interface order cannot be changed after the server is created.
