...
In the [Gcore Customer Portal](https://portal.gcore.com), navigate to **Cloud** > **Bare Metal** and click **Create Bare Metal**.
...
<p>BM [flavors](/api-reference/cloud/bare-metal/list-bare-metal-flavors) describe the physical hardware: CPU model, core count, RAM, local storage, and NIC speed. Use `GET /cloud/v1/projects/{project_id}/baremetal/flavors` to retrieve flavors across all regions in one call.</p>

<p>List flavors across all regions:</p>
...
<p>To filter by specific regions, add `region_ids` parameters:</p>

```bash
curl "https://api.gcore.com/cloud/v1/projects/$GCORE_CLOUD_PROJECT_ID/baremetal/flavors?region_ids=148&region_ids=5&include_capacity=true" \
  -H "Authorization: APIKey $GCORE_API_KEY"
```

    <p>The API returns one entry per flavor per region. Each entry includes `networking_capabilities`, which describes the network features supported by that hardware:</p>
...
<p>`capacity: 0` means the flavor is temporarily unavailable in that region. Filter to entries with `capacity > 0` before provisioning.</p>

<p>Save the `name` value of the chosen flavor for the next step:</p>

```bash
export BM_FLAVOR="{flavor-name}"
```

<Info>
The same `name` value is used in the server creation request as the `flavor` field.
</Info>
