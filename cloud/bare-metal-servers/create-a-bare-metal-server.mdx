...
`capacity: 0` means the flavor is temporarily unavailable in the region. Flavor IDs are region-specific - run this call to see what is available in the target region.
...
Use the public IP from Step 4 to connect. The default user for Ubuntu images is `ubuntu`.
...
`bond0` is the public interface. `bond0.<vlan_id>` sub-interfaces appear when private networks are attached at creation.
