...
6: bond0.2853@bond0: ... state UP
    inet 10.0.0.212/24 metric 100 brd 10.0.0.255 scope global dynamic bond0.2853
```

The private sub-interface gets `metric 100`, which means the OS prefers the public interface for outbound traffic by default. The sub-interface order cannot be changed after the server is created.
