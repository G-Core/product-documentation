...
3. Select the key when [creating a Bare Metal server](/cloud/bare-metal-servers/create-a-bare-metal-server).
...
To allow multiple users or devices to access the same server, you can add several public SSH keys. Use the **User Data** field when creating a server:
...
You can find your server's username in the Gcore Customer Portal: go to **Cloud** > **Bare Metal Servers**, open your server, and check the information next to the **Access to Console** link in the format `[login]@[ip-of-your-server]`.
...
If you need password access, configure a password during server creation using the **User Data** field:
...
We do not recommend enabling SSH password authentication due to security concerns. Use this password only for console access when SSH key authentication is not available.
</Warning>
...
1. **If you configured a password during server creation:** Access the server via console and add a new public key to `~/.ssh/authorized_keys`.

2. **If no password was configured:** You will need to reinstall the server with a new SSH key.

<Tip>
**Tip**

To prevent losing access, add multiple SSH keys to your server during creation or configure a console password as a backup access method.
</Tip>
