...
2. **If no password was configured:** Open **Access to Console** and install a new public key from recovery mode. See [Recover SSH access in recovery mode](/cloud/virtual-instances/troubleshooting/recover-ssh-access-via-recovery-mode). If the GRUB menu has no recovery mode entry, reinstall the server with a new SSH key.
