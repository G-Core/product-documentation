...
1. [Generate SSH keys](/cloud/virtual-instances/connect/connect-to-your-instance-via-ssh#generate-ssh-keys) or use existing keys.
2. [Add the public key to the Gcore Customer Portal](/cloud/virtual-instances/connect/connect-to-your-instance-via-ssh#add-a-public-ssh-key-to-the-customer-portal).
...
To allow multiple users or devices to access the same server, add several SSH keys through the Gcore Customer Portal when [creating a Bare Metal server](/cloud/bare-metal-servers/create-a-bare-metal-server). In the **SSH key** section, select multiple keys from the saved keys list.
...
- **Linux servers** — add a `cloud-init` script in the **User data** field. See [Connect with a password](/cloud/bare-metal-servers/connect-to-your-bare-metal-server-via-ssh#connect-with-a-password) for the `#cloud-config` example.
...
If a previously created private subnetwork is used during creation, the server can be provisioned without a public IP.

SSH from external networks requires a routable address. Attach a [floating IP address](/cloud/networking/ip-address/create-and-configure-a-floating-ip-address) to the server for external access.
...
Use RDP (Remote Desktop Protocol) instead. On Windows PCs, the Remote Desktop client is installed by default. For console access from the portal, see [Connect from the Customer Portal](/cloud/virtual-instances/connect/connect-to-your-instance-via-control-panel).
