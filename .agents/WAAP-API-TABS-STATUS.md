...
| 18 | waap-rules/advanced-rules.mdx | TODO | TODO | TODO | TODO | MISSED. Article explicitly states "only available through our API". GET/POST /waap/v1/domains/{id}/advanced-rules, GET/PATCH/DELETE /{rule_id}, PATCH /{rule_id}/{action}. Source written in CEL. |
| 19 | waap-rules/advanced-rules/advanced-rate-limiting-rules.mdx | TODO | TODO | TODO | TODO | MISSED. Dedicated article for rate-limiting within advanced rules. Same /advanced-rules endpoints, but source uses request.limit_rate() CEL method. Show create/update/delete examples specific to rate limiting. |
...
| 22 | api-discovery-and-protection/api-discovery.mdx | TODO | TODO | TODO | TODO | MISSED. Full CRUD for API paths: GET/POST /waap/v1/domains/{id}/api-discovery/paths, GET/PATCH/DELETE /{path_id}. Upload Swagger: POST /upload, POST /scan. Settings: GET/PATCH /api-discovery/settings. Scan results: GET /scan-results. |
| 23 | threat-intelligence/security-insights/view-insights.mdx | TODO | TODO | TODO | TODO | MISSED. GET /waap/v1/domains/{id}/insights (list with filters). GET /domains/{id}/insights/{id}. GET /waap/v1/security-insights/types. InsightStatus enum: OPEN/ACKED/CLOSED. |
| 24 | threat-intelligence/security-insights/manage-insights.mdx | TODO | TODO | TODO | TODO | MISSED. PUT /waap/v1/domains/{id}/insights/{id} (update status: ACKED=read, CLOSED=close, OPEN=reopen). POST /waap/v1/domains/{id}/insight-silences (create silence with insight_type + labels). |
| 25 | threat-intelligence/security-insights/manage-silence-rules.mdx | TODO | TODO | TODO | TODO | MISSED. GET /waap/v1/domains/{id}/insight-silences (list). PATCH /insight-silences/{id} (update duration/labels). DELETE /insight-silences/{id} (unsilence). |
| 26 | ip-security/ip-spotlight.mdx | TODO | TODO | TODO | TODO | MISSED. Multiple GET /waap/v1/ip-info/* endpoints: /ip-info (comprehensive IP data), /counts, /blocked-requests, /attack-time-series, /top-attacks, /top-urls, /top-user-agents, /top-domains, /top-sessions, /attack-map, /ddos. v2 scoped endpoints also available. |
