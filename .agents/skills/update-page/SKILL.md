...
### Ticket is a signal, not a blueprint

A Jira ticket tells you **what content is missing** — not how to structure the article or
how much space the change deserves. The tech writer decides structure.

- An "optional" feature mentioned in a ticket → one sentence where it naturally fits, not a new section.
- A minor behavior fix → fit it into the existing paragraph where that behavior is described.
- The ticket's wording is a starting point, not copy-paste material.
- Do not inflate small changes. A ticket about a validation rule does not justify
  rewriting the surrounding flow or adding callout boxes. Find where the behavior
  is already described and add the minimum accurate statement there.

### DOC tickets auto-generated by agents may be wrong

DOC tickets with the label `docops-auto` are created automatically by a doc-ops agent, not
by a human reporter. The agent reads a dev ticket and infers what documentation changes are
needed — it can be wrong, incomplete, or overeager.

**Before making any change, apply your own judgment:**

1. Read the source dev ticket (linked in the DOC ticket description).
2. Ask: does this change produce information a customer actually needs while following the article?
3. Ask: is this already communicated by the product UI, portal form, or existing documentation?
4. Ask: would the customer notice or be blocked if we said nothing?

If the answer to 2–4 is "no" — do not make the change. Report it as not needed and explain why.

Self-explanatory UI behavior (form auto-correction, inline validation messages, labels shown
next to fields) does not require documentation. The portal already communicates it.
A dev ticket marked Done and fixing a backend bug does not require documentation.
Do not document what the UI already says.
