...
- Replaced `floating_ip = { source = "new" }` block with `gcore_cloud_floating_ip` resource using `port_id` and `fixed_ip_address` referencing the LB's computed attributes.
