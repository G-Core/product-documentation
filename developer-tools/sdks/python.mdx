...
Never hardcode credentials in source files. Store the three variables in a `.env` file and load them with [python-dotenv](https://pypi.org/project/python-dotenv/).
...
async def main():
    async with AsyncGcore() as client:
