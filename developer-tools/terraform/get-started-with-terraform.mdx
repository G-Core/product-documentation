...
<Tabs>
  <Tab title="Windows">
    The fastest way to install on Windows is with [Chocolatey](https://chocolatey.org). Run the following from an elevated PowerShell session:
    ...
    If Chocolatey is not available, download the binary manually:
    ...
    4. Add that folder to the PATH:
       ...
       - Click **OK** to save.
    5. Open a new PowerShell window and run `terraform version` to verify.
  </Tab>
  <Tab title="macOS">
