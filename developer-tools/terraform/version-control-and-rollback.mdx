...
ai-navigation: Integrate Terraform and Git for infrastructure version control, using Git branches, commits, tags, and rollback procedures to revert infrastructure changes via git checkout and terraform apply commands.
---

Integrate Terraform and Git to get robust versioning and rollback capabilities for your infrastructure configurations. This setup ensures better stability as you can quickly revert any changes with minimal downtime and fewer potential issues. 
...
To proceed with the following steps, you need to configure [API keys](/account-settings/api-tokens#create-a-permanent-api-token) and install both [Git](https://git-scm.com/) and [Terraform](/developer-tools/terraform/get-started-with-terraform) on your machine. 
...
```
git add . 
git commit -m "Updated resource configuration" 
```
