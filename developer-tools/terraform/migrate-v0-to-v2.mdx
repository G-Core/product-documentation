...
<Steps>
  <Step title="Back up the state">
    Back up the state before making any changes. If something goes wrong during migration, restoring this backup returns Terraform to its v0 state without touching the actual infrastructure.
    ...
    <Tabs>
      <Tab title="Local state">
        ...
        To restore, copy the backup back over the active state file:
      ...
      <Tab title="Remote backend (S3, Terraform Cloud, and similar)">
        Pull the current state to a local file:

        ```bash
        terraform state pull > terraform.tfstate.v0backup
        ```

        Record the `lineage` and `serial` values from the file — these identify the exact state version. Store the backup outside the repository and outside version control — the state file contains sensitive values: secrets, tokens, and computed credentials.

        To restore after a partial migration, push the backup back to the remote backend:

        ```bash
        terraform state push terraform.tfstate.v0backup
        ```

        `terraform state push` checks the serial number to prevent overwriting a newer state. If the push is rejected, pass `-force` only after confirming the backup is the correct version to restore.
      </Tab>
    </Tabs>
  </Step>
  <Step title="Remove resources from state">
    ...
    <Info>
    No Gcore API calls are made and no infrastructure is affected. `terraform state rm` modifies only the Terraform state — local file or remote backend — not the actual resources.
    </Info>
  </Step>
  <Step title="Update providers.tf">
    ...
    **v2:**
    ...
    <Info>
    The latest v2 release is listed on the [Terraform&nbsp;Registry](https://registry.terraform.io/providers/G-Core/gcore/latest).
    </Info>
  ...
  <Step title="Update resource blocks in .tf files">
    ...
    All cross-resource references must reflect the rename — a subnet that referenced `gcore_network.main.id` now references `gcore_cloud_network.main.id`.
  </Step>
  <Step title="Run terraform init -upgrade">
    ...
    - Installed g-core/gcore v2.0.0 (signed by a HashiCorp partner, key ID 7DC84F37C1A1C035)
    ...
    Terraform has been successfully initialized!
    ```
  </Step>
  <Step title="Import each resource under v2">
