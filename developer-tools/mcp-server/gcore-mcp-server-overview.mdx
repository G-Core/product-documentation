...
ai-navigation: Deploy Gcore MCP Server to expose CDN, Cloud, DNS, FastEdge, IAM, Security, Storage, Streaming, and WAAP API operations as callable tools for AI assistants via Model Context Protocol, supporting Claude Code, Cursor IDE, and MCP Inspector clients with automatic tool schema loading and multi-step workflow chaining.
---
...
FastEdge also has a separate plugin and MCP server dedicated to scaffolding, deploying, and testing edge applications from Claude Code or Codex, documented in [AI-assisted&nbsp;development](/fastedge/local-development/ai-assisted-development).
...
<Frame>![Claude Code planning a VM creation: it resolved region, flavor, and image automatically and asked for confirmation before proceeding](/images/docs/developer-tools/mcp-server/gcore-mcp-server-overview/claude-code-create-vm.png)</Frame>

## Get started

To set up Gcore MCP Server:

1. [Install and configure Gcore MCP Server](/developer-tools/mcp-server/install-and-configure-gcore-mcp-server) — set up the server with Claude Code or Cursor IDE
2. [Test tools with MCP Inspector](/developer-tools/mcp-server/test-tools-with-mcp-inspector) — explore available tools in a browser interface
3. [Filter tools with GCORE_TOOLS](/developer-tools/mcp-server/filter-tools-with-gcore-tools) — limit exposed tools for specific workflows
