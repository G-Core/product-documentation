...
ai-navigation: Gcore Cloud API write operations return a task ID immediately while the operation runs in the background; poll the task endpoint until the state is FINISHED, then extract the created resource ID from the response.
---
