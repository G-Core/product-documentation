...
Most write operations in the Gcore Cloud API — creating, deleting, or modifying resources — are asynchronous: instead of returning the created resource directly, the API returns a task ID, because the actual operation runs in the background and the result is available only after polling to completion.
