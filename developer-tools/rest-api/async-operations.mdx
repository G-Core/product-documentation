...
**Step 1 — Make the request.** A write call returns a task ID immediately:
...
**Step 2 — Poll the task.** Call `GET /cloud/v1/tasks/{task_id}` repeatedly until the `state` field is no longer `RUNNING`:
...
**Step 3 — Extract the result.** When `state` is `FINISHED`, find the created resource ID in `created_resources`.
...
Polling too frequently wastes rate limit budget. Polling too rarely delays pipelines.
...
Full task management endpoints are in the [Cloud API reference](/api-reference/cloud).
