...
ai-navigation: Create an API token in the Gcore Customer Portal, store it as an environment variable, and verify it works.
---

Every Gcore API request requires an API token. The token goes in the `Authorization` header and works across all products — Cloud, CDN, DNS, Storage, and others.

## Step 1. Create a token

1. Sign in to the [Gcore Customer Portal](https://portal.gcore.com).
2. Click the account icon in the top-right corner and navigate to **Profile**.
3. Select the **API tokens** tab.
4. Click **Create token**.
5. Give the token a name, select a role (use `Administrators` for full access), and set an expiration date if needed.
...
| `Bad token: 29841` | Token value is incomplete | Verify the full token value was copied correctly in Step 1 |
...
When signing in via SAML SSO, the identity provider controls portal access but API tokens operate independently, so revoking a user's SSO access does not invalidate their tokens. To block API access for an SSO user, delete their tokens manually in the [API&nbsp;tokens](/account-settings/api-tokens) section.
