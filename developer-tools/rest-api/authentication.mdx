...
6. Click **Create**. A dialog shows the token value — **copy it now**. It will not be shown again.
...
The full token management guide — including how to share tokens between users and filter by role — is in [API tokens](/account-settings/api-tokens).
