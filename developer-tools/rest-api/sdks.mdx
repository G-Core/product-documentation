...
sidebarTitle: "SDKs"
ai-navigation: Official Python SDK and Go SDK for the Gcore API with installation, client initialization, method naming patterns, built-in task polling, async support, and JavaScript fetch and axios examples for all major API operations.
---
...
The package supports both **synchronous** (`Gcore`) and **asynchronous** ([`AsyncGcore`](#async-and-sync-support)) usage. All examples in this section use the sync client.
...
Cloud API write operations return a task ID. The recommended approach is to use the built-in `_and_poll` convenience methods available on most resources — they create the resource and block until it is ready:
...
Use `Gcore` (sync) for scripts, CLI tools, and sequential automation. Use `AsyncGcore` when running multiple API calls concurrently or integrating with async frameworks such as FastAPI or aiohttp.

Both clients have identical method signatures — the only difference is `await` and the `async with` context manager:
...
The main advantage of the async client is concurrent execution with `asyncio.gather()`. The example below creates three networks in parallel instead of sequentially:
