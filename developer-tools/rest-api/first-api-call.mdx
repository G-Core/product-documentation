...
ai-navigation: Set up an API token, project ID, and region ID, then make the first Cloud API call using curl, Python SDK, Go SDK, or JavaScript.
---
...
```
  ...
  <Tab title="Go SDK">
    ```go
    projects, err := client.Cloud.Projects.List(ctx, cloud.ProjectListParams{})
...
<p>The API returns a list of regions. Each entry looks like:</p>
...
```
  ...
  <Tab title="Go SDK">
    ```go
    regions, err := client.Cloud.Regions.List(ctx, cloud.RegionListParams{
        Limit: gcore.Int(50),
    })
    if err != nil { panic(err) }
...
**Go SDK** — all three values are read automatically by `gcore.NewClient()`, the same way the Python SDK reads them:

```bash
export GCORE_API_KEY="..."             # Read by gcore.NewClient() automatically
export GCORE_CLOUD_PROJECT_ID=1234567  # Read by gcore.NewClient() automatically
export GCORE_CLOUD_REGION_ID=148       # Read by gcore.NewClient() automatically
```
