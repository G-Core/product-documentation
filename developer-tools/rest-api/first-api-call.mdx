...
Three values are needed before any Cloud API call works: an API token, a project ID, and a region ID. This page walks through obtaining all three and ends with a verified working request.

<Info>
**Before starting:** a terminal must be open and `curl` must be installed. If either is unfamiliar, read [API basics](/developer-tools/rest-api/api-basics) first — it covers both in a few minutes.
</Info>

## Step 1. Set up the API token

If the token has not been created yet, follow [Authentication](/developer-tools/rest-api/authentication) to create one in the Customer Portal and copy the value.
...
With these in place, the next step is to create a resource. Follow the [Cloud Via API guides](/cloud/via-api/deploy-vm-from-scratch) to deploy a VM, set up a network, or configure other infrastructure. Resource creation is asynchronous — the response returns a task ID, not the finished resource. [Async operations](/developer-tools/rest-api/async-operations) explains how that works.
