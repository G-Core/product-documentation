...
The Gcore API provides programmatic access to everything available in the Customer Portal. Instead of clicking through a browser, HTTP requests are sent from a terminal or script — and the API returns data in JSON format. This makes it possible to automate deployments, build integrations, and manage infrastructure programmatically.
...
<Warning>
Environment variables are session-scoped — closing the terminal window loses all variables set with `export`. The next session requires setting them again.

To persist variables across sessions on macOS/Linux, add the `export` line to `~/.zshrc` (or `~/.bashrc`) and restart the terminal. On Windows PowerShell, add it to the profile file.
</Warning>

<Warning>
Gcore API tokens contain a `$` character. In bash, `$` inside a double-quoted string starts variable substitution, which silently truncates the token. Escape the `$` with a backslash when setting the variable:
