...
Gcore API calls are HTTP requests sent from a terminal or a script. Before making the first call, three things are needed: a terminal application, the `curl` command-line tool, and an understanding of JSON — the format the API uses for both requests and responses. This page covers all three, plus environment variables and HTTP status codes.
