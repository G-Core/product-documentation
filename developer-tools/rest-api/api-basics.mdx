...
**If new to APIs:** start with Postman — the GUI removes the need to learn curl syntax. Import the Gcore OpenAPI spec and all endpoints are pre-filled.

**If comfortable with the terminal:** curl works everywhere and needs no installation on most systems.

Full setup instructions for each tool are in [Tools](/developer-tools/rest-api/tools).

These guides use curl in all examples because it works on every platform without additional setup.
