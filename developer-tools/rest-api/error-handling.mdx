...
- Submit a [quota request](/cloud/getting-started/request-a-quota-increase) via the Customer Portal
...
The correct header format and fix for each error is in [Authentication](/developer-tools/rest-api/authentication).
...
Not every error is worth retrying. This table shows which errors indicate a fixable request problem versus a transient server condition:
