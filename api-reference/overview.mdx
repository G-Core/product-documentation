...
An [API token](/account-settings/api-tokens#create-a-permanent-api-token) is required — generate one in **Customer Portal → [API tokens](https://portal.gcore.com/accounts/settings/api-tokens)**, then pass it as an `APIKey` value in the `Authorization` header:
