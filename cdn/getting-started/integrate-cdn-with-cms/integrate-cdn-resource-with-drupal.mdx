...
Administrator access to Drupal, Composer, an active CDN resource, and an HTTPS-enabled [custom&nbsp;domain](/cdn/cdn-resource-options/general/create-and-set-a-custom-domain-for-the-content-delivery-via-cdn) are required. The CDN resource must use the Drupal site as its origin and forward query strings so image style derivatives continue to work. Back up the Drupal files and database before installing the module.

<Steps>
  <Step title="Install the CDN module">
    From the Drupal project root, run the command for the installed Drupal version.

    For Drupal 9.4 or Drupal 10, install the stable 4.1 release:

    ```bash
    composer require 'drupal/cdn:^4.1'
    ```

    For Drupal 10.3 or Drupal 11, the compatible 5.0 release is currently an alpha version:
  ...
  <Step title="(Optional) Set file caching">
    Enable the far-future or forever-cacheable files option to generate long-lived, immutable asset URLs.
