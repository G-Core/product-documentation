...
```
    ...
    Replace it with the following code. It preserves `$var_dirs_web['cache']`, so the generated URL follows the configured X-Cart cache path:

```php
if ($_SERVER['HTTPS'] != 'on')
{
    $cacheWebFile = "http://cdn.site.com" . $var_dirs_web['cache'] . '/' . $label . '.' . $md5Suffix . '.' . $type;
}
else
{
