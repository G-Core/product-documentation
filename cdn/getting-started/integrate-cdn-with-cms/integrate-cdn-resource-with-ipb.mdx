---
title: Integrate a CDN resource with IPB
sidebarTitle: IPB
ai-navigation: Configure IP.Board 3.x CDN Settings to serve images, CSS, and JavaScript through a Gcore CDN custom domain.
---

IP.Board (IPB) 3.2 through 3.x can rewrite image, CSS, and JavaScript URLs to a Gcore CDN [custom&nbsp;domain](/cdn/cdn-resource-options/general/create-and-set-a-custom-domain-for-the-content-delivery-via-cdn) through the Admin Control Panel. These steps apply only to IP.Board 3.x with an unmodified Admin Control Panel layout that still shows **CDN Settings** under **General Configuration**. IPS Community Suite 4.x and current Invision Community use different admin paths and have not been verified against this procedure. The [IPS&nbsp;integration](/cdn/getting-started/integrate-cdn-with-cms/integrate-cdn-resource-with-ips) guide covers those product generations.

Create a&nbsp;[CDN&nbsp;resource](/cdn/getting-started/create-a-cdn-resource/create-a-cdn-resource) before starting the integration.

<Warning>
Back up the site files and database before making changes. A manually modified Admin Control Panel layout or a version outside IP.Board 3.x might not match these steps.
</Warning>

Before configuring IP.Board, collect the **custom domain** value to enter in **CDN Settings** — it typically looks like `cdn.site.com`:

<Steps>
  <Step title="Open the CDN resource">
    In the [Gcore&nbsp;Customer&nbsp;Portal](https://portal.gcore.com), navigate to **CDN** > **CDN resources** and open the resource.
  </Step>
  <Step title="Copy the custom domain">
    Note the **custom domain** shown for the resource — this is the hostname to enter in IP.Board **CDN Settings**.
  </Step>
  <Step title="Confirm the DNS CNAME">
    Confirm that the custom domain has a CNAME record pointing to the Gcore target hostname shown in the Customer Portal (`cl-****.gcdn.co`). Do not put the `gcdn.co` hostname in IP.Board.
  </Step>
  <Step title="Confirm SSL">
    Confirm the custom domain has a valid SSL certificate so assets can load over HTTPS.
  </Step>
</Steps>

Then configure IP.Board 3.x:

<Steps>
  <Step title="Open the Admin Control Panel">
    Log in to the IP.Board Admin Control Panel.
  </Step>
  <Step title="Open System">
    Navigate to **System**.
  </Step>
  <Step title="Open System Settings">
    Select **System Settings**.
  </Step>
  <Step title="Open General Configuration">
    Select **General Configuration**.
  </Step>
  <Step title="Enter the custom domain">
    Scroll to **CDN Settings** and enter the custom domain copied from the Customer Portal.
  </Step>
  <Step title="Save the configuration">
    Save the settings.
  </Step>
  <Step title="Confirm the integration">
    Open the browser developer tools (press F12), select the **Network** tab, and refresh the page — static files should now load from the custom domain instead of the original domain.
  </Step>
</Steps>
