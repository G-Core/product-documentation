...
<Tabs>
  <Tab title="Customer Portal">
    ...
    **Tip**
    ...
    d. Enter the **Bucket name**. CDN servers use this value only when calculating the AWS Signature V4 `Authorization` header for the private origin.

    When the bucket name must appear in the request URL, include it in the origin hostname — `my-bucket.s3.amazonaws.com` or `my-bucket.s-ed1.cloud.gcore.lu`. Path-style origins need a [Rewrite](/cdn/cdn-resource-options/rewrite-redirect-requests-from-the-cdn-to-the-origin) rule that prepends `/my-bucket` to the path — `/(.*) /my-bucket/$1` — when request URLs do not already contain that segment.
