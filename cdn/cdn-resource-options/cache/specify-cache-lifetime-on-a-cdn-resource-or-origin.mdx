...
| CACHE: BYPASS | The file was requested directly from the origin, bypassing the CDN cache. This status appears on the first request after a cache purge, or when custom settings force the CDN to skip caching (e.g., certain Cache-Control directives or origin responses (when the origin responds with a Set-Cookie header, the CDN treats each response as a unique object). Subsequent requests should return `CACHE: HIT` once the content is re-cached. If `BYPASS` persists across multiple requests, check your CDN caching settings and the origin's Cache-Control headers. |
