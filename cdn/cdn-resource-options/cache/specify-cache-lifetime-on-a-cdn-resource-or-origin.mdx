...
CDN caching applies only to responses with status codes 200, 206, 301, and 302. Responses with 4xx and 5xx status codes are not cached by default. To cache additional codes — such as 404 — configure an advanced caching rule under the **Enable CDN caching via advanced caching rules** tab below.
</Warning>
...
X-ID: ab-cd-d123 | Identifier of the Gcore CDN edge server that delivered the file. Resolve `{x-id}.fe.gc.onl` using `nslookup` or `dig` to get the server's IP address for sending repeat requests to the same edge node — useful for [cache diagnostics](/cdn/troubleshooting/cache-percentage-is-low-how-to-solve-the-issue).  
