---
title: "Сache Sharding: share cache between CDN servers"
sidebarTitle: Cache Sharding
ai-navigation: Enable Cache Sharding to distribute cached content across CDN servers within a PoP using consistent hashing, reducing origin requests and improving cache hit
ai-navigation: Enable Cache Sharding on CDN resources to share cache between servers in same PoP using consistent hashing algorithm.
---
...
In a system without Cache Sharding, a CDN server only checks _its own_ cache for a file request. This is inefficient when there are many servers in one PoP: if a server _without_ the file gets a request, it retrieves a file from the origin instead of a neighboring server.

When Cache Sharding is enabled for a CDN resource, CDN servers within one PoP (or data center) are clustered to use each others' cache with the [consistent hashing algorithm](https://toptal.com/big-data/consistent-hashing#:~:text=according%20to%20Wikipedia\).-,Consistent%20Hashing%20is%20a%20distributed%20hashing%20scheme%20that%20operates%20independently,without%20affecting%20the%20overall%20system).
...
<Frame>![How does the Cache Sharding feature work](/images/docs/cdn/cdn-resource-options/cache/cache-sharding-share-cache-between-cdn-servers/cache-sharding-scheme.png)</Frame>
...
[Newly created CDN resources](/cdn/getting-started/create-a-cdn-resource/create-a-cdn-resource-for-only-static-files) have Cache Sharding enabled automatically. If you're a long-term user unsure whether the feature is activated, please contact our [Gcore Support](mailto:support@gcore.com) to request activation.

<Note>
**Note**

The Cache Sharding option is not used for [Streaming CDN resources](/streaming/how-the-streaming-platform-interact-with-the-cdn).
</Note>
