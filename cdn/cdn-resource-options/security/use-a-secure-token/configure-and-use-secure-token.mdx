...
The path is important. The CDN validates the token against the path used in the hash string. Two modes exist:

- **Exact path**: the token is generated for a specific file path (e.g. `/images/poster.jpeg`). The CDN recomputes the hash using the requested path and rejects any request where the paths do not match exactly.
- **Directory path**: when the path ends with `/` (e.g. `/videos/video-1/`), the CDN uses prefix matching — any request whose path starts with that prefix is accepted with the same token. This is a property of Secure Token itself, not of Query String Forwarding.

[Query&nbsp;String&nbsp;Forwarding](/cdn/cdn-resource-options/query-string-forwarding) is a separate mechanism: it copies `md5` and `expires` from playlist body links to the actual media segment requests. This allows a video player to receive the token with the master manifest and have the CDN automatically propagate it to all nested requests — but each of those requests is still validated by Secure Token using directory prefix matching against the original path.
