...
<Tabs>
  ...
  <Tab title="Non-Amazon storage">
    ...
    7\. Specify a hostname—a name that's assigned to a storage server within a network and is used instead of an IP address. For Gcore Object Storage, the hostname is in **Details**.

    8\. Specify the access key ID. A secret access key is also required to configure log export. For Gcore Object Storage, create the key in [access keys](/storage/manage-object-storage/manage-s3-access-keys).

    9\. Specify the secret access key. For Gcore Object Storage, copy the secret when the key is created.
