...
ai-navigation: Enable Raw Logs to automatically export CDN resource logs containing user requests to cache servers and Origin Shielding pre-cache servers to S3, FTP, or SFTP storage, with configurable export intervals, status monitoring (Pending, OK, Failed, Pause), and optional origin shielding log inclusion.
---
...
The final value of log data may differ slightly from the billing statistics as there may be cases where not all logs are received, such as: * You are using the [Origin Shielding](/cdn/cdn-resource-options/general/enable-and-configure-origin-shielding) feature but didn't select the **Add logs from origin shielding** option. * You have a rate limit set on your storage side, and when CDN started generating logs, some logs weren't downloaded because of the rate limitation.
</Info>
