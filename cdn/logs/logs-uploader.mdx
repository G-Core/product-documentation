---
title: "Logs Uploader"
ai-navigation: Configure CDN Logs Uploader to export request and cache logs to Object Storage, S3, FTP, SFTP, HTTP, or Alibaba SLS destinations.
---
...
<Note>
WAAP fields are added only when WAAP is enabled on the CDN resource. Confirm this in the [Gcore Customer Portal](https://portal.gcore.com/cdn/resources/list): on **CDN** > **CDN resources**, open the resource **Security** settings and check that the **WAAP** toggle is on, or confirm that the resource **WAAP Status** is Active. For resources where WAAP is not enabled, the CDN log line is unchanged.
</Note>
...
**HTTP(S) target not returning 2xx**

Logs Uploader treats any non-`2xx` response from an HTTP(S) endpoint as a delivery failure and retries. Ensure the receiver endpoint is running, returns `200 OK` or another `2xx` status, and does not time out before processing the request body. If compression is enabled on the target, the receiver must accept gzip-encoded bodies (`Content-Encoding: gzip`).

**Configuration not linked to traffic-generating resources**
