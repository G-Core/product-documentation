  ...
  <Tab title="Policies">
    <AccordionGroup>
      ...
      <Accordion title="Edit policies">
        ...
        - Edit the **Delimiter** and the **Separator**.
  ...
  <Tab title="Targets">
    <AccordionGroup>
      ...
      <Accordion title="Add targets">
        - Navigate to **Logs uploader**, select the **Targets** tab, and click **Add target**.
...
<AccordionGroup>
  ...
  <Accordion title="Log fields">
    ...
    <Note>
      Two commonly requested fields are not available in CDN logs. No unix epoch timestamp field exists — `$timestamp` is an internal variable that always produces an empty value; use `[$time_local]` for time-based analysis and convert the Common Log Format timestamp in the processing pipeline if needed. No client port field is available.
    </Note>
...
| `$geoip_country_code`                                         | `KZ`                                                                                                                                               | User’s country code according to the [ISO&nbsp;3166](https://iso.org/obp/ui/#search/code) (Alpha-2 code)                                                                                                                                                                                                                                                                                                                                                   |
...
| `$server_country_code`                                        | `PL`                                                                                                                                               | Server’s country code according to the [ISO&nbsp;3166](https://iso.org/obp/ui/#search/code) (Alpha-2 code)                                                                                                                                                                                                                                                                                                                                                 |
...
| `$gcdn_host`<br />(internal system variable - To be deprecated after migration) | &nbsp;                                                                                                                                             |                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `$waap_request_type`               | `regular`                       | WAAP request classification (`regular`, `injected`).                                                                                        |
  ...
  <Accordion title="Log schema versioning">
    Each exported log line carries a schema version field that identifies the JSON payload version (for instance `"v":"10"`). The version is incremented whenever fields are added, renamed, or removed. New fields are added in a backwards-compatible way where possible (existing field names and positions are preserved), so most schema changes do not require parser updates. Breaking changes are announced in advance through the standard CDN update notifications. Pin downstream parsers (SIEM, ETL) to a known version and check the `v` value to detect schema changes safely.
  </Accordion>
</AccordionGroup>
