---
title: "Logs Uploader"
ai-navigation: Configure CDN Logs Uploader to export request and cache logs to Object Storage, S3, FTP, SFTP, HTTP, or Alibaba Cloud Simple Log Service (SLS) destinations.
---
...
Accounts that already use the hourly [Raw Logs](/cdn/logs/raw-logs-export-cdn-resource-logs-to-your-storage) export to Amazon Simple Storage Service (S3), File Transfer Protocol (FTP), or SSH File Transfer Protocol (SFTP) can keep that configuration. Use Logs Uploader for new setups. To migrate an existing Raw Logs export, contact [Gcore&nbsp;Support](mailto:support@gcore.com).
...
Control which fields are exported, how frequently the logs are delivered, and which storage service they are sent to. Supported destinations include Gcore Object Storage, Amazon Simple Storage Service (S3)–compatible storage, File Transfer Protocol (FTP) and SSH File Transfer Protocol (SFTP) servers, HTTP(S) endpoints, and Alibaba Cloud Simple Log Service (SLS). Use these exports for observability, analytics, or compliance workflows while managing data volume and processing requirements.
...
<Tabs>
  <Tab title="Configurations">
    <AccordionGroup>
      ...
      <Accordion title="Configuration creation">
        <Steps>
      ...
      <Accordion title="Configuration status and deletion">
        <Steps>
  ...
  <Tab title="Policies">
    <AccordionGroup>
      ...
      <Accordion title="Policy creation">
        <Steps>
      ...
      <Accordion title="Policy editing">
        <Steps>
      ...
      <Accordion title="Policy deletion">
        <Steps>
  ...
  <Tab title="Targets">
    <AccordionGroup>
      ...
      <Accordion title="Target creation">
        <Steps>
          ...
          <Step title="Configure the destination">
            ...
            <Tabs>
              ...
              <Tab title="SLS">
                ...
                1. Enter the **Region** for the SLS target (for example, `cn-hangzhou`).
                2. Enter the **Project** name (3–63 characters; lowercase letters, digits, and hyphens only).
                3. Enter the **Logstore** name (3–36 characters; lowercase letters, digits, hyphens, and underscores only).
                4. Enter the **Access Key ID** and the **Secret Access Key**.
                5. Optionally, enter the **Endpoint** to override the default (`region.log.aliyuncs.com`).
                6. Optionally, enter a **Topic** (up to 128 characters).

                <Info>
                  Alibaba documents [SLS&nbsp;AccessKey](https://www.alibabacloud.com/help/en/sls/developer-reference/access-key) pair creation, and project and logstore setup in the [SLS&nbsp;quick&nbsp;start](https://www.alibabacloud.com/help/en/sls/getting-started/getting-started-with-simple-log-service).
                </Info>
              </Tab>
              <Tab title="S3 Other">
      ...
      <Accordion title="Target editing">
        <Steps>
      ...
      <Accordion title="Target deletion">
        <Steps>
...
| `$gcdn_finalize_reason`                                        | `ok`                                                                                                                                               | Why the request stopped producing a body. `ok` means the body finished. Empty or `unset` means no reason was recorded. Other values identify the cause: client (`client_closed`, `client_timeout`, `client_error`); upstream (`upstream_connect_failed`, `upstream_connect_timeout`, `upstream_headers_null`, `upstream_header_timeout`, `upstream_body_null`, `upstream_body_partial`, `upstream_body_timeout`, `upstream_ssl_error`, `upstream_error`, `upstream_send_timeout`); this proxy (`internal_error`, `shutdown`); or slice (`slice_bad_status`, `slice_missing`, `slice_etag`, `slice_range`). |
  ...
  <Accordion title="Log schema versioning">
    ...
    The version increments when fields are added, renamed, or removed. New fields are added in a backward-compatible way where possible, so most schema changes do not require parser updates. Breaking changes are announced in advance through standard CDN update notifications. Make downstream Security Information and Event Management (SIEM) and extract, transform, load (ETL) parsers version-aware, check the `v` value, and reject or flag unsupported schema versions.
...
<Note>
WAAP fields are added only when WAAP is enabled on the CDN resource. Confirm this in the [Gcore Customer Portal](https://portal.gcore.com/cdn/resources/list). The **WAAP Status** column on the **CDN** > **CDN resources** page shows Active when WAAP is enabled. The **WAAP** toggle is in that resource's **Security** settings. For resources where WAAP is not enabled, the CDN log line is unchanged.
</Note>

<Note>
WAAP fields are included in the next exported batch after WAAP is enabled. No Logs Uploader reconfiguration is required. Existing configurations and policies automatically start receiving the additional fields. Because new fields are appended at the end of each log line, positional parsers based on existing CDN fields continue to work without changes. Parsers that validate a fixed schema or reject unknown columns must be updated to accept the WAAP fields before WAAP is enabled, or they will encounter parse errors. The `v` field in HTTP(S) payloads increments when the schema changes; check it in downstream parsers to detect schema updates.
</Note>
...
This section covers delivery failures after a target or configuration is saved, including cases where authentication succeeds but no log file arrives.

### Missing files after successful authentication

A target showing `Authentication complete` confirms that Logs Uploader can reach and authenticate to the destination. It does not guarantee that log files are being delivered. Check the following if files are not appearing:

#### Delivery interval timing

Files are delivered at the end of each **Time interval** configured in the linked policy (minimum 5 minutes). Wait at least one full interval after CDN traffic has been generated before expecting the first file.

#### Missing CDN traffic

If the CDN resources covered by the configuration receive no requests during an interval and **Include empty logs** is not selected, no file is sent. Enable **Include empty logs** in the policy to confirm delivery is working even when traffic is absent.

#### Missing destination path

For S3, FTP, SFTP, and SLS targets, the bucket, directory, or logstore specified in the target must already exist. Logs Uploader does not create missing paths. Verify that the destination path was created in the storage service before expecting files.

#### Insufficient write permissions

The credentials used by the target must have write permission to the destination path. For S3 targets, the access key must have `s3:PutObject` (and `s3:GetBucketLocation` for some providers) on the target bucket. For FTP and SFTP targets, the login must have write access to the prepend directory. For SLS targets, the access key must have `Write` permission on the target logstore.

#### Non-2xx HTTP(S) response

Logs Uploader treats any non-`2xx` response from an HTTP(S) endpoint as a delivery failure and retries. The receiver endpoint must be running, return `200 OK` or another `2xx` status, and not time out before processing the request body. If compression is enabled on the target, the receiver must accept gzip-encoded bodies (`Content-Encoding: gzip`).

#### Configuration resource scope

A configuration set to **Specific Resources** only delivers logs for the CDN resources explicitly listed. Verify that the resources generating traffic are included in the configuration. Configurations set to **All Resources** cover all resources, including newly created ones.

#### Delivery error details

Delivery error details are not currently displayed in the portal. Contact [Gcore&nbsp;Support](mailto:support@gcore.com) to request delivery logs for a specific target or time range.
