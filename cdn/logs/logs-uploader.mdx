...
| `$gcdn_finalize_reason`                                        | `ok`                                                                                                                                               | Why the request stopped producing a body. `ok` means the body finished. Empty or `unset` means no reason was recorded. Other values identify the cause: client (`client_closed`, `client_timeout`, `client_error`); upstream (`upstream_connect_failed`, `upstream_connect_timeout`, `upstream_headers_null`, `upstream_header_timeout`, `upstream_body_null`, `upstream_body_partial`, `upstream_body_timeout`, `upstream_ssl_error`, `upstream_error`, `upstream_send_timeout`); this proxy (`internal_error`, `shutdown`); or slice (`slice_bad_status`, `slice_missing`, `slice_etag`, `slice_range`). |
  ...
  <Accordion title="Log schema versioning">
    ...
    The version increments when fields are added, renamed, or removed. New fields are added in a backward-compatible way where possible, so most schema changes do not require parser updates. Breaking changes are announced in advance through standard CDN update notifications. Make downstream parsers (SIEM, ETL) version-aware, check the `v` value, and reject or flag unsupported schema versions.
...
<Note>
WAAP fields are added only for resources that have WAAP enabled and use the current edge protection architecture — the inline WAAP integration available to all Gcore CDN resources created or updated after WAAP was introduced. If you are unsure whether a specific resource uses this architecture, contact [Gcore&nbsp;Support](mailto:support@gcore.com). For resources without WAAP, the existing CDN log line is unchanged.
</Note>

<Note>
WAAP fields begin appearing in logs immediately after WAAP is enabled on the resource — no Logs Uploader reconfiguration is required. Existing configurations and policies automatically start receiving the additional fields. Because new fields are appended at the end of each log line, positional parsers based on existing CDN fields continue to work without changes. Parsers that validate a fixed schema or reject unknown columns must be updated to accept the WAAP fields before WAAP is enabled, or they will encounter parse errors. The `v` field in HTTP(S) payloads increments when the schema changes; check it in downstream parsers to detect schema updates.
</Note>
...
### Authentication complete, but no files are received
