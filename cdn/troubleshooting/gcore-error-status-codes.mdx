...
HTTP status codes in CDN logs show the outcome of a request. Several causes can produce the same code, so each table lists the usual Gcore-specific causes and the matching fix.
...
Unlike 403, which is an access decision on the CDN, 404 can mean the origin has no object at that path. Some CDN options also return 404 when the origin fetch fails.
...
The remaining codes each have a primary cause. A 504 means the CDN did not get a timely origin response. A blocked origin ACL is one cause. Timeouts, an unreachable origin, or origin overload can produce the same code.
...
| 504 | The origin did not respond in time. | The CDN timed out waiting for the origin. A blocked origin ACL is one cause. An unreachable origin, a slow origin, or origin overload can also return 504. | Confirm the origin is reachable from CDN. If an ACL blocks Gcore addresses, apply the origin ACL or Request headers fix from the 404 table. |
...
HLS media playlists often request MPEG Transport Stream segments with a `.ts` extension, so Gcore treats `.ts` as video delivery. Those requests can return `402` until streaming is enabled. The same extension is also used for TypeScript source files. If TypeScript files return `402`, rename or build them to JavaScript (`.js`) before delivery, or contact [Gcore&nbsp;Support](mailto:support@gcore.com) to review the CDN resource configuration.
