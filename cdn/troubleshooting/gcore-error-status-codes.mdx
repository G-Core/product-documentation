...
| 508 | The CDN hostname and origin form a loop. | A CNAME that points the origin and the CDN hostname at each other sends requests back and forth. Example: a CNAME that uses the origin domain instead of the CDN hostname from [resource&nbsp;setup](/cdn/getting-started/create-a-cdn-resource/create-a-cdn-resource). | Fix the custom domain CNAME in [content&nbsp;unavailability](/cdn/troubleshooting/content-is-unavailable-after-a-cdn-resource-creation-how-to-solve-the-issue#1-check-the-configuration-of-the-personal-domain). Confirm the CDN hostname CNAME matches the value in the Customer Portal. |
