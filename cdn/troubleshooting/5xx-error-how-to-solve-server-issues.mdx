...
ai-navigation: Diagnose and resolve HTTP 5xx errors on CDN edge servers and origin servers by checking Gcore Status Page for incidents, verifying origin availability and response times, whitelisting CDN edge server IPs in origin ACL, and configuring correct origin pull protocol and Host Header settings in CDN resource options.
---
...
If you use Cloudflare or CloudFront services as an origin server, select `HTTP` as the origin pull protocol. If your origin only supports HTTPS, or it has a redirect from HTTP to HTTPS, contact our [Gcore Support](mailto:support@gcore.com) to add settings. 
...
To receive detailed data about error responses, use the [Log viewer](/cdn/logs/log-viewer-view-and-download-cdn-resource-logs) — a self-serve tool available directly in the Customer Portal. For long-term log export and custom analysis, Raw logs is a paid option; see the [Raw logs article](/cdn/logs/raw-logs-export-cdn-resource-logs-to-your-storage) for setup and field specifications. To activate Raw logs, contact your account manager or [Gcore Support](mailto:support@gcore.com).
