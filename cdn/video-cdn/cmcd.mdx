...
Gcore processes query-string CMCD. Header processing still requires the special request above.

In the table, Query and Header show which transport methods each player can send. Supported means the player implements that method. Gcore processes headers only after that special request.
...
| Shaka Player | Supported | Supported | v1 | [Shaka](https://shaka-player-demo.appspot.com/docs/api/tutorial-config.html). |
...
The examples below cover selected players from the table. JW Player and Roku are listed as supported; use each vendor's CMCD documentation for those products. The examples use these default CMCD values:
