...
ai-navigation: Integrate Terraform and Git for CDN configuration versioning, rollback procedures, and infrastructure change management with commit history and tagged releases.
ai-navigation: Configure Terraform and Git version control with rollback to previous CDN configurations using tags and commit history.
---
...
To proceed with the following steps, you need to configure [API keys](/account-settings/api-tokens#create-a-permanent-api-token) and install both [Git](https://git-scm.com/) and [Terraform](/cdn/terraform/install-and-configure-terraform) on your machine. 
