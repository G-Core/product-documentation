...
<Note>
Every option object uses two fields: `enabled` determines whether the option is configured on the resource (`true` = active, `false` = present but inactive), and `value` holds the actual setting (a boolean, string, or list depending on the option). Setting an option to `null` removes it from the CDN resource entirely and reverts to account-level defaults. Options not listed in the `options` map are not modified.
</Note>
...
| `http3_enabled` | Enable HTTP/3 (QUIC). Requires `ssl_enabled = true`. Set `value = true`. |
...
HTTP/3 (QUIC) improves delivery for clients that support it. The option requires SSL to be enabled on the CDN resource — configure `ssl_enabled` and `ssl_data` before activating HTTP/3.
...
To disable HTTP/3 without removing the option, set `value = false` while keeping `enabled = true`. The same setting is available in the Customer Portal under the **HTTP/3** option in the **General** section of CDN resource settings.
