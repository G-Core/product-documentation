...
- (optional) Specify `field_remap` — a map that renames exported log fields. Each key must be a field name already present in `fields`; the value is the name it carries in the exported log. All output names must be unique.

```hcl
resource "gcore_cdn_logs_uploader_policy" "renamed_fields" {
  name   = "my-log-policy"
  fields = ["timestamp", "clientIP", "uri", "status", "responseSize"]
