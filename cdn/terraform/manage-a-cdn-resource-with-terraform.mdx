...
ai-navigation: Create and manage CDN resources including origin groups with Terraform using gcore provider, configuring active and backup origins, upstream failover behavior, resource options, CDN rules, and SSL certificates via main.tf and terraform plan/apply commands.
---
...
For detailed guidelines on how to create and manage CDN resources, configure resource options, create rules, and add SSL certificates, check out the following sections.
...
Open the _main.tf_ file where you [configured the Gcore provider for Terraform](/developer-tools/terraform/get-started-with-terraform).
...
If a CDN resource was created via Terraform, you can add a [rule](/cdn/cdn-resource-options/rules-for-particular-files/create-a-rule-manually-or-from-a-template-to-configure-settings-for-particular-files) to it via Terraform. Use the following steps.

### Step 1. Open the configuration file

Open the _main.tf_ file. It should contain the code for the creation of your CDN resource. If it is missing, add the code according to the [Create a new CDN resource](#create-a-new-cdn-resource) section.

<Tip>
**Tip**

Terraform will not duplicate a resource. It requires the code used to create a resource only to identify a resource that should be changed.
</Tip>
...
If you want to add options to the rule, paste the code below. Replace the hints in the brackets with your values and remove the brackets.
...
If you don't need options, skip this step and do not add the _options_ block.

### Step 4. Close the resource block

Add a curly bracket on a new line:
...
Save the changes in the configuration file. In the "Terraform" folder in the command-line interface, run `terraform plan` — it will show what changes Terraform is going to make. If the code contains an error, the output will give a brief description of it.

Run `terraform apply` — it will make changes to the CDN. Terraform will ask you to confirm the action — enter "yes".

Here is an example of a configuration file with a rule that has the following parameters:
  ...
  * The "WebP Compression" option with a final quality of 66
...
If a CDN resource was created via Terraform, you can add an SSL certificate via Terraform. Use the following steps.
...
### Step 2. Add the certificate resource

Add the code below before the `resource "gcore_cdn_resource"` block. Replace the hints in the brackets with your values and remove the brackets.
...
### Step 3. Reference the certificate in the CDN resource

Add the following lines after the _secondary_hostnames_ string, if present; otherwise, after _origin_protocol_. Replace the hints in the brackets with your values and remove the brackets.
...
Here is an example of a configuration file with an SSL certificate with the following parameters:

  * example_cert — the name of the certificate in Terraform
  * example_certificate — the name of the certificate in the Gcore Customer Portal
  * Public key: `-----BEGIN CERTIFICATE-----MIIDkjCCAnqgAwIBAgIgTfqoZeTGCEvm...-----END CERTIFICATE-----`
  * Private key: `-----BEGIN RSA PRIVATE KEY-----MIIEpAIBAAKCAQEAzzj54zBOWxBIJRFMBtG...-----END RSA PRIVATE KEY-----`
