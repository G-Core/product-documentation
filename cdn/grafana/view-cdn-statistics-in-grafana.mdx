...
4. Configure the connection fields:
   ...
   - **API URL** — leave empty to use the default host, or enter a hostname only with no scheme or path — `api.gcore.com`.
...
A successful test confirms authentication with the account name. If the test fails, verify the token and that the API URL is a hostname only.
