...
Enable **Use next upstream** only when every origin contains the exact same content. When enabled, Gcore CDN retries the request on the next available origin after connection errors, timeouts, invalid headers, or response codes 403, 404, 429, 500, 502, 503, or 504. If no origin is available, the CDN serves the last origin's response.
...
| **Use next upstream** is enabled; one origin is active and the rest are backups | After connection errors, timeouts, invalid headers, or response codes 403, 404, 429, 500, 502, 503, or 504, the CDN tries the remaining origins in list order |
| **Use next upstream** is enabled; all origins are active | After connection errors, timeouts, invalid headers, or response codes 403, 404, 429, 500, 502, 503, or 504, the CDN tries the remaining origins in list order |
