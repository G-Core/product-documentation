...
Enable **Use next upstream** only when every origin contains the exact same content. Gcore CDN tries fetching content from the next available origin(s) after receiving any 4xx or 5xx responses (except status code 400); if no origin is available, the CDN serves the last origin's response.
