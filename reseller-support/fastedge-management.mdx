...
You can save the preferred filters and columns displayed in the applications and templates tables. For instructions on how to configure and save presets, check out the [Search presets](/reseller-support/new-admin-panel-interface#search-presets) section.
...
* **Upgradable**: If the column shows **Outdated**, this means that the application was created from a template and is using an old version of the binary. In this case, it’s best to [update the application](/reseller-support/fastedge-management#upgrade-a-binary-version) to the newest version.  
...
* **Group ID**. A unique identifier for the default group that’s associated with a single reseller and includes all customers created by that reseller. You can create and manage groups via [API](/api-reference/fastedge-resellers/groups/list-groups).
