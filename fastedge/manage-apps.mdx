...
<p>All management operations use the <code>https://api.gcore.com/fastedge/v1/apps/{id}</code> endpoint, where `id` is the numeric application identifier. Retrieve all application IDs with <code>GET /fastedge/v1/apps</code>. The same ID is used as <code>app_id</code> in Terraform — using a placeholder value results in a "Provided app ID does not exist" error.</p>
