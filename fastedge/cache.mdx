...
**Coalesced population**: A `getOrSet` operation prevents duplicate work within a single WASM instance — concurrent callers for the same key share one populator execution.
...
| Atomic counters | Yes (`incr`, `decr`) | No |
