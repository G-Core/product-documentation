...
1\. Upload the Wasm binary to our edge servers by running the following [API request](/api-reference/fastedge/binaries/store-compiled-wasm-binary) from the repo's root directory. Insert your [permanent API token](/account-settings/api-tokens) instead of the api_key:
