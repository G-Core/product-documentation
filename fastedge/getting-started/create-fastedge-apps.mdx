...
1\. Upload the Wasm binary to our edge servers by running the following [API request](https://api.gcore.com/docs/fastedge#tag/Binaries/operation/storeBinary) from the repo's root directory. Insert your [permanent API token](/account-settings/api-tokens) instead of the api_key:
