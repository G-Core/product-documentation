...
The Modern Rust SDK is based on [WASI-HTTP](https://github.com/WebAssembly/WASI), the standard WebAssembly interface for HTTP applications. It uses the `wstd` crate, which provides Rust bindings for WASI-HTTP, and is the recommended approach for new FastEdge HTTP applications.
