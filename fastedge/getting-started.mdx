...
FastEdge tracks request calls at three levels simultaneously: hourly, daily (UTC midnight), and monthly (UTC calendar month). Exceeding the limit at any one level automatically disables the application until that window resets — consumption is not measured against a single billing cycle alone.

When a limit is exceeded, the application status changes to `3` (hourly limit exceeded) or `4` (daily limit exceeded) and stops serving traffic until that window resets. Other per-plan resource limits — execution time, memory, and outbound requests — are listed at https://gcore.com/docs/fastedge/troubleshooting.
...
| `monthly_consumption` | Calls across all apps during the current calendar month (UTC) |
