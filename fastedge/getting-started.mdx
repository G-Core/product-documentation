...
FastEdge addresses three operational constraints common to edge workloads: latency, server management, and isolation.
...
Each plan includes a monthly call allowance tracked at the account level across all applications; per-plan values are on the [FastEdge pricing page](https://gcore.com/pricing/edge-network#fastedge). When the allowance is exceeded, Gcore charges for overage or suspends the account depending on the plan, rather than disabling individual applications.
...
FastEdge is suited to scenarios where latency, origin load, or traffic routing are the primary constraints.
