...
<p>Volumes provide persistent block storage for GPU cluster nodes. Data remains available when a cluster is powered off or rebuilt, so volumes can store training datasets, model checkpoints, and job outputs.</p>
...
<p>GPU Cloud [volumes](/api-reference/cloud/volumes) keep data independently of cluster nodes. Attach them to nodes that need persistent storage, and detach or reuse them without affecting the stored data.</p>
