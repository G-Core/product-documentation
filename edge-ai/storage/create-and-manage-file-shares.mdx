...
<p>For data that needs to be accessed by multiple nodes simultaneously, use a VAST file share. VAST provides high-performance shared NFS storage for datasets, checkpoints, and distributed outputs.</p>
...
<p>Size can only be increased. The file share can be resized while it is mounted.</p>
...
<p>Increase the size of an existing file share. Size can only be increased. The file share can be resized while it is mounted.</p>
...
<p>Edit `size` in the resource block and run `terraform apply`. Size can only be increased. The file share can be resized while it is mounted.</p>
