...
sidebarTitle: Create a GPU Kubernetes cluster
---
...
Unlike traditional Kubernetes deployments that require manual control plane management, Managed Kubernetes handles the control plane automatically. This allows focus on deploying and scaling AI/ML workloads rather than cluster administration.
...
Available regions include Chester, London-2, Luxembourg-2, Luxembourg-3, Sines-2, and Sines-3. GPU availability and pricing vary by region.
...
<img src="/images/docs/edge-ai/managed-kubernetes/step3-node-types.png" alt="Node pool configuration showing instance type selection and pool settings" />
...
1. Review the **Estimated cost** panel showing monthly and hourly pricing for all resources.

<img src="/images/docs/edge-ai/managed-kubernetes/step9-cost.png" alt="Estimated cost panel with Create Cluster button" />

2. Click **Create Cluster**.

Cluster provisioning typically takes 5-15 minutes depending on the number and type of nodes. The cluster status changes to **Provisioned** when ready.

## Connect to the cluster

After creation, download the kubeconfig file to connect with kubectl.
...
Deleting a cluster removes all nodes and associated resources. Persistent volumes and file shares created outside the cluster are not affected.
