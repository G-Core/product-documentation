...
<p>Retrieve the kubeconfig for an existing GPU Kubernetes cluster using the [`gcore_cloud_k8s_cluster_kubeconfig`](https://registry.terraform.io/providers/G-Core/gcore/latest/docs/data-sources/cloud_k8s_cluster_kubeconfig) data source from the [Terraform&nbsp;provider](/developer-tools/terraform/overview) v2. No infrastructure is created — the data source reads the kubeconfig from the Gcore API on every run.</p>
