...
6. Choose the GPU flavor from the dropdown. Available flavors depend on the region. The table below shows example configurations:

| Region | GPU | GPU Memory | RAM | NVMe Storage |
|--------|-----|------------|-----|--------------|
| Chester | 8x H200 | 80 GB each | 2 TB | 2x960 GB + 6x3.84 TB |
| Helsinki, Incheon, London-2, Sines-2, Sines-3 | 8x H100 | 80 GB each | 2 TB | varies |
| Luxembourg-2, Luxembourg-3 | 8x H100 or 8x A100 or 8x L40s | 80 GB (H100/A100), 48 GB (L40s) | 2 TB | varies |
| Manassas, Singapore | 8x L40s | 48 GB each | 2 TB | 2x1 TB + 4x8 TB |

   For the exact specifications and pricing for your region, check the flavor cards in the portal or the [GPU Cloud pricing page](https://gcore.com/pricing/ai). For GPU Bare metal instances, NVMe storage is built in. No separate boot volume configuration is required.
...
<Frame>
  <img src="/images/docs/edge-ai/managed-kubernetes/add-gpu-pool.png" alt="Add GPU pool dialog showing GPU Bare metal instances selection and available GPU flavor options" />
</Frame>
