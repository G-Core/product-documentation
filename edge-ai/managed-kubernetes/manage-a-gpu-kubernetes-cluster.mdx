...
Gcore manages the Kubernetes control plane. Cluster operators are responsible for node pools, workload scheduling, storage configuration, access control, and keeping the cluster version current.

This guide covers the main day-to-day operations: connecting with kubectl, managing node pools and autoscaling, configuring storage and logging, setting up OIDC authentication, upgrading the cluster version, and deleting the cluster when it is no longer needed.
...
The expanded view shows the pool name, autoscaling status, instance flavor, volume configuration, node list, and current status.
...
<Warning>
Node boot volume size cannot be changed after pool creation. To resize storage, create a new pool with the required volume size, migrate workloads, then delete the old pool.
</Warning>
...
<Warning>
Pool deletion is irreversible. All nodes in the pool are terminated. Migrate workloads or confirm they are no longer needed before proceeding.
</Warning>
...
<Info>
The Cluster Autoscaler scales based on CPU and memory utilization only. For GPU utilization-based autoscaling, configure [KEDA (Kubernetes Event-Driven Autoscaling)](https://gcore.com/docs/cloud/kubernetes/clusters/autoscaling/configure-gpu-autoscaling-for-kubernetes).
</Info>
...
Ensure all nodes show the same version as the control plane before starting the upgrade. Review the [Kubernetes changelog](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/) for deprecated APIs that may affect running workloads.

Rolling upgrades require quota for one additional node of the same flavor during the process. For a 10-node pool, quota for 11 nodes is needed temporarily.
...
The Cluster Autoscaler image must match the cluster's Kubernetes minor version. Find the latest release for the new version on the [Cluster Autoscaler releases page](https://github.com/kubernetes/autoscaler/releases), then update the image:
...
The NVIDIA device plugin DaemonSet must be updated after a cluster upgrade to maintain GPU resource scheduling. Find the latest compatible release on the [NVIDIA k8s-device-plugin releases page](https://github.com/NVIDIA/k8s-device-plugin/releases), then apply it:
