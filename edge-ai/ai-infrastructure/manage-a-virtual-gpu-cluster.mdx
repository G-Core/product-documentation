...
ai-navigation: Manage Virtual GPU clusters via Customer Portal, REST API, or Terraform; rename, scale, control power state, manage volumes and interfaces, and delete clusters.
---
...
<p>The list endpoint returns all clusters in a project and region. Query string filters — by name, flavor, node count, tags, and timestamps — are documented in the [API&nbsp;reference](/api-reference/cloud/gpu-virtual). When multiple filters are specified, a cluster must match all of them.</p>
