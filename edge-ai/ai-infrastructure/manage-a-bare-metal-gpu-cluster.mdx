...
ai-navigation: Manage Bare Metal GPU cluster nodes — resize, control power, rebuild, replace, and delete — via Customer Portal, REST API, or Terraform.
---
...
<Info>
The list call returns a page of clusters. Pass `limit` and `offset` to control page size and position.
</Info>
...
<Tabs>
  <Tab title="Python SDK">
    ...
    task_id_list = client.cloud.gpu_baremetal.clusters.action(
        cluster_id="{CLUSTER_ID}",
        action="soft_reboot",
    )
    print(task_id_list.tasks)
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        // Swap OfSoftReboot for OfStart, OfStop, or OfHardReboot as needed
        taskIDList, err := client.Cloud.GPUBaremetal.Clusters.Action(ctx,
            "{CLUSTER_ID}",
...
<p>Pushes updated cluster server settings (image, SSH key, and user data) onto one node or every node. The call re-images the target servers. Set `max_disruption` to `rebuild`. The cluster settings must already be patched before this call.</p>
...
<Tabs>
  <Tab title="Python SDK">
    ...
    # One node
    task_id_list = client.cloud.gpu_baremetal.clusters.servers.apply_settings(
        server_id="{SERVER_ID}",
        cluster_id="{CLUSTER_ID}",
    ...
    task_id_list = client.cloud.gpu_baremetal.clusters.apply_settings(
        cluster_id="{CLUSTER_ID}",
        max_disruption="rebuild",
    )
    print(task_id_list.tasks)
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        // One node
        taskIDList, err := client.Cloud.GPUBaremetal.Clusters.Servers.ApplySettings(ctx,
            "{SERVER_ID}",
            cloud.GPUBaremetalClusterServerApplySettingsParams{
                ClusterID:     "{CLUSTER_ID}",
        ...
        // Every node in the cluster
        taskIDList, err = client.Cloud.GPUBaremetal.Clusters.ApplySettings(ctx,
            "{CLUSTER_ID}",
            cloud.GPUBaremetalClusterApplySettingsParams{
