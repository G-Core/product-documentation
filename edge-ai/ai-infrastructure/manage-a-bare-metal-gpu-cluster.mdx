...
ai-navigation: Manage Bare Metal GPU cluster nodes, resize instance count, configure power states, network interfaces, and delete individual nodes or entire clusters via the Gcore Customer Portal cluster details page.
---
...
3. Select the desired action:
   ...
   - **Replace**: Delete the current node and provision a new one with the same configuration (flavor, image, network settings). Use this to recover a node stuck in a failed state without changing the cluster setup.
...
- **Resize behavior**: Scaling down removes a random node. Use per-node delete for targeted removal. Scaling down to zero nodes is not supported — the minimum cluster size is one node.
