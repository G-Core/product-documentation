...
<p>The cluster details page displays summary information in the header panel:</p>
...
<p>The form shows the current **Image**, **SSH Key**, and **User data**. Click **Change** next to any field to select a new value. Click **Next** to proceed.</p>
...
<p>Under **Apply to**, choose **All Instances in Cluster** or **Selected Instances**. Click **Reconfigure** to confirm.</p>
...
<p>Rebuild reinstalls the OS on a node. The dialog shows the currently running **Image**, **SSH Key**, and **User data** — click **Change** next to any of them to use different values for this rebuild. All data on local storage is deleted.</p>
...
<p>Replace is also available via the [Replace bare metal GPU cluster server](/api-reference/cloud/gpu-bare-metal/replace-bare-metal-gpu-cluster-server) API endpoint.</p>
...
<p>The **Networking** tab displays network interfaces for each node. Click on a node name to expand its interface details.</p>
...
<p>Reinstalls the OS on a single node without affecting other nodes in the cluster. All local storage on the node is erased. Pass `image_id`, `ssh_key_name`, or `user_data` to use different values for this rebuild; omit any field to keep the cluster's current setting.</p>

<Tabs>
  <Tab title="curl">
    ```bash
    curl -X POST \
      "https://api.gcore.com/cloud/v4/gpu/baremetal/${GCORE_CLOUD_PROJECT_ID}/${GCORE_CLOUD_REGION_ID}/clusters/{CLUSTER_ID}/servers/{SERVER_ID}/rebuild" \
      -H "Authorization: APIKey ${GCORE_API_KEY}" \
      -H "Content-Type: application/json" \
      -d '{
        "image_id": "{IMAGE_ID}",
    ...
    <p>All body fields are optional. Omit any field to keep the cluster's current value.</p>
