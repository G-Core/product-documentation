...
ai-navigation: Manage Bare Metal GPU cluster nodes, resize instance count, configure power states, and network interfaces via Customer Portal or list and filter clusters via REST API using name, flavor, node count, image IDs, tags, and date range filters.
---
...
<p>Manage bare metal GPU clusters through the REST API: list, inspect, resize, control power state, rebuild, replace nodes, and delete.</p>
...
| `name` | `contains`, `prefix`, `exact` | Match cluster name (case-insensitive) |
| `flavor` | `prefix`, `exact` | Match flavor name (case-insensitive) |
...
| `created_at` | `gte`, `lte` | Filter by creation time (UTC ISO 8601) |
| `updated_at` | `gte`, `lte` | Filter by last-update time (UTC ISO 8601) |
| `tags` | `[key]=value` | Exact tag key-value pair. Multiple pairs are ANDed; values match case-insensitively |
| `tag_key` | `contains`, `prefix`, `exact` | Match tag key regardless of value |
| `tag_value` | `contains`, `prefix`, `exact` | Match tag value regardless of key |

<Info>
Cluster `status` is not available as a filter. It is computed from the underlying infrastructure and is not stored as a queryable field.
</Info>
...
<Tabs>
  <Tab title="Python SDK">
    ...
    page = client.cloud.gpu_baremetal.clusters.list(
        ...
        # Narrow to clusters whose name starts with "prod-"
        name={"prefix": "prod-"},
        # Narrow to a specific GPU flavor family
        flavor={"prefix": "bm3-"},
        # Narrow to clusters with 4 or more nodes
        servers_count={"gte": 4},
        # Narrow to clusters tagged env=production
        tags={"env": "production"},
    )
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        projectID, _ := strconv.Atoi(os.Getenv("GCORE_CLOUD_PROJECT_ID"))
        regionID, _ := strconv.Atoi(os.Getenv("GCORE_CLOUD_REGION_ID"))
        ...
        page, err := client.Cloud.GPUBaremetal.Clusters.List(context.TODO(),
            cloud.GPUBaremetalClusterListParams{
                ...
                Name:         &cloud.StringFilter{Prefix: "prod-"},
                Flavor:       &cloud.StringFilter{Prefix: "bm3-"},
                ServersCount: &cloud.IntRangeFilter{GTE: gcore.Int(4)},
                Tags:         cloud.TagFilter{"env": "production"},
            })
        if err != nil {
            panic(err)
        }
...
```bash
# Clusters provisioned with a specific image
curl -G "https://api.gcore.com/cloud/v3/gpu/baremetal/${GCORE_CLOUD_PROJECT_ID}/${GCORE_CLOUD_REGION_ID}/clusters" \
  -H "Authorization: APIKey ${GCORE_API_KEY}" \
  --data-urlencode "image_ids=3793c250-0b3b-4678-bab3-e11afbc29657"

# Clusters created after June 1, 2026
curl -G "https://api.gcore.com/cloud/v3/gpu/baremetal/${GCORE_CLOUD_PROJECT_ID}/${GCORE_CLOUD_REGION_ID}/clusters" \
  -H "Authorization: APIKey ${GCORE_API_KEY}" \
  --data-urlencode "created_at[gte]=2026-06-01T00:00:00Z"

# Clusters with node count between 4 and 16
curl -G "https://api.gcore.com/cloud/v3/gpu/baremetal/${GCORE_CLOUD_PROJECT_ID}/${GCORE_CLOUD_REGION_ID}/clusters" \
  -H "Authorization: APIKey ${GCORE_API_KEY}" \
  --data-urlencode "servers_count[gte]=4" \
  --data-urlencode "servers_count[lte]=16"

# Clusters whose tag value starts with "prod"
curl -G "https://api.gcore.com/cloud/v3/gpu/baremetal/${GCORE_CLOUD_PROJECT_ID}/${GCORE_CLOUD_REGION_ID}/clusters" \
  -H "Authorization: APIKey ${GCORE_API_KEY}" \
  --data-urlencode "tag_value[prefix]=prod"
```

<p>Use `limit` and `offset` for pagination. The default page size is 10; the maximum is 1000.</p>
...
The API returns a cluster object with `id`, `name`, `status`, `flavor`, `tags`, `servers_count`, `created_at`, `updated_at`, `servers_ids`, and `has_pending_changes` fields.
...
<p>Poll `GET /cloud/v1/tasks/{task_id}` every 5 seconds until `state` is `FINISHED`. Bare Metal H100 nodes may take 15–40 minutes to provision.</p>
...
<Tabs>
  <Tab title="Python SDK">
    ...
    # Replace "soft_reboot" with "start", "stop", or "hard_reboot" as needed
  ...
  <Tab title="Go SDK">
    ...
    func main() {
        ...
        taskIDList, err := client.Cloud.GPUBaremetal.Clusters.Action(context.TODO(),
            "{CLUSTER_ID}",
...
<Tabs>
  ...
  <Tab title="curl">
    ```bash
    curl -X DELETE \
      "https://api.gcore.com/cloud/v3/gpu/baremetal/${GCORE_CLOUD_PROJECT_ID}/${GCORE_CLOUD_REGION_ID}/clusters/{CLUSTER_ID}/servers/{SERVER_ID}" \
      -H "Authorization: APIKey ${GCORE_API_KEY}"
    ```
  </Tab>
</Tabs>
...
<p>Poll `GET /cloud/v1/tasks/{task_id}` every 5 seconds until `state` is `FINISHED`.</p>
