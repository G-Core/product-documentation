...
5\. Set up a [network interface](https://gcore.com/docs/cloud/networking/create-and-manage-a-network). You can choose a public or private one:
...
9\. (Optional) Add a [VAST file share](/cloud/file-shares/configure-file-shares) to your GPU cluster for high-performance storage:

  * Select an existing VAST file share or create a new one.
  * Specify the **Mount path** where the file share will be accessible on your cluster nodes.

<Frame>![Add file share to AI cluster](/images/docs/gpu-cloud/create-ai-cluster-file-share.png)</Frame>

<Info>
  When you add a VAST file share, the required network interfaces are automatically configured for each node in your cluster.
</Info>

By default, user data is disabled and the file share will be mounted automatically when the cluster starts. If you enable **User data**, mounting commands will be included in the user data script.

<Warning>
  If user data is enabled, do not modify or delete the auto-generated mounting commands, as this may break automatic mounting.
</Warning>
...
You've successfully created the cluster. Use the IP address of your AI Cluster and the SSH key from Step 10 to connect to your server.
