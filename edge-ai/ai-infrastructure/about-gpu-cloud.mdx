...
**SHARP (Scalable Hierarchical Aggregation and Reduction Protocol)** is automatically enabled on InfiniBand networks to further optimize performance. SHARP offloads collective communication operations like reductions (sum, max, etc.) directly to the network switches, minimizing data movement between nodes and reducing latency. This enhancement significantly accelerates distributed training and HPC workloads by optimizing gradient synchronization and other collective operations without requiring any manual configuration.
