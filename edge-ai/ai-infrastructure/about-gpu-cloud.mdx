...
<Info>
Virtual GPU clusters can be shelved (powered off) to stop billing. However, restart is not guaranteed if capacity is unavailable. Local disks are deleted when shelved. For guaranteed availability, use Bare Metal clusters.
</Info>
