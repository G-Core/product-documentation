...
## SHARP acceleration for InfiniBand clusters

For GPU clusters that use InfiniBand, Gcore enables SHARP (Scalable Hierarchical Aggregation and Reduction Protocol) to offload collective communication operations, such as reductions, directly to InfiniBand switches. This reduces data movement between nodes, lowers latency, and improves performance for distributed training and multi-node inference workloads.

SHARP reservations are managed automatically. When you create, scale, or delete a supported InfiniBand-based GPU cluster, the system creates, updates, or removes the corresponding SHARP allocations without any manual configuration in UFM. If SHARP or SHARP reservations are disabled in UFM, InfiniBand networking continues to work, but without SHARP-based acceleration.
