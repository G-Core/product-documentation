...
SHARP (Scalable Hierarchical Aggregation and Reduction Protocol) is an advanced feature automatically enabled on InfiniBand-equipped clusters. SHARP accelerates distributed AI and HPC workloads by offloading collective communication operations—such as reductions, broadcasts, and allreduce—directly to the InfiniBand switches.

Traditional collective operations require data to travel from GPUs to CPUs and back through multiple network hops. SHARP performs these operations in the network fabric itself, reducing latency and freeing GPU compute resources for actual training or inference work.

SHARP provides the most benefit for:

- **Multi-node distributed training** where frequent gradient synchronization occurs between nodes
- **Large-scale inference** where model weights are distributed across multiple servers
- **HPC simulations** requiring frequent data exchange between compute nodes

SHARP allocations are managed automatically during cluster operations. When scaling a cluster, SHARP reservations update automatically to include new nodes. When deleting nodes, SHARP reservations are released without manual intervention.

<Info>
SHARP requires compatible InfiniBand hardware and UFM (Unified Fabric Manager) configuration. All InfiniBand-equipped GPU clusters include SHARP support automatically—no additional setup or configuration is required.
</Info>
