...
To optimize performance further, clusters with InfiniBand automatically enable SHARP (Scalable Hierarchical Aggregation and Reduction Protocol). SHARP offloads collective communication operations like reductions (sum, max, etc.) directly to the network switches rather than processing them on the servers. This minimizes data movement between nodes, reduces latency, and significantly accelerates performance for HPC and AI workloads. SHARP allocations are managed automatically during cluster creation, scaling, and deletion—no manual configuration is required.

InfiniBand matters most for distributed training, where models that don't fit on a single node require frequent gradient synchronization between GPUs. The same applies to multi-node inference when large models are split across servers. In these cases, InfiniBand with SHARP reduces communication overhead significantly compared to Ethernet by optimizing how collective operations are processed across the network.
