...
A GPU cluster is a group of interconnected servers, each equipped with multiple high-performance GPUs. Clusters are designed for workloads that require massive parallel processing power, such as training large language models, fine-tuning foundation models, running inference at scale, and high-performance computing tasks.
...
|------|-------------|----------|
| **Bare Metal GPU** | Dedicated physical servers with guaranteed resources. No virtualization overhead. | Production workloads, long-running training jobs, latency-sensitive inference |
| **Spot Bare Metal GPU** | Same hardware as Bare Metal, but at reduced price (up to 50% discount). Instances can be preempted with 24-hour notice when capacity is needed. | Fault-tolerant training with checkpointing, batch processing, development and testing |
...
InfiniBand is configured automatically when you create a cluster. If the selected configuration includes InfiniBand network cards, all nodes are placed in the same InfiniBand domain with no manual setup required. H100 configurations typically have 8 InfiniBand ports per node, each creating a dedicated network interface.
...
When planning a GPU cluster, consider **provisioning time** (Bare Metal H100 servers take 15-40 minutes), **configuration immutability** (image, file shares, and network settings are fixed at creation), **data persistence** (local NVMe is temporary; use file shares for critical data), **resize constraints** (maximum nodes depend on regional stock), and **spot preemption** (Spot clusters may be interrupted with 24-hour notice). Hardware firewall support is available for servers with Bluefield network cards.
