...
Use a cluster when your workload requires more resources than a single node can provide or when scaling across multiple machines for distributed training or inference.

### What is the difference between a single GPU server and a cluster?

A single-GPU server runs workloads on a single node.

A cluster combines multiple nodes into one environment:

- Single server: 1–8 GPUs on one node
- Cluster: Multiple nodes with 16\+ GPUs in total

Use a single server for development or smaller workloads. Use a cluster for large models, distributed training, or high-scale inference.
...
- `ibstat` — verifies InfiniBand connectivity
