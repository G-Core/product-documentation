...
Gcore GPU clusters with InfiniBand automatically enable SHARP (Scalable Hierarchical Aggregation and Reduction Protocol), an advanced optimization that offloads collective communication operations like reductions and broadcasts directly to the network switches. SHARP minimizes data movement between nodes and reduces latency, providing additional performance benefits for HPC and AI workloads that rely on frequent inter-node communication. This optimization is configured automatically during cluster provisioning and operates transparently without requiring manual configuration.
