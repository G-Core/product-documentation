...
- **Spot Bare Metal GPU**: Discounted bare metal servers with non-guaranteed availability. Suitable for batch processing, training, and testing where interruptions are acceptable.

<Info>
  Spot instances can be reclaimed with 24 hours' notice. For details on the reclamation process and data preservation, see [Spot Bare Metal GPU](/edge-ai/ai-infrastructure/spot-bare-metal-gpu).
</Info>
...
For flavors with InfiniBand cards, high-speed inter-node networking is configured automatically. InfiniBand is a low-latency, high-bandwidth interconnect designed for GPU-to-GPU communication in distributed training. The platform creates a dedicated InfiniBand subnet (visible as "GPU-cluster B-subnet" in the interface list) with one interface per InfiniBand port (typically 8 for H100 configurations). No manual InfiniBand or NCCL (NVIDIA Collective Communications Library) configuration is required.
...
- **Distributed training**: Frameworks like PyTorch DDP (Distributed Data Parallel) or Horovod require shared storage for coordination
...
- **Persistence**: File shares are independent resources. Deleting a cluster does not delete attached file shares. One file share can be attached to multiple clusters in the same region.
...
2. Select an existing file share from the dropdown, or click **Create new File Share** to create one.
   ...
   - **Additional options**: Configure Root squash, Slurm (workload manager) compatibility, and character/path length settings as needed.
...
1. Select an existing firewall from the dropdown, or use the default firewall.

2. Click **Add Firewall** to attach additional firewalls if needed.
...
In the **SSH key** section, configure authentication:

1. Select an existing key from the dropdown, or create a new one:
   - To upload a public key, click **add a new SSH Key**.
   - To generate a new key pair, click **generate a SSH Key**. Save the private key immediately---it cannot be retrieved later.
