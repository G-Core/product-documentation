...
<Steps>
  ...
  <Step title="Configure cluster capacity">
    ...
    - **GPU Cluster type**: Choose **Bare Metal GPU** for dedicated servers, or **Spot Bare Metal GPU** for discounted interruptible capacity — see [Spot&nbsp;Bare&nbsp;Metal&nbsp;GPU](/edge-ai/ai-infrastructure/spot-bare-metal-gpu) for pricing and reclamation details.
  ...
  <Step title="Configure file share integration (optional)">
    File shares provide shared storage accessible from all cluster nodes. GPU-enabled regions use VAST-backed high-performance shares. This option appears only in regions where file shares are available — for share types, sizing, and access rules, see [file&nbsp;shares](/cloud/file-shares/configure-file-shares).
