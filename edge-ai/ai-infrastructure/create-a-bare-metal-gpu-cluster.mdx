...
For flavors with InfiniBand cards, high-speed inter-node networking is configured automatically. This includes SHARP (Scalable Hierarchical Aggregation and Reduction Protocol) optimization, which accelerates distributed workloads by offloading collective communication operations to the network switches. No manual network configuration is required for distributed training.

For detailed information about InfiniBand and SHARP features, see [About GPU Cloud](/edge-ai/ai-infrastructure/about-gpu-cloud#sharp-optimization).
...
The default Ubuntu images include pre-installed NVIDIA drivers and CUDA toolkit. Image names encode the software stack version. For example, `ubuntu-22.04-x64-nvidia-570-cuda-12.9-eni` indicates Ubuntu 22.04 with NVIDIA driver version 570 and CUDA Toolkit 12.9. The `eni` suffix (Enhanced Networking Interface) appears only on images configured for InfiniBand interconnect.
