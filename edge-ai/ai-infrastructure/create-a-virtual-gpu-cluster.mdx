...
Before creating a Virtual GPU cluster, confirm that the following are in place:
...
Creating a cluster consists of the following steps:
...
The **File share integration** section connects a [shared file system](/cloud/file-shares/configure-file-shares) to all instances in the cluster, making it accessible across nodes.

Use file share integration for workflows that require shared data access — distributed training, shared datasets, or any workload that reads the same files from multiple nodes.
...
In the **Network settings** section, select an [interface type](/cloud/networking/create-and-manage-a-network):
...
In the **SSH key** section, select an existing key from the dropdown or create a new one. Keys can be uploaded or generated directly in the Customer Portal. If generating a new key pair, save the private key immediately as it cannot be retrieved later.
...
Before clicking **Create Cluster**, review the estimated cost shown in the right panel.
