...
Each Virtual GPU cluster consists of one or more virtual machine nodes. All nodes share the same base configuration (image, network settings), but individual nodes can have different disk configurations after creation.

Each node has:

- A **local data disk** added by default. This disk cannot be edited and is deleted when the node is shelved.
- Optional **network disks** that can be attached during creation or added later. Network disks persist independently of node state.
...
Each Virtual GPU cluster node has two types of storage:

- **Local data disk**: Added automatically to every node. This disk cannot be edited or removed. Data on this disk is deleted when the node is shelved (powered off).
- **Network disks**: Optional persistent storage that survives power cycles. Up to 8 network disks of different sizes and types can be attached.
...
To add network disks:

1. Click **Add Disk** to add a network disk to the configuration.

2. For each disk, select:
   - **Disk type**: Select from available storage types in the region
   - **Size**: Minimum 120 GiB

3. Repeat to add additional disks (maximum 8 disks total).

All configured network disks are attached to every node in the cluster at creation.

<Warning>
Store important data on network disks, not on the local disk. The local disk is deleted when the node is shelved, resulting in data loss.
</Warning>
