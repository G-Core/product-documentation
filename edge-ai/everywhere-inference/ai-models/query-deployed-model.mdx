---
title: Query your deployed model
...
<Frame>![Deployment overview showing endpoint URL](/images/docs/edge-ai/everywhere-inference/ai-models/query-deployed-model/query-1.png)</Frame>
...
By default, API key authentication is disabled and the `api_key` / `apiKey` field is not validated — pass any non-empty string. When authentication is enabled on the deployment, pass the key as a Bearer token: `Authorization: Bearer <YOUR_API_KEY>`.
...
By default, the endpoint is publicly accessible. To restrict access, enable **API Key authentication** in the deployment [Settings](/edge-ai/everywhere-inference/api-keys/create-inference-deployment-with-auth).
