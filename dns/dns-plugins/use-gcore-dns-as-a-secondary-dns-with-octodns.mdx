...
ai-navigation: Configure Gcore managed DNS as secondary DNS using OctoDNS to synchronize zone records from primary DNS providers via Python virtualenv and YAML configuration.
ai-navigation: Configure Gcore Managed DNS as secondary DNS with OctoDNS provider, synchronizing zone records from primary DNS via API credentials.
---
