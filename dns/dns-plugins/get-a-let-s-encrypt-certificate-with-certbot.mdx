...
The Certbot plugin automates Let's Encrypt certificate issuance and renewal using the Gcore DNS API. To confirm domain ownership, Let's Encrypt uses a [DNS-01&nbsp;challenge](https://letsencrypt.org/docs/challenge-types): a TXT record with a specific value must be present under the domain name. The plugin creates and removes these TXT records automatically when obtaining, renewing, or revoking certificates. The [Certbot&nbsp;docs](https://eff-certbot.readthedocs.io/en/stable) cover general usage and advanced renewal configuration.

**Requirements:**

- A domain delegated to Gcore nameservers (`ns1.gcorelabs.net` and `ns2.gcdn.services`)
- A Gcore account

## Install the Certbot plugin

The commands below apply to Ubuntu 22.04. Other Linux distributions may require different package names.

<Steps>
<Step title="Connect to the server via SSH">
</Step>
...
Create a file named `gcore.ini` using any text editor:
...
Replace `*.example.com` with the target domain. Specifying `*.example.com` issues a wildcard certificate covering all subdomains.
...
- **Manual:** Run the following command to renew any previously issued certificate expiring within 30 days. Activate the virtual environment before running, and use the same working directory and parameters as during issuance.

```sh
certbot renew
```

- **Automatic (recommended):** Configure a `cron` daemon for background renewal. The [Certbot&nbsp;docs](https://eff-certbot.readthedocs.io/en/stable/using.html?highlight=cron) include `cron` daemon setup instructions.
