...
ai-navigation: Create and manage Gcore Managed DNS zones and records using Terraform provider v2 gcore_dns_zone and gcore_dns_zone_rrset resources, supporting A, AAAA, CNAME, MX, TXT, SRV, CAA, PTR record types, DNSSEC, SOA parameters, and geo-balancing with pickers and JSON-encoded meta values.
---
...
A PTR record maps an IP address to a hostname, used for reverse DNS lookups. PTR records must be created inside a dedicated reverse DNS zone: `<reversed-octets>.in-addr.arpa` for IPv4 or `<reversed-nibbles>.ip6.arpa` for IPv6.
...
- Specify `zone_name` — the reverse DNS zone name (e.g., `0.0.203.in-addr.arpa` for the `203.0.0.0/24` subnet).
- Specify `rrset_name` — the full reverse record name (e.g., `1.0.0.203.in-addr.arpa` for the IP `203.0.0.1`).
- Specify `content` — a single-element list with the target hostname followed by a trailing dot.
