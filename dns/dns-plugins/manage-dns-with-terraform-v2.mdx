...
The Gcore Terraform provider v2 lets you manage Gcore Managed DNS — zones, records, and geo-balancing — using declarative configuration files.
...
Provider v2 uses `api_key` for authentication and `gcore_dns_zone_rrset` for DNS records instead of the v0 `gcore_dns_zone_record`. Provider installation and authentication are covered in the [Terraform overview](/developer-tools/terraform/overview). To migrate an existing v0 configuration, follow the [migration guide](/developer-tools/terraform/migrate-v0-to-v2).
...
- Add `meta` inside each `resource_records` entry to associate geographic data with that IP:
  ...
  - (optional) `countries` — `jsonencode(["NL"])` — two-letter country code.

For more information, refer to [Configure weight balancing and geobalancing](/dns/dns-records/configure-weight-balancing-and-geobalancing).
...
<Tip>
After importing, run `terraform plan` to verify the configuration matches the imported state. Update your `main.tf` file to resolve any reported differences before running `terraform apply`.
</Tip>
