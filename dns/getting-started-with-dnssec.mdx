...
<Warning>
DNSSEC does not encrypt traffic between the end user and the resolver. For confidentiality and integrity at that layer, use [DNS over HTTPS (DoH) or DNS over TLS (DoT)](/dns/configure-doh-and-dot).
</Warning>
...
<p>DS propagation can take up to an hour at most registrars, and up to 24 hours in some cases. While waiting for DS confirmation, the **DNSSEC** toggle stays on and the zone header shows **Add DS record**, the activation time (**since**), and **Parent DS** as **Not found yet** — this is the transitional `pending` state, not disabled. After Gcore detects the DS record at the registrar, **Parent DS** updates and the zone status becomes `active`.</p>

<Frame>![Zone header showing Parent DS Not found yet and the Show DNSSEC Configuration tooltip](/images/docs/dns/getting-started-with-dnssec/dnssec-pending-header.png)</Frame>
...
|---|---|---|---|
| `pending` | DNSSEC is enabled, signing keys are generated, but Gcore has not yet detected the DS record at the registrar | Toggle on, **Add DS record**, activation time, **Parent DS** not yet confirmed (**Not found yet**) | Add the DS record Gcore provided to the registrar |
