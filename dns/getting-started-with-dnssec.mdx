...
sidebarTitle: DNSSEC
ai-navigation: Enable DNSSEC for Gcore Managed DNS zones to validate digital signatures across the chain of trust, obtain Delegation Signer (DS) records, and configure DS values in your domain registrar's control panel via API or portal.
---
...
<Tabs>
  <Tab title="DNS API">
    ...
    1\. [Create a domain zone](/api-reference/dns#tag/Zones/operation/CreateZone) for which you want to use DNSSEC using the following API request:
    ...
    **Tip**
    ...
    2\. [Create an RRSet](/api-reference/dns#tag/RRsets/operation/CreateRRSet) using the following API request:
  ...
  <Tab title="Customer Portal">
    ...
    <Frame>![Enable DNSSEC](/images/docs/dns/getting-started-with-dnssec/enable-dnssec-cp.png)</Frame>
    ...
    <Frame>![Copy the DS record](/images/docs/dns/getting-started-with-dnssec/ds-value-dnssec.png)</Frame>
