...
Without encrypted DNS, anyone between your device and the resolver — a public Wi-Fi operator, ISP, or network attacker — can see which domains you look up and forge responses. [Gcore Public DNS](/dns/public-dns) supports two encrypted protocols that prevent this. DNS queries are anonymized and processed under EU jurisdiction, and are not shared with third parties except as required by law.

[Gcore Public DNS](https://gcore.com/public-dns) supports two encrypted DNS protocols that close this gap.
...
Use the following addresses to configure DoH or DoT. For plain DNS (IPv4 and IPv6) addresses, see [Public DNS](/dns/public-dns).
...
Windows 11 knows a few DoH providers by default (Google, Cloudflare, Quad9), but Gcore is not among them. The DNS over HTTPS option in Settings only appears for addresses Windows recognizes, so you need to register Gcore first. Open PowerShell as Administrator (right-click Start → **Windows PowerShell (Admin)**) and run both commands:
...
<Note>
If the commands fail with an access error, your device is managed by an organization. Contact your IT administrator to enable DoH or configure it centrally.
</Note>

**Step 2 — configure the network adapter**

1. Open **Settings** > **Network & internet**.
2. Select **Wi-Fi** or **Ethernet**, then click the name of your active connection.
...
Other providers such as Google and Cloudflare may also appear in the list — that is expected.

**What changed:** DNS queries from this device are now encrypted and routed through Gcore. Your ISP and anyone else on the network path can no longer read which domains you look up or tamper with the responses. Browsing speed and behavior are unaffected.
...
Until an official profile is available, configure DoH in the browser (see the Browsers tab) or install [Stubby](https://dnsprivacy.org/dns_privacy_daemon_-_stubby/) for system-wide DoT:
...
<p>Choose based on your init system: if `systemd-resolved` is running (Ubuntu 20.04+, Fedora, Arch), follow the **systemd-resolved** steps below. On all other distributions, skip to **Stubby**.</p>
...
For Android 8 and earlier, which do not support encrypted DNS, see [Configure a DNS resolver](/dns/configure-plain-dns).

</MethodSection>

<MethodSection id="ios" label="iOS and iPadOS">

<p>iOS 14 and later support system-wide DoH and DoT through configuration profiles. There is no built-in UI for encrypted DNS — a profile must be installed.</p>

Until an official profile is available, system-wide encrypted DNS is not available on iOS. For per-browser DoH, use Firefox or Chrome on iOS and configure a custom DoH provider in the browser settings — note that browser DoH settings on iOS may behave differently than on desktop.
...
3. In the provider dropdown, choose the custom provider option and enter:
   ```
...
3. In the provider dropdown, choose the option to add a custom provider.
...
1. Go to **Services** > **DNS Resolver** (Unbound) > **General Settings**.
...
The `Server:` line should show the router's LAN IP (for example, `192.168.1.1`), confirming the device sends DNS queries to the router. The router then forwards them to Gcore over port 853 — check **WAN** > **Internet Connection** to confirm the DoT connection status is active.
