...
Gcore Managed DNS provides CNAME flattening to customers on all plans by default. To configure it, navigate to the [Gcore Customer Portal](https://portal.gcore.com), open **DNS** → **Managed DNS**, click the zone name, and open the **Records** tab.

1\. In the records list, click the delete icon next to the A record for the root domain. In the **Delete record** dialog that opens, click **Yes, delete**.
...
2\. Click **Add record**, select **CNAME** as the type, enter the root domain in the name field, enter the target domain in the **Content** field, and click **Add**. The zone must contain exactly one CNAME record at the root for flattening to work correctly.
...
The [Gcore&nbsp;DNS&nbsp;Lookup](https://gcore.com/dev-tools/dns-lookup) tool verifies the CNAME record's resolved value.