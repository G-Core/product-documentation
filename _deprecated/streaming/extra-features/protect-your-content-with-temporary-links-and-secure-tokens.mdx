...
<Tabs>
  ...
  <Tab title="❌ Don't use secure tokens if...">
    **You just need to control WHERE content is available:**
    ...
    - **Geo-restrictions only**: Use [Country access policy](https://gcore.com/docs/cdn/cdn-resource-options/security/control-access-to-the-content-with-country-referrer-ip-and-user-agents-policies#country-access-policy) instead
      - Example: "Stream only available in Dominican Republic"
      - This is simpler and doesn't require backend token generation

    - **Domain restrictions only**: Use [Referrer validation](https://gcore.com/docs/cdn/cdn-resource-options/security/control-access-to-the-content-with-country-referrer-ip-and-user-agents-policies#allow-referrer-policy) instead
      - Example: "Video can only be embedded on mywebsite.com"
      - No token generation needed

    - **Public content**: If your content is free and publicly accessible, you don't need any protection

    - **Legal authorization**: If you're asking about copyright/broadcasting rights clearance, this is a legal question—contact [support@gcore.com](mailto:support@gcore.com)
...
<Warning>
**Important:** Secure tokens require backend development to generate tokens for each user/request. If you just need geo-blocking or domain restrictions, use the simpler CDN security policies instead.
</Warning>
...
Gcore has several methods of protecting video content:
  ...
  1. **Protected temporary links with secure token** (this article) - Controls WHO can access
  2. [**Country access policy (geo-restrictions)**](/cdn/cdn-resource-options/security/control-access-to-the-content-with-country-referrer-ip-and-user-agents-policies#country-access-policy) - Controls WHERE content is available
  3. [**Referrer validation**](/cdn/cdn-resource-options/security/control-access-to-the-content-with-country-referrer-ip-and-user-agents-policies#allow-referrer-policy) - Controls which websites can embed
  4. [**AES-128 encryption**](/streaming/video-hosting/protect-your-videos-with-the-aes-128-encryption) - Encrypts video content
...
### Quick decision guide: Which protection method do I need?

| Your requirement | Protection method | Complexity |
|-----------------|-------------------|------------|
| "Only allow viewers in specific countries" | [Country access policy](https://gcore.com/docs/cdn/cdn-resource-options/security/control-access-to-the-content-with-country-referrer-ip-and-user-agents-policies#country-access-policy) | ⭐ Easy (CDN setting) |
| "Only allow embedding on my website" | [Referrer validation](https://gcore.com/docs/cdn/cdn-resource-options/security/control-access-to-the-content-with-country-referrer-ip-and-user-agents-policies#allow-referrer-policy) | ⭐ Easy (CDN setting) |
| "Only paid/logged-in users can watch" | Secure tokens (this article) | ⭐⭐⭐ Advanced (requires backend) |
| "Encrypt video files for DRM" | [AES-128 encryption](https://gcore.com/docs/streaming/video-hosting/protect-your-videos-with-the-aes-128-encryption) | ⭐⭐⭐ Advanced (requires key management) |
| "Prevent unauthorized downloads" | Combine secure tokens + AES-128 | ⭐⭐⭐ Advanced |

<Info>
**Most common scenario:** If you just need to restrict content by country (e.g., "only available in Dominican Republic for broadcasting rights"), use **Country access policy**—not secure tokens. It's much simpler and doesn't require backend development.
</Info>
...
- Open access on one CDN resource (e.g., `public.gvideo.io`)
- Protected access on another CDN resource (e.g., `protected.gvideo.io`)
- Geo-restricted access on a third resource (e.g., `us-only.gvideo.io`)

**Example configurations:**

| CDN resource | Security policy | Use case |
|-------------|----------------|----------|
| `demo-protected.gvideo.io` | Secure tokens (no IP binding) | Paid content accessible from any location |
| `demo-protected-ip.gvideo.io` | Secure tokens (with IP binding) | Enhanced security with IP validation |
| `demo-geo.gvideo.io` | Country access policy | Content available only in specific countries |
| `demo-public.gvideo.io` | No restrictions | Free, public content |

<Info>
**To create custom CDN resources:** Contact [support team](mailto:support@gcore.com) or follow the [CDN resource creation guide](https://gcore.com/docs/cdn/about-cdn-resources-interface-how-it-is-arranged). Support will help configure `vp_rules` to route your streaming content correctly.
</Info>
