...
When pinging a server, packet loss or response times above 110 milliseconds indicate a network issue. The steps below apply to servers that are still reachable. For completely unreachable servers, a [separate&nbsp;guide](/hosting/dedicated-servers/troubleshooting/troubleshoot-a-server-that-is-not-responding-to-ping-requests) covers that scenario.
