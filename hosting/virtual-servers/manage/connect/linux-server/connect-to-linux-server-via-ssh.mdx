---
title: Connect to a Linux server via SSH
sidebarTitle: SSH connection
ai-navigation: Connect to Linux virtual servers via SSH using credentials from the Hosting portal Instructions tab, from any terminal on macOS, Linux, or Windows.
---

SSH provides encrypted remote access to Linux virtual servers — Windows servers connect via [RDP](/hosting/dedicated-servers/manage/connect/connect-to-a-windows-server).

<Steps>
  <Step title="Get connection credentials">
    Log in to the [Hosting&nbsp;portal](https://hosting.gcore.com) and navigate to **Products/Services** > **Virtual private servers**. Select the server row by checking its checkbox, then click **Instructions** in the toolbar.

    <Frame>
      ![Virtual private servers list with a server row selected and the Instructions button highlighted in the toolbar](/images/docs/hosting/virtual-servers/manage/connect/linux-server/connect-to-linux-server-via-ssh/connect-to-linux-server-via-ssh-image1.png)
    </Frame>

    The Instructions page lists the server IP address, username, and password needed for the SSH connection.

    <Frame>
      ![Virtual server activation page showing server information including IP address, username, and password](/images/docs/hosting/virtual-servers/manage/connect/linux-server/connect-to-linux-server-via-ssh/connect-to-linux-server-via-ssh-image2.png)
    </Frame>
  </Step>
  <Step title="Connect to the server">
    Open a terminal. On macOS and Linux, use the built-in Terminal app. On Windows, use Command Prompt, PowerShell, or Windows Terminal.

    Run the SSH command using the username and IP address from the Instructions page:

    ```sh
    ssh <username>@<server_ip>
    ```
  </Step>
  <Step title="Enter the password">
    At the password prompt, enter the server password from the Instructions page.

    <Note>
      Some SSH clients do not display characters as they are typed. Enter the password and press Enter — the connection establishes even though the screen appears blank.
    </Note>
  </Step>
</Steps>
