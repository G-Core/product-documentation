...
<Info>
The original video provided from your side must strictly comply with the input parameters and codec requirements described in the [Input Parameters and Codecs](/docs/streaming-platform/live-streams-and-videos-protocols-and-codecs/input-parameters-and-codecs) documentation.  
...
To copy a video from another server, specify the attribute `origin_url` in the [POST API request](/docs/api-reference/streaming/videos/create-video). The original video will be downloaded for Video Hosting on our server side. 
...
Once successfully uploaded and processed, the video will be available in the Customer Portal. You can check the status of the video by the [GET API request](/docs/api-reference/streaming/videos/get-video) or via webhook [notifications](/streaming-platform/extra-features/get-webhooks-from-the-streaming-platform). 
...
Specify the main parameters, and do not use the `origin_url` attribute in the [POST API request](/docs/api-reference/streaming/videos/create-video).
...
To upload a file, you must get a TUS server URL and secret token. Specify the "id" parameter from step 1 and [get the TUS session parameters](/docs/api-reference/streaming/videos/get-tus-parameters-for-direct-upload).
